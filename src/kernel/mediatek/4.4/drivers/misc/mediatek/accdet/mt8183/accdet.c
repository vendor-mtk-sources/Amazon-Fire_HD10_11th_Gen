...
#endif
#ifdef CONFIG_ACCDET_TS3A226AE
struct typeswitch_gpio_attr {
	const char *name;
	bool gpio_prepare;
	struct pinctrl_state *gpioctrl;
};
enum typeswitch_gpio_type {
	GPIO_TS3A226AE_OFF = 0,
	GPIO_TS3A226AE_ON,
	GPIO_NUM
};
static struct typeswitch_gpio_attr typeswitch_gpios[GPIO_NUM] = {
	[GPIO_TS3A226AE_OFF] = {"typeswitch_off", false, NULL},
	[GPIO_TS3A226AE_ON] = {"typeswitch_on", false, NULL},
};
#endif
...
#ifdef CONFIG_ACCDET_TS3A226AE
static inline int accdet_TS3A226AE_setup_gpio(struct platform_device *device)
{
	int index_gpio = 0;
	int ret;

	pinctrl_switchIC = devm_pinctrl_get(&device->dev);
	if (IS_ERR(pinctrl_switchIC)) {
		ret = PTR_ERR(pinctrl_switchIC);
		ACCDET_ERROR("[accdet] Cannot find pinctrl_switchIC ret = %d !\n", ret);
		return ret;
	}
	for (index_gpio = 0; index_gpio < ARRAY_SIZE(typeswitch_gpios); index_gpio++) {
		typeswitch_gpios[index_gpio].gpioctrl =
			pinctrl_lookup_state(pinctrl_switchIC,
			typeswitch_gpios[index_gpio].name);
		if (IS_ERR(typeswitch_gpios[index_gpio].gpioctrl)) {
			ret = PTR_ERR(typeswitch_gpios[index_gpio].gpioctrl);
			ACCDET_ERROR("[accdet] %s pinctrl_lookup_state %s fail %d\n",
				 __func__, typeswitch_gpios[index_gpio].name, ret);
		} else {
			typeswitch_gpios[index_gpio].gpio_prepare = true;
			pr_debug("[accdet] %s pinctrl_lookup_state %s success!\n",
				 __func__, typeswitch_gpios[index_gpio].name);
		}
	}
	return 0;
}
...
{
	if (typeswitch_gpios[GPIO_TS3A226AE_ON].gpio_prepare) {
		pinctrl_select_state(pinctrl_switchIC,
			typeswitch_gpios[GPIO_TS3A226AE_ON].gpioctrl);
...
{
	if (typeswitch_gpios[GPIO_TS3A226AE_OFF].gpio_prepare) {
		pinctrl_select_state(pinctrl_switchIC,
			typeswitch_gpios[GPIO_TS3A226AE_OFF].gpioctrl);
...
/* #elif defined CONFIG_ACCDET_SUPPORT_BI_EINT */
	...
	if (ret == 0) {
		...
		#endif
#ifdef CONFIG_ACCDET_TS3A226AE
		accdet_TS3A226AE_setup_gpio(dev);
#endif
