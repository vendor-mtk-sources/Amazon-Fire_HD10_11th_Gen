	...
	s32 thread, void *iwc_ptr)
{
	...
	iwc->command.metadata.srcHandle = task->secData.srcHandle;
#endif

#ifndef CONFIG_MTK_CMDQ_TAB
	/* mtee part has no isp metadata */
	...
	struct cmdqCommandStruct *pCommandDesc)
{
	...
	task->cmd_buffer_va = (u32 *)(uintptr_t)pCommandDesc->pVABase;
	...
	struct TaskStruct *task)
{
	...
	task->secData.enginesDisablePortSecurity =
		desc->secData.enginesDisablePortSecurity;
	task->secData.enginesDisableDAPC =
		desc->secData.enginesDisableDAPC;
