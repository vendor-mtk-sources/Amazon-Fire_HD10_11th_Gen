...
#define BMA253_ID_REG_VAL	0xfa

/* Chip power modes */
#define BMA180_LOW_POWER	0x03

#define BMA250_INT_STATUS0_REG	0x09
#define BMA250_INT_MAP0_REG	0x19
#define BMA250_RANGE_REG	0x0f
#define BMA250_BW_REG		0x10
#define BMA250_POWER_REG	0x11
#define BMA250_RESET_REG	0x14
#define BMA250_INT0_ENABLE_REG	0x16
#define BMA250_INT_ENABLE_REG	0x17
#define BMA250_INT_MAP0_REG	0x19
#define BMA250_INT_MAP_REG	0x1a
#define BMA250_INT_MAP2_REG	0x1b
#define BMA250_INT_OUT_REG	0x20
...
#define BMA253_BW_MASK		GENMASK(2, 0) /* Accel bandwidth */
#define BMA250_SUSPEND_MASK	BIT(7) /* chip will sleep */
#define BMA250_LOWPOWER_MASK	BIT(6)
#define BMA250_DATA_INTEN_MASK	BIT(4)
#define BMA250_INT1_DATA_MASK	BIT(0)
#define BMA250_INT0_DTAP_MASK	BIT(4)
#define BMA250_INT0_STAP_MASK	BIT(5)
#define BMA250_INT_RESET_MASK	BIT(7) /* Reset pending interrupts */

#define BMA250_INT_MAP_MASK	0x30	/* int[1/2]_s_tap | int[1/2]_d_tap */
#define BMA250_INT_MAP_VAL	0x3
#define BMA250_INT_OUT_MASK	0xf	/*int2_od int2_lvl int1_od int1_lvl */
#define BMA250_INT_RST_MASK	GENMASK(3, 0)	/* latch_int */
#define BMA250_INT_RST_VAL	0xe	/* 50ms */
#define BMA250_INT_8_MASK	GENMASK(2, 0)	/* tap_dur */
#define BMA250_INT_8_VAL	0x4     /* 250ms */
#define BMA250_INT_9_MASK	GENMASK(4, 0)	/* tap_th */
#define BMA250_INT_9_VAL	0xa
#define BMA250_INT0_ENABLE_MASK	0x30	/* s_tap_en | d_tap_en */
#define BMA250_INT0_ENABLE_VAL	0x3
...
static int bma253_bw_table[] = { 8, 16, 31, 63, 125, 250, 500, 1000 }; /* Hz */
static int bma253_scale_table[] = { 0, 0, 0, 2000000, 0, 4000000, 0, 0,
	8000000, 0, 0, 0, 16000000, 0, 0, 0};
...
static int bma180_chip_init(struct bma180_data *data)
{
	...
	if ((ret != BMA180_ID_REG_VAL) && (ret != BMA253_ID_REG_VAL))
		return -ENODEV;
...
static int bma253_chip_config(struct bma180_data *data)
{
	u8 irq_conf;
	u32 irq_type;
	int ret = bma180_chip_init(data);
	...
	ret = bma180_set_bw(data, 1000); /* 16 Hz */
	if (ret)
		goto err;

	ret = bma180_set_scale(data, 2000000); /* 2 G */
	if (ret)
		goto err;

	/* route interrupts to INT1 pin */
	ret = bma180_set_bits(data, BMA250_INT_MAP0_REG,
		BMA250_INT_MAP_MASK, BMA250_INT_MAP_VAL);
	if (ret)
		goto err;

	irq_type = irqd_get_trigger_type(irq_get_irq_data(data->client->irq));

	if (irq_type & (IRQ_TYPE_LEVEL_LOW | IRQ_TYPE_EDGE_FALLING))
		irq_conf = 0xa; /* opendrain, activelow */
	else
		irq_conf = 0xf; /* opendrain, activehigh */

	/* electrical behavior for INT[1&2] pins */
	ret = bma180_set_bits(data, BMA250_INT_OUT_REG,
		BMA250_INT_OUT_MASK, irq_conf);
	if (ret)
		goto err;

	/* interrupt latching */
	ret = bma180_set_bits(data, BMA250_INT_RESET_REG,
		BMA250_INT_RST_MASK, BMA250_INT_RST_VAL);
	if (ret)
		goto err;

	/* tap duration */
	ret = bma180_set_bits(data, BMA250_INT_8_REG,
		BMA250_INT_8_MASK, BMA250_INT_8_VAL);
	if (ret)
		goto err;

	/* tap threshold */
	ret = bma180_set_bits(data, BMA250_INT_9_REG,
		BMA250_INT_9_MASK, BMA250_INT_9_VAL);
	if (ret)
		goto err;

	/* enable interrupts */
	ret = bma180_set_bits(data, BMA250_INT0_ENABLE_REG,
		BMA250_INT0_ENABLE_MASK, BMA250_INT0_ENABLE_VAL);
	if (ret)
		goto err;
...
static const struct iio_chan_spec bma253_channels[] = {
...
static const struct bma180_part_info bma180_part_info[] = {
	...
	[BMA253] = {
		bma253_channels, ARRAY_SIZE(bma253_channels),
		bma253_scale_table, ARRAY_SIZE(bma253_scale_table),
		bma253_bw_table, ARRAY_SIZE(bma253_bw_table),
		BMA250_INT_RESET_REG, BMA250_INT_RESET_MASK,
		BMA250_POWER_REG, BMA250_SUSPEND_MASK,
		BMA250_BW_REG, BMA253_BW_MASK,
		BMA250_RANGE_REG, BMA250_RANGE_MASK,
		BMA250_POWER_REG, BMA250_LOWPOWER_MASK, 1,
		BMA250_INT0_ENABLE_REG, BMA250_INT0_ENABLE_MASK,
		BMA250_RESET_REG,
		bma253_chip_config,
		bma250_chip_disable,
	},
...
static int bma180_trig_try_reen(struct iio_trigger *trig)
{
	struct iio_dev *indio_dev = iio_trigger_get_drvdata(trig);
	struct bma180_data *data = iio_priv(indio_dev);
	u8 val = i2c_smbus_read_byte_data(data->client,
						BMA250_INT_STATUS0_REG);

	dev_dbg(&data->client->dev, "%s 0x%x\n", (val & BMA250_INT0_STAP_MASK) ?
		"<< Single TAP >>" : (val & BMA250_INT0_DTAP_MASK) ?
		"<< Double TAP >>" : "Unkown interupt", val);
		...
		const struct i2c_device_id *id)
{
	...
	if (client->irq > 0) {
		...
		ret = devm_request_threaded_irq(&client->dev, client->irq, NULL,
			iio_trigger_generic_data_rdy_poll, IRQF_ONESHOT,
