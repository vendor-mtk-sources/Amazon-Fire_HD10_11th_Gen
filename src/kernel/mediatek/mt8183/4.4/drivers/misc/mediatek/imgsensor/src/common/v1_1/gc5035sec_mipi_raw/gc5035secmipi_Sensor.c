...
#include "gc5035secmipi_Sensor.h"
#include "gc5035sec_Otp.h"

/************************** Modify Following Strings for Debug **************************/
#define PFX "gc5035sec_camera_sensor"
#define LOG_1 LOG_INF("GC5035SECMIPI, 2LANE\n")
/****************************   Modify end    *******************************************/
...
static kal_uint32 Dgain_ratio = GC5035SEC_SENSOR_DGAIN_BASE;

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC5035SEC_SENSOR_ID,
...
#if GC5035SEC_MIRROR_FLIP_ENABLE
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	if (reg_gain < GC5035SEC_SENSOR_GAIN_BASE)
		reg_gain = GC5035SEC_SENSOR_GAIN_BASE;
	else if (reg_gain > GC5035SEC_SENSOR_GAIN_MAX)
		reg_gain = GC5035SEC_SENSOR_GAIN_MAX;
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	kal_uint16 GC5035SEC_AGC_Param[GC5035SEC_SENSOR_GAIN_MAP_SIZE][2] = {
		{  256,  0 },
		{  302,  1 },
		{  358,  2 },
		{  425,  3 },
	...
	for (gain_index = GC5035SEC_SENSOR_GAIN_MAX_VALID_INDEX - 1; gain_index > 0; gain_index--)
		if (reg_gain >= GC5035SEC_AGC_Param[gain_index][0])
			break;

	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xb6, GC5035SEC_AGC_Param[gain_index][1]);
	temp_gain = reg_gain * Dgain_ratio / GC5035SEC_AGC_Param[gain_index][0];
	write_cmos_sensor(0xb1, (temp_gain >> 8) & 0x0f);
	write_cmos_sensor(0xb2, temp_gain & 0xfc);
	LOG_DBG("Exit! GC5035SEC_AGC_Param[gain_index][1] = 0x%x, temp_gain = 0x%x, reg_gain = %d\n",
		GC5035SEC_AGC_Param[gain_index][1], temp_gain, reg_gain);
...
static kal_uint32 streaming_control(kal_bool enable)
{
	...
	if (enable) {
		while (retry < 6) {
			...
			usleep_range(66666, 66666);
			streamingReg = read_cmos_sensor(0x3e);
			if (streamingReg == 0x91)
				break;
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x17, GC5035SEC_MIRROR);
	...
	write_cmos_sensor(0x54, GC5035SEC_RSTDUMMY1);
	...
	write_cmos_sensor(0x22, GC5035SEC_RSTDUMMY2);
	...
	UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_REGISTER:
		...
		LOG_DBG("adb_i2c_read 0x%x = 0x%x\n", sensor_reg_data->RegAddr, sensor_reg_data->RegData);
		break;
...
UINT32 GC5035SEC_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
