...
#if defined(CONFIG_MACH_MT6775) || defined(CONFIG_MACH_MT8183)
...
char *dvfs_get_opp_info(int opp)
{
	struct opp_profile *opp_ctrl_table = opp_table;
	static char opp_info[64];

	memset(opp_info, 0, sizeof(opp_info));
	if (opp >= 0 && opp < NUM_OPP)
		scnprintf(opp_info, 64, "[OPP%d] vcore_uv: %d , ddr_khz : %d ",
					opp,
...
static void set_vcorefs_en(void)
{
	...
	spm_go_to_vcorefs(flag);
	mutex_unlock(&governor_mutex);
#if defined(CONFIG_MACH_MT6759) || defined(CONFIG_MACH_MT6758) || defined(CONFIG_MACH_MT8183)
