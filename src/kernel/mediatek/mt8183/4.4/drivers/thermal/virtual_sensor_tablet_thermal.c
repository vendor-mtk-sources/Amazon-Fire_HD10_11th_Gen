...
#ifdef CONFIG_AMAZON_METRICS_LOG
		/* Log in metrics around every 50 mins normally
...
static void virtual_sensor_thermal_tripsdata_convert(char *type, unsigned long *tripsdata, struct trip_t *trips, int trips_num)
{
	int i = 0;

	if (trips_num > THERMAL_MAX_TRIPS)
		trips_num = THERMAL_MAX_TRIPS;
...
static void virtual_sensor_thermal_init_trips(const struct device_node *np, struct trip_t *trips, int trips_num)
{
	...
	virtual_sensor_thermal_tripsdata_convert("temp", tripsdata, trips, trips_num);
	virtual_sensor_thermal_parse_node_int_array(np, "type", tripsdata);
	virtual_sensor_thermal_tripsdata_convert("type", tripsdata, trips, trips_num);
	virtual_sensor_thermal_parse_node_int_array(np, "hyst", tripsdata);
	virtual_sensor_thermal_tripsdata_convert("hyst", tripsdata, trips, trips_num);
}
...
static int virtual_sensor_thermal_init_cust_data_from_dt(struct platform_device *dev)
{
	...
	virtual_sensor_thermal_parse_node_string_index(np, "governor_name", 0, (char *)&p_virtual_sensor_thermal_data->tzp.governor_name);
	thermal_parse_node_int(np, "num_tbps", &p_virtual_sensor_thermal_data->tzp.num_tbps);
	virtual_sensor_thermal_init_trips(np, p_virtual_sensor_thermal_data->trips, p_virtual_sensor_thermal_data->num_trips);
