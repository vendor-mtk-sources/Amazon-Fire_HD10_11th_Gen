...
#define DELAY_READ_REG						250  /* unit:ms */
...
extern int g_debugfs_enable_latency_check;
static int FT8201_support;
/*****************************************************************************
...
int fts_reset_proc(int hdelayms)
{
	...
	if (FT8201_support == 1)
		ft8201_share_tp_resetpin_low();
	else
		ft8201ab_share_tp_resetpin_low();
	msleep(1);
	/* tpd_gpio_output(fts_data->pdata->reset_gpio, 1); */
	if (FT8201_support == 1)
		ft8201_share_tp_resetpin_high();
	else
		ft8201ab_share_tp_resetpin_high();
	if (hdelayms) {
		msleep(hdelayms);
	}
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
	...
	if (FT8201_support == 1)
		ft8201_share_request_powerpin_resource(&ts_data->client->dev);
	else
		ft8201ab_share_request_powerpin_resource(&ts_data->client->dev);
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (ft8201_incell_compare_id() != VID_NULL) {
		FT8201_support = 1;
		FTS_INFO("ft8201 is supported");
	} else if (ft8201ab_incell_compare_id() != VID_NULL) {
		FT8201_support = 0;
		FTS_INFO("ft8201ab is supported");
	} else {
		FT8201_support = -1;
		FTS_ERROR("lcm_id does not match ft8201 and ft8201ab incell IC, exit.");
		return -ENODEV;
	}
...
void tpd_suspend(struct device *dev)
{
	int ret = 0;
	struct fts_ts_data *ts_data = fts_data;
	...
	if (ts_data->gesture_mode) {
		...
		if (FT8201_support == 1)
			ft8201_share_powerpin_off();
		else
			ft8201ab_share_powerpin_off();
	}
...
void tpd_resume(struct device *dev)
{
	struct fts_ts_data *ts_data = fts_data;
	...
	if (!ts_data->gesture_mode) {
		if (FT8201_support == 1)
			ft8201_share_powerpin_on();
		else
			ft8201ab_share_powerpin_on();
	}
	msleep(DELAY_READ_REG);
	fts_wait_tp_to_valid();
	fts_ex_mode_recovery(ts_data);
