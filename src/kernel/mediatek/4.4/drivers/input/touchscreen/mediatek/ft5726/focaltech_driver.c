...
struct tpd_ft5726_priv {
	unsigned int tpd_rotate_degree;
};
...
static int fts_report_value(struct ts_event *data)
{
	...
	for (i = 0; i < (buf.fingers) && (i < CFG_MAX_TOUCH_POINTS);
		...
		if (touch->event == 0 || touch->event == 2) {
			priv = i2c_get_clientdata(i2c_client);

			if (priv->tpd_rotate_degree == 90) {
				tpd_rotate_90(&x, &y);
			} else if (priv->tpd_rotate_degree == 180) {
				tpd_rotate_180(&x, &y);
			} else if (priv->tpd_rotate_degree == 270) {
				tpd_rotate_270(&x, &y);
			} else if (priv->tpd_rotate_degree != 0) {
				pr_warn("[focal] tpd_rotate_degree=%u is not supported",
					priv->tpd_rotate_degree);
			}
...
{
	struct tpd_ft5726_priv *priv = dev_get_drvdata(dev);
	if (dev->of_node) {
	...
	of_property_read_u32(dev->of_node, "tpd_rotate_degree", &priv->tpd_rotate_degree);

	pr_debug("[focal] tpd_rst_gpio= %d, tpd_int_gpio= %d, irq = %d, tpd_rotate_degree=%u\n",
	       tpd_rst_gpio_number, tpd_int_gpio_number, touch_irq, priv->tpd_rotate_degree);
	return 0;
}
#if 0
			   ...
			   const struct i2c_device_id *id)
{
	...
	priv = devm_kzalloc(&i2c_client->dev, sizeof(*priv), GFP_KERNEL);
	if (!priv)
		return -ENOMEM;
