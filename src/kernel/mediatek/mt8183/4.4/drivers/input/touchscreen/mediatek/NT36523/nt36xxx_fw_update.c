...
#if NVT_MULT_BIN
struct nvt_upgrade_module nvt_module_list[] = {
	{VID_NT_TG_BOE, PID_TG_BOE, "TG_BOE", "NVT_FW_TG_BOE_V0x07.bin", "NVT_MP_Criteria_TG_BOE.csv"},
};

int nvt_module_num = sizeof(nvt_module_list)/sizeof(struct nvt_upgrade_module);
#endif
...
void Boot_Update_Firmware(struct work_struct *work)
{
	...
	NVT_LOG("Panel_ID=%x fw_ver=%02X, pid=%04X\n", temp_id, ts->fw_ver, ts->nvt_pid);
	for (i = 0; i < nvt_module_num; i++) {
		info = &nvt_module_list[i];
		if (temp_id == info->id) {
			sprintf(firmware_name, info->fw_name);
			break;
		}
	}
#else
	...
	if (atomic_read(&ts->i2c_enable) == 0) {
		NVT_ERR("i2c is not enabled during suspend, failed to upgrade fw!");
