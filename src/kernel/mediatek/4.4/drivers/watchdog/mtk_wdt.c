...
#define WDT_NONRST2_RECOVERY      (1 << 1)
#define WDT_NONRST2_BOOTLOADER    (1 << 2)
...
struct mtk_wdt_dev {
	...
	int wdt_hw_timeout;
	unsigned long last_wdt_pet;
	unsigned long saved_last_wdt_pet;
	struct delayed_work wdt_delay_task;
};

static void __iomem *toprgu_base;
static struct watchdog_device *wdt_dev;

static void wdt_update_task(struct work_struct *work)
{
	struct mtk_wdt_dev *mtk_wdt;

	mtk_wdt = container_of(work, struct mtk_wdt_dev, wdt_delay_task.work);
	if (mtk_wdt->last_wdt_pet == mtk_wdt->saved_last_wdt_pet) {
		pr_alert("no user side petting, last wdg pet %lu\n", mtk_wdt->last_wdt_pet);
		/* schedule timer every second */
		schedule_delayed_work(&mtk_wdt->wdt_delay_task, msecs_to_jiffies(1000));
	}
}
				...
				void *cmd)
{
	...
	reg &= ~(WDT_MODE_DUAL_EN | WDT_MODE_IRQ_EN | WDT_MODE_EN | WDT_MODE_AUTO_START | WDT_MODE_IRQ_LVL);
	reg |= WDT_MODE_KEY;
	iowrite32(reg, wdt_base + WDT_MODE);
	if (cmd && !strcmp(cmd, "rpmbp-eng")) {
	...
	} else if (cmd && !strcmp(cmd, "rpmbp-prod")) {
		nonrst_reg2 = ioread32(wdt_base + WDT_NONRST_REG2);
		nonrst_reg2 |= WDT_NONRST2_RPMB_PROG_KEY;
		nonrst_reg2 |= WDT_NONRST2_RPMB_PROG_KEY_EXTEND;
		iowrite32(nonrst_reg2, wdt_base + WDT_NONRST_REG2);
	} else if (cmd && (strstr(cmd, "recovery"))) {
		iowrite32(ioread32(wdt_base + WDT_NONRST_REG2) | WDT_NONRST2_RECOVERY, wdt_base + WDT_NONRST_REG2);
		#ifdef CONFIG_MT6397_MISC
		...
		iowrite32(ioread32(wdt_base + WDT_NONRST_REG2) | WDT_NONRST2_BOOTLOADER, wdt_base + WDT_NONRST_REG2);
		#ifdef CONFIG_MT6397_MISC
		mtk_misc_mark_fast();
		#else
		rtc_mark_fast();
...
static int mtk_wdt_ping(struct watchdog_device *wdt_dev)
{
	...
	mtk_wdt->last_wdt_pet = jiffies;
	cancel_delayed_work_sync(&mtk_wdt->wdt_delay_task);

	/*
	 * Set a timer for 80% of HW WDT timeout (mtk_wdt->wdt_hw_timeout/5)
	 * So before HW WDT timeout is reached and system is rebooted
	 * Print every second message that HW timer will expire
	 * and no petting from user side has occurred.
	 */
	if (mtk_wdt->wdt_hw_timeout / 5) {
		mtk_wdt->saved_last_wdt_pet = mtk_wdt->last_wdt_pet;
		schedule_delayed_work(&mtk_wdt->wdt_delay_task, msecs_to_jiffies((mtk_wdt->wdt_hw_timeout * 4/5) * 1000));
	}
				...
				unsigned int timeout)
{
	...
	wdt_dev->timeout = timeout;
	mtk_wdt->wdt_hw_timeout = timeout;
...
static int mtk_wdt_start(struct watchdog_device *wdt_dev)
{
	...
	reg = ioread32(wdt_base + WDT_MODE);
	reg |= (WDT_MODE_DUAL_EN | WDT_MODE_IRQ_EN | WDT_MODE_EXRST_EN | WDT_MODE_IRQ_LVL);
...
bool mtk_wdt_mode_config_for_sysrq(void)
{
	...
	reg |= WDT_MODE_KEY | WDT_MODE_AUTO_START | WDT_MODE_IRQ_LVL;
...
static int mtk_wdt_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&mtk_wdt->wdt_delay_task, wdt_update_task);
