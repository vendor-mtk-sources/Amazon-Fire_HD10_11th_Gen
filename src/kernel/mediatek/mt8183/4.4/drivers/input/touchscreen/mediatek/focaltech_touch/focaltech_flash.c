...
/* Focaltec is using Kelli C instead of IDC tool. */
#define SUPPORT_VENDOR_IDC_TOOL 0
/*****************************************************************************
...
#if FTS_AUTO_LIC_UPGRADE_EN
	ret = fts_read_reg(FTS_REG_LIC_NEW_VER, ver);
#else
...
static int fts_fwupg_get_ver_in_tp(u8 *ver)
{
	...
	ret = fts_read_reg(FTS_REG_FACTORY_MODE_DETACH_FLAG, &rev_ver);
	if (ret < 0) {
		FTS_ERROR("read Reverse fw ver from tp fail");
		return ret;
	}

	FTS_INFO("fw ver=%X rev_ver=%X", *ver, rev_ver);
	if ((*ver + rev_ver) != 0xFF) {
		FTS_ERROR("0xFF fw check fail ver=%X rev_ver=%X", *ver, rev_ver);

		ret = fts_read_reg(FTS_REG_FW_VER, ver);
		if (ret < 0) {
			FTS_ERROR("read fw ver from tp again 1 fail");
			return ret;
		}
		...
		FTS_INFO("read fw ver from tp again 1 fw ver=%X rev_ver=%X", *ver, rev_ver);

		ret = fts_read_reg(FTS_REG_POWER_MODE, &val);
		if (ret < 0) {
			FTS_ERROR("read powermode fail");
			return ret;
		}
		if (val == FTS_REG_POWER_MODE_MONITOR) {
			FTS_INFO("make TP enter into active mode");
			...
			ret = fts_read_reg(FTS_REG_FW_VER, ver);
			if (ret < 0) {
				FTS_ERROR("read fw ver from tp again 2 fail");
				return ret;
			}
			ret = fts_read_reg(FTS_REG_FACTORY_MODE_DETACH_FLAG, &rev_ver);
			if (ret < 0) {
			...
			FTS_INFO("read fw ver from tp again 2 fw ver=%X rev_ver=%X", *ver, rev_ver);

		}
		return -EINVAL;
	}
	if (*ver == 1) {
		FTS_ERROR("error fw ver=1");
		return -EINVAL;
	}
	return 0;
