...
static void startTimer(struct hrtimer *timer, int delay_ms, bool first)
{
	...
	if (first) {
		obj->target_ktime = ktime_add_ns(ktime_get(),
		...
		do {
			obj->target_ktime = ktime_add_ns(obj->target_ktime,
						(int64_t)delay_ms*1000000);
...
static void acc_work_func(struct work_struct *work)
{
	...
	/* cxt->drv_data.acc_data.values[1], */
	/* cxt->drv_data.acc_data.values[2]); */
...
static struct acc_context *acc_context_alloc_object(void)
{
	...
	/*5Hz ,  set work queue delay time 200ms */
...
static int acc_enable_and_batch(void)
{
	...
	if (atomic_read(&cxt->acc_pause)) {
		...
		ACC_PR_ERR("already in pause, just save enable state: %d\n",
	...
	for (i = 0; i < MAX_CHOOSE_G_NUM; i++) {
		if ((i == 0) && (gsensor_init_list[0] == NULL)) {
			...
			if (platform_driver_register(&gsensor_driver)) {
				ACC_PR_ERR("failed to register gensor ");
				ACC_PR_ERR("driver already exist\n");
			}
		}
			  ...
			  size_t count, loff_t *ppos)
{
	...
	read_cnt = sensor_event_read(acc_context_obj->mdev.minor,
...
DEVICE_ATTR(accenablenodata, S_IWUSR | S_IRUGO, acc_show_enable_nodata,
