...
static int as6200_init(struct iio_dev *indio_dev)
{
	struct as6200_data *data = iio_priv(indio_dev);
	int err, value;
	s32 temp_check_low_limit, temp_check_high_limit;
	...
	if (!of_property_read_s32(data->client->dev.of_node,
				 "temp_check_low_limit",
				 &temp_check_low_limit)) {
		/*
		 * One LSB in the threshold register corresponds to 0.0625 C
		 * Hence divide the temp value from DTS by the resolution.
		 */
		value = (((temp_check_low_limit * 10000) / 625) / 1000) << 4;
		dev_info(&data->client->dev,
				"Temp check low limit is %d , write value %d\n",
				temp_check_low_limit, value);
		err = regmap_write(data->regmap, AS6200_REG_TLOW, value);
		if (err < 0)
			return err;
	}

	if (!of_property_read_s32(data->client->dev.of_node,
				 "temp_check_high_limit",
				 &temp_check_high_limit)) {
		/*
		 * One LSB in the threshold register corresponds to 0.0625 C
		 * Hence divide the temp value from DTS by the resolution.
		 */
		value = (((temp_check_high_limit * 10000) / 625) / 1000) << 4;
		dev_info(&data->client->dev,
				"Temp check high limit is %d, write value %d\n",
				temp_check_high_limit,value);
		err = regmap_write(data->regmap, AS6200_REG_THIGH, value);
		if (err < 0)
			return err;
	}
	...
	const struct i2c_device_id *id)
{
	...
	if (device_property_present(&indio_dev->dev, "interrupt-parent")) {
