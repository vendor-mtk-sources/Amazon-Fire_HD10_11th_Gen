...
static void msdc_set_mclk(struct msdc_host *host, unsigned char timing, u32 hz)
{
	...
	if (host->sclk <= 52000000) {
		...
		if (host->base_top != NULL) {
			writel(host->def_tune_para.sd_top_control,
			       host->base_top + MSDC_TOP_CONTROL);
			writel(host->def_tune_para.sd_top_cmd,
			       host->base_top + MSDC_TOP_CMD);
		} else {
				...
				struct mmc_command *cmd, struct mmc_data *data)
{
	...
	/*
	 * Dump register vaule if it is not a TMO error
	 * No need to dump register if it is a Tuning cmd
	 * Dump the register only one time
	 */
...
static void msdc_init_hw(struct msdc_host *host)
{
	...
	if (host->base_top) {
		writel(0, host->base_top + MSDC_TOP_CONTROL);
		writel(0, host->base_top + MSDC_TOP_CMD);
	} else {
	...
	if (host->dev_comp->async_fifo) {
		...
		if (host->dev_comp->enhance_rx) {
			if (host->base_top != NULL)
				sdr_set_bits(host->base_top + MSDC_TOP_CONTROL,
					     SDC_RX_ENH_EN);
			else
	...
	if (host->dev_comp->data_tune) {
		if (host->base_top != NULL) {
			...
			sdr_clr_bits(host->base_top + MSDC_TOP_CONTROL,
				     DATA_K_VALUE_SEL);
			sdr_set_bits(host->base_top + MSDC_TOP_CMD,
	...
	if (host->base_top != NULL) {
		host->def_tune_para.sd_top_control =
			readl(host->base_top + MSDC_TOP_CONTROL);
		host->def_tune_para.sd_top_cmd =
			readl(host->base_top + MSDC_TOP_CMD);
...
static int execute_online_tuning(struct msdc_host *host, u8 *res)
{
	...
	if (autok_pad_dly_sel(&uCmdDatInfo) == 0) {
		/* Appling DAT tuning result to CMD */
		autok_param_update(CMD_EDGE, uCmdDatInfo.opt_edge_sel,
				   p_autok_tune_res);
		autok_paddly_update(CMD_PAD_RDLY, uCmdDatInfo.opt_dly_cnt,
				    p_autok_tune_res);
...
{
	if (sdio_host) {
		pr_info("SDIO Performing DVFS AUTOK.\n");

		if (vcorefs_request_dvfs_opp(KIR_AUTOK_SDIO, OPP_0)!= 0)
			pr_err("DVFS request failed...\n");

		mmc_claim_host(sdio_host->mmc);
		spm_msdc_dvfs_setting(KIR_AUTOK_SDIO, 1);
		autok_execute_tuning(sdio_host, sdio_autok_res);
