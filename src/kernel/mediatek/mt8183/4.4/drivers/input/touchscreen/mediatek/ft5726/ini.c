...
*/
	...
	while (1) {
		...
		g_st_ini_file_data[g_used_key_num].iSectionNameLen =
				strnlen(tmpSectionName, sizeof(tmpSectionName));
		pSectionName_size =
		sizeof(g_st_ini_file_data[g_used_key_num].pSectionName);

		if (g_st_ini_file_data[g_used_key_num].iSectionNameLen >=
				pSectionName_size) {
			pr_err("CFG_ERR_OUT_OF_LEN\n");
			ret = CFG_ERR_OUT_OF_LEN;
			goto cfg_scts_end;
		}

		strncpy(g_st_ini_file_data[g_used_key_num].pSectionName,
			tmpSectionName, pSectionName_size);
