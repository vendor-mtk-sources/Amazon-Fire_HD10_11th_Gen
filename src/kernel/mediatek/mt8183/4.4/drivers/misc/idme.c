...
char *idme_get_sensorcal(s16 *data)
{
	struct device_node *ap = NULL;
	char *sensorcal = NULL;

	pr_info("idme_get_sensorcal enter\n");
	...
	pr_info("idme_get_sensorcal exit\n");
...
{
	struct device_node *ap = NULL;
	const char *alscal = NULL;
	int res;

	pr_info("idme_get_alscal_value enter\n");
	...
	if (unlikely(kstrtouint(alscal, 10, &res)))
		pr_err("idme_get_board_rev kstrtouint failed!\v");
