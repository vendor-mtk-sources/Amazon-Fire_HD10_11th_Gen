...
#define DL_GAIN_L_REG_MASK 0x001f
#define DL_GAIN_R_REG_MASK 0x0f80
...
{
	int retval;
	if (strcmp(CONFIG_ARCH_MTK_PROJECT, "abc123") == 0 || strcmp(CONFIG_ARCH_MTK_PROJECT, "trona") == 0
		|| strcmp(CONFIG_ARCH_MTK_PROJECT, "abx123") == 0) {
...
static int Ext_Speaker_Amp_Gain_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	pr_aud("%s()\n", __func__);
	ucontrol->value.integer.value[0] =
	    mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_EXT_SPK_AMP];
...
static int Ext_Speaker_Amp_Gain_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	int retval = 0;
	pr_aud("%s() gain = %ld\n ", __func__, ucontrol->value.integer.value[0]);
	if (mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_EXTSPKAMP])
		retval = AudDrv_GPIO_EXT_SPK_AMP_Select(true, mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_EXT_SPK_AMP]);
	if (retval == 0)
		mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_EXT_SPK_AMP] = ucontrol->value.integer.value[0];
	return retval;
}
...
static int Ext_Headphone_Amp_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	int retval;
	pr_aud("%s() switch = %ld\n ", __func__, ucontrol->value.integer.value[0]);
	retval = AudDrv_GPIO_EXT_HP_AMP_Select(ucontrol->value.integer.value[0],
		mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_EXT_HP_AMP]);
	if (retval == 0)
		mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_EXTHPAMP] = ucontrol->value.integer.value[0];
	return retval;
}

static int Ext_Headphone_Amp_Gain_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	pr_aud("%s()\n", __func__);
	ucontrol->value.integer.value[0] =
	    mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_EXT_HP_AMP];
...
static int Ext_Headphone_Amp_Gain_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	int retval = 0;
	pr_aud("%s() gain = %ld\n ", __func__, ucontrol->value.integer.value[0]);
	if (mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_EXTHPAMP])
		retval = AudDrv_GPIO_EXT_HP_AMP_Select(true, mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_EXT_HP_AMP]);
	if (retval == 0)
		mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_EXT_HP_AMP] = ucontrol->value.integer.value[0];
	return retval;
}
...
/* ext amp use */
static const char *const EXT_SPK_AMP_GAIN[] = { "11dB", "14dB" };

/* ext hp amp use */
static const char *const EXT_HP_AMP_GAIN[] = { "0dB", "6dB" };
...
#ifdef CONFIG_MT8183_QUERY_PCB_ID
...
static const struct soc_enum Audio_DL_Enum[] = {
	...
	// for EXT AMP, power/volume of SPK/HP path
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(amp_function), amp_function),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(EXT_SPK_AMP_GAIN), EXT_SPK_AMP_GAIN),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(amp_function), amp_function),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(EXT_HP_AMP_GAIN), EXT_HP_AMP_GAIN)
};
...
#endif
	SOC_ENUM_EXT("Ext_Speaker_Amp_Switch", Audio_DL_Enum[14],
		     Ext_Speaker_Amp_Get,
		     Ext_Speaker_Amp_Set),
	SOC_ENUM_EXT("Ext_Speaker_Amp_Gain", Audio_DL_Enum[15],
		     Ext_Speaker_Amp_Gain_Get,
		     Ext_Speaker_Amp_Gain_Set),
	SOC_ENUM_EXT("Ext_Headphone_Amp_Switch", Audio_DL_Enum[16],
		     Ext_Headphone_Amp_Get,
		     Ext_Headphone_Amp_Set),
	SOC_ENUM_EXT("Ext_Headphone_Amp_Gain", Audio_DL_Enum[17],
		     Ext_Headphone_Amp_Gain_Get,
		     Ext_Headphone_Amp_Gain_Set),
};
...
static void ADC_LOOP_DAC_Function(int mode)
{
	pr_debug("%s value = %d\n", __func__, mode);
...
static int Codec_Loopback_Mode_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	ADC_LOOP_DAC_Function(Codec_Loopback_Mode);

	return 0;
}

static const struct soc_enum Pmic_Loopback_Test_Enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(Pmic_Loopback_Mode), Pmic_Loopback_Mode),
};

static const struct snd_kcontrol_new mt6358_pmic_loopback_Test_controls[] = {
	SOC_ENUM_EXT("Codec_Loopback_Setting", Pmic_Loopback_Test_Enum[0],
...
void InitCodecDefault(void)
{
	...
	mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_EXT_SPK_AMP] = 1;
	mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_EXT_HP_AMP] = 0;
...
static int mt6358_codec_probe(struct snd_soc_codec *codec)
{
	...
	snd_soc_add_codec_controls(codec, mt6358_pmic_loopback_Test_controls,
				   ARRAY_SIZE(mt6358_pmic_loopback_Test_controls));
