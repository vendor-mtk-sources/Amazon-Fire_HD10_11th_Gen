...
struct _GLUE_INFO_T {
	...
	BOOLEAN fgIsAntSwitchDisable;
	WLAN_STATUS rNewOrientationStatus;
	BOOLEAN fgIsEnableAntSwQuery;
	BOOLEAN fgIsHwSupportAntSw;
	UINT_8 ucPanelType;
	UINT_8 ucPanelIdmePattern;
	UINT_8 ucASGoodPanel;
	BOOLEAN fgWas1stSwitch;
	BOOLEAN fgIsEnableDelaySwap;
	UINT_32 u4AsDelayMSec;
#endif
...
#if CFG_SUPPORT_ANT_DIVERSITY
#define GLUE_NOT_SUPPORT_DUAL_ANT(p) \
	((p)->fgIsHwSupportAntSw == FALSE || \
	((p)->ucPanelIdmePattern & \
	BIT(((((p)->ucASGoodPanel & \
	BIT((p)->ucPanelType)) != 0 ? 1 : 0) << 1) | \
	(p)->fgIsAntSwitchDisable)) == FALSE)
#endif
...
#if CFG_SUPPORT_ANT_DIVERSITY
extern int wmt_plat_is_support_ant_sw(void);
extern int disp_lcm_get_vendor_id(void);
#endif
