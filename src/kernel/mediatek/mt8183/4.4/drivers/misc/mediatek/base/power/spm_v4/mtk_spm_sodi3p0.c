...
static char sodi3_status[300];
char *sodi3_map_status(u32 step)
{
	char *p = sodi3_status;
	char *buff_end = sodi3_status + sizeof(sodi3_status);

	memset(sodi3_status, 0, sizeof(sodi3_status));
	if (step == 0)
		return "leave sodi3 successful !!";

	if (step & (1 << SPM_SODI3_ENTER))
		p += scnprintf(p, buff_end-p, "Enter sodi3 |");
	if (step & (1 << SPM_SODI3_ENTER_SSPM_ASYNC_IPI_BEFORE_WFI))
		p += scnprintf(p, buff_end-p, "Enter sspm async ipi before wfi |");
	if (step & (1 << SPM_SODI3_ENTER_SPM_FLOW))
		p += scnprintf(p, buff_end-p, "Enter spm flow |");
	if (step & (1 << SPM_SODI3_ENTER_UART_SLEEP))
		p += scnprintf(p, buff_end-p, "Enter uart sleep |");
	if (step & (1 << SPM_SODI3_B4))
		p += scnprintf(p, buff_end-p, "Enter sodi b4 |");
	if (step & (1 << SPM_SODI3_B5))
		p += scnprintf(p, buff_end-p, "Enter sodi b5 |");
	if (step & (1 << SPM_SODI3_B6))
		p += scnprintf(p, buff_end-p, "Enter sodi b6 |");
	if (step & (1 << SPM_SODI3_ENTER_WFI))
		p += scnprintf(p, buff_end-p, "Enter wfi |");
	if (step & (1 << SPM_SODI3_LEAVE_WFI))
		p += scnprintf(p, buff_end-p, "Leave wfi |");
	if (step & (1 << SPM_SODI3_ENTER_UART_AWAKE))
		p += scnprintf(p, buff_end-p, "Leave uart sleep |");
	if (step & (1 << SPM_SODI3_LEAVE_SSPM_ASYNC_IPI_AFTER_WFI))
		p += scnprintf(p, buff_end-p, "Leave sspm async ipi after wfi |");
	if (step & (1 << SPM_SODI3_LEAVE_SPM_FLOW))
		p += scnprintf(p, buff_end-p, "Leave spm flow |");
	if (step & (1 << SPM_SODI3_LEAVE))
		p += scnprintf(p, buff_end-p, "Leave sodi3");
