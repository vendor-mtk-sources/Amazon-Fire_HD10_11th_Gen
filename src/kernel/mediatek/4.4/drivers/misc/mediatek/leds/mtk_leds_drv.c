...
#endif
#ifdef CONFIG_BACKLIGHT_RESUME_SYNC_TO_LCM
static unsigned int g_last_level = 1;
#endif
/****************************************************************************
 * DEBUG MACROS
 ***************************************************************************/
...
/* max_limit is the imposed brightness limit from other things such as thermal or power. */
static unsigned int max_limit = 255;
...
int setMaxbrightness(int max_level, int enable)
{
	...
	if (current_level > max_limit)
		current_level = max_limit;
...
#ifdef CONTROL_BL_TEMPERATURE
static ssize_t true_brightness_show(struct device *dev,
					struct device_attribute *attr, char *buf)
{
		LEDS_DRV_DEBUG("get thermal brightness %u\n", current_level);
			return sprintf(buf, "%u\n", current_level);
}
static DEVICE_ATTR(true_brightness, 0440, true_brightness_show, NULL);

static ssize_t max_limit_brightness_show(struct device *dev,
						struct device_attribute *attr, char *buf)
{
		LEDS_DRV_DEBUG("get max limit brightness %d\n", max_limit);
			return sprintf(buf, "%u\n", max_limit);
}
static DEVICE_ATTR(max_limit_brightness, 0440, max_limit_brightness_show, NULL);
#endif
...
extern unsigned int leds_normalize_enable;

static ssize_t show_normalize_enable(struct device *dev, struct device_attribute *attr,
			char *buf)
{
	return sprintf(buf, "%d\n", leds_normalize_enable);
}

static ssize_t store_normalize_enable(struct device *dev, struct device_attribute *attr,
			 const char *buf, size_t size)
{
	int ret, cus_val;
...
static DEVICE_ATTR(normalize_enable, 0664, show_normalize_enable, store_normalize_enable);
#endif
...
{
	struct mt65xx_led_data *led_data = container_of(self, struct mt65xx_led_data,
					fb_notifier);
	...
	if (evdata && evdata->data) {
		...
		LEDS_DRV_INFO("%s: event: %lu, blank: %d for LED %s", __func__,
				event, blank, led_data->cust.name);

		if (blank == FB_BLANK_UNBLANK) {
			if (FB_EVENT_BLANK == event) {
				if (strcmp(led_data->cust.name, "lcd-backlight") == 0) {
					/* Only restore brightness when user space did not */
					if (led_data->last_level && !led_data->level) {
						mt65xx_led_set(&led_data->cdev,
								led_data->last_level);
						LEDS_DRV_INFO("%s: restore bl brightness to %d",
								__func__, led_data->last_level);
					}
				}
			}
			...
			if (FB_EARLY_EVENT_BLANK == event) {
				if (strcmp(led_data->cust.name, "lcd-backlight") == 0) {
					/* Save curent brightness */
					led_data->last_level = led_data->level;
					/* Only turn off backlight LED when user space did not */
					if (led_data->level) {
						mt65xx_led_set(&led_data->cdev, LED_OFF);
						LEDS_DRV_INFO("%s: turn off bl LED", __func__);
					}
				}
			}
		}
...
static inline void mt65xx_leds_register_fb(unsigned int i)
{
	g_leds_data[i]->fb_notifier.notifier_call = fb_notifier_callback;
	fb_register_client(&g_leds_data[i]->fb_notifier);
}

static inline void mt65xx_leds_unregister_fb(unsigned int i)
{
	fb_unregister_client(&g_leds_data[i]->fb_notifier);
}
...
static int mt65xx_leds_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
		...
		mt65xx_leds_register_fb(i);

		g_leds_data[i]->cust.ncust_table = cust_led_list[i].ncust_table;
		g_leds_data[i]->cust.cust_bri_table = cust_led_list[i].cust_bri_table;

#ifdef CONTROL_BL_TEMPERATURE
		if (strcmp(g_leds_data[i]->cdev.name, "lcd-backlight") == 0) {
			rc = device_create_file(g_leds_data[i]->cdev.dev,
				&dev_attr_true_brightness);
			if (rc)
				LEDS_DRV_DEBUG("device_create_file true_brightness falid\n");
			rc = device_create_file(g_leds_data[i]->cdev.dev,
				&dev_attr_max_limit_brightness);
			if (rc)
				LEDS_DRV_DEBUG("device_create_file max_limit_brightness falid\n");
		}
#endif
#ifdef CONFIG_NORMALIZE_PANEL_BRIGHTNESS_SUPPORT
		rc = device_create_file(g_leds_data[i]->cdev.dev, &dev_attr_normalize_enable);
		if (rc) {
			LEDS_DRV_DEBUG("device_create_file normalize_enable fail!\n");
		}
#endif
		#if 0
		if (strcmp(g_leds_data[i]->cdev.name, "lcd-backlight") == 0) {
 ...
 err:
	if (i) {
		for (i = i - 1; i >= 0; i--) {
			if (!g_leds_data[i])
				continue;
			mt65xx_leds_unregister_fb(i);
			if (g_leds_data[i]->cust.cust_bri_table)
				kfree(g_leds_data[i]->cust.cust_bri_table);
