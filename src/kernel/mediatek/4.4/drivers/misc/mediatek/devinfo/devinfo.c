...
static ssize_t devinfo_brom_cmd_dis_read(struct file *filp, char __user *buf, size_t len, loff_t *ppos)
{
	char devinfo_brom_cmd_dis_buff[128];
	u32 ret = get_devinfo_with_index(BROM_CMD_DIS_INDEX);
	if(ret != 0xffffffff) {
		sprintf(devinfo_brom_cmd_dis_buff, "brom_cmd_dis=0x%x\n", (ret & (1 << BROM_CMD_DIS_OFFSET)) >> BROM_CMD_DIS_OFFSET);
		return simple_read_from_buffer(buf, len, ppos, devinfo_brom_cmd_dis_buff, strlen(devinfo_brom_cmd_dis_buff));
	}
	return -EINVAL;
}

static const struct file_operations devinfo_brom_cmd_dis_fops = {
	.owner = THIS_MODULE,
	.read = devinfo_brom_cmd_dis_read,
};
...
static int __init devinfo_init(void)
{
	...
	if (!debugfs_create_file("brom_cmd_dis", 0444, devinfo_segment_root, NULL, &devinfo_brom_cmd_dis_fops))
		return -ENOMEM;
