...
static void eem_set_eem_volt(struct eem_det *det)
{
	...
	dsb(sy);
#if EN_EEM_STAT
	/* dump the DVFS table here for big-data statistic in SMT, it include the info below, */
	/* - Pmic, the voltage write to pmic */
	/* - Orig, the original table, (should be ATE calibration result?) */
	/* - MaxLimit, the voltage calibrated on the current mainboard */
	if (statDump == 1) {
		tmp_offset_val = det->volt_offset + low_temp_offset - aging_val;
		switch (det->ctrl_id) {
		case EEM_CTRL_2L:
			for (i = 0; i < det->num_freq_tbl; i++)
				printk("[EEM_CTRL_2L] freq %d, pmic %d, orig %d, ate %d, tbl %d\n", (det->max_freq_khz * det->freq_tbl[i]) / 100,
					det->ops->pmic_2_volt(det, det->volt_tbl_pmic[i]),
					det->ops->pmic_2_volt(det, det->volt_tbl_orig[i]),
					det->ops->pmic_2_volt(det, det->volt_tbl[i] + det->volt_offset),
					det->ops->pmic_2_volt(det, det->volt_tbl[i] + tmp_offset_val));
			break;
		case EEM_CTRL_L:
			for (i = 0; i < det->num_freq_tbl; i++)
				printk("[EEM_CTRL_L] freq %d, pmic %d, orig %d, ate %d, tbl %d\n", (det->max_freq_khz * det->freq_tbl[i]) / 100,
					det->ops->pmic_2_volt(det, det->volt_tbl_pmic[i]),
					det->ops->pmic_2_volt(det, det->volt_tbl_orig[i] + MARGIN_ADD_OFF_VER4),
					det->ops->pmic_2_volt(det, det->volt_tbl[i] + det->volt_offset),
					det->ops->pmic_2_volt(det, det->volt_tbl[i] + tmp_offset_val));
			break;
		case EEM_CTRL_CCI:
			for (i = 0; i < det->num_freq_tbl; i++)
				printk("[EEM_CTRL_CCI] freq %d, pmic %d, orig %d, ate %d, tbl %d\n", (det->max_freq_khz * det->freq_tbl[i]) / 100,
					det->ops->pmic_2_volt(det, det->volt_tbl_pmic[i]),
					det->ops->pmic_2_volt(det, det->volt_tbl_orig[i]),
					det->ops->pmic_2_volt(det, det->volt_tbl[i] + det->volt_offset),
					det->ops->pmic_2_volt(det, det->volt_tbl[i] + tmp_offset_val));
		...
		case EEM_CTRL_GPU:
			for (i = 0; i < det->num_freq_tbl; i++)
				printk("[EEM_CTRL_GPU] freq %d, pmic %d, orig %d, ate %d, tbl %d\n", (det->max_freq_khz * det->freq_tbl[i]) / 100,
					det->ops->pmic_2_volt(det, det->volt_tbl_pmic[i]),
					det->ops->pmic_2_volt(det, det->volt_tbl_orig[i]),
					det->ops->pmic_2_volt(det, det->volt_tbl[i] + det->volt_offset),
					det->ops->pmic_2_volt(det, det->volt_tbl[i] + tmp_offset_val));
			statDump = 0;
			printk("[EEM_CTRL] table all dumped");
			break;
		default:
			break;
		}
