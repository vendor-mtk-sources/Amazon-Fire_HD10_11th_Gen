...
VOID nicRxProcessDataPacket(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
	...
	/* non BA session */
		...
		fgDrop = TRUE;
		DBGLOG(RSN, ERROR, "fgDrop packet\n");
		if (!HAL_RX_STATUS_IS_ICV_ERROR(prRxStatus)
			...
			*/
			if (prSwRfb->ucPayloadFormat
				...
				DBGLOG(RSN, ERROR, "change fgDrop to FALSE\n");
				fgDrop = FALSE;
			...
			if ((prSwRfb->ucGroupVLD & BIT(RX_GROUP_VLD_3)) == 0) {
				...
				if ((ucRxMode == RX_VT_LEGACY_CCK) || (ucRxMode == RX_VT_LEGACY_OFDM)) {
					...
					prStaRec->u2LastPhyRate =
						nicGetHwRateByPhyRate (ucRxRate) * 5;
				} else {
					ucMcs = ((prRxStatusGroup3)->u4RxVector[0] & RX_VT_RX_RATE_AC_MASK);
					/* VHTA1 B0-B1 */
					ucFrMode = (((prRxStatusGroup3)->u4RxVector[0] & RX_VT_FR_MODE_MASK) >> RX_VT_FR_MODE_OFFSET);
					...
					/* ucRate(500kbs) = u4PhyRate(100kbps) / 5, max ucRate = 0xFF */
					prStaRec->u2LastPhyRate = nicGetPhyRateByMcsRate (ucMcs, ucFrMode, ucShortGI);
				}
			}
		}
...
VOID nicRxProcessEventPacket(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb, int is_wakeup)
{
	...
	case EVENT_ID_BSS_BEACON_TIMEOUT:
		...
		if (prAdapter->fgDisBcnLostDetection == FALSE) {
			...
			rssi = RCPI_TO_dBm(prEventBssBeaconTimeout->ucRcpi);

			if (prEventBssBeaconTimeout->ucBssIndex >= BSS_INFO_NUM)
				break;

			DBGLOG(RX, INFO, "Beacon Timeout Reason: %d, update bad RSSI: %d to upper layer\n",
			       prEventBssBeaconTimeout->ucReasonCode, rssi);
#if CFG_SUPPORT_EXCEPTION_STATISTICS
			prAdapter->total_beacon_timeout_count++;
			if (prEventBssBeaconTimeout->ucReasonCode >= BEACON_TIMEOUT_TYPE_NUM) {
				DBGLOG(RX, WARN, "Invaild Beacon Timeout Reason: %d\n",
									prEventBssBeaconTimeout->ucReasonCode);
				break;
			} else {
				prAdapter->beacon_timeout_count[prEventBssBeaconTimeout->ucReasonCode]++;
			}
#endif
