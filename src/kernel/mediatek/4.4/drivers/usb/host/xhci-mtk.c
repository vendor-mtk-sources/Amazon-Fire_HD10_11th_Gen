...
int xhci_mtk_init_sram(struct xhci_hcd *xhci)
{
	...
	for (i = 0; i < XHCI_SRAM_BLOCK_NUM; i++) {
		...
		pr_debug("[%d] p :%lu, v=%p, len=%d\n",
				 i, (unsigned long)xhci_sram[i].msram_phys_addr,
				 xhci_sram[i].msram_virt_addr,
				 xhci_sram[i].mlength);
	}
	return 0;
						   ...
						   unsigned char **msram_virt_addr)
{
	...
	*msram_virt_addr =
	...
	pr_info("%s get [%d] p :%lu, v=%p, len=%d\n",
			__func__, id, (unsigned long)xhci_sram[id].msram_phys_addr,
