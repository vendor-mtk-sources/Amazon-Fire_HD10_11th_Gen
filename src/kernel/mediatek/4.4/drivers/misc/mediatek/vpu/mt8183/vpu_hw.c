...
void MET_Events_BusyRate_Trace(struct vpu_device *vpu_device)
{
	...
	vpu_dvfs_get_usage(vpu_device, 0, &total_time0, &busy_time0);
	vpu_dvfs_get_usage(vpu_device, 1, &total_time1, &busy_time1);
	vpu_met_event_busyrate(0, (unsigned int)(busy_time0  * 100U / total_time0),
			       1, (unsigned int)(busy_time1  * 100U / total_time1));
}
...
void vpu_thermal_register_cb(void)
{
	thermal_cb = true;
}
