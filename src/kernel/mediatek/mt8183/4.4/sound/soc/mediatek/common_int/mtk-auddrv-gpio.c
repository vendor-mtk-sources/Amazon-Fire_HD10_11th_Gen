...
enum audio_system_gpio_type {
	...
	// added for external amplifier for speaker & headphone
	GPIO_EXT_SPK_AMP_ON,
	GPIO_EXT_SPK_AMP_OFF,
	GPIO_EXT_SPK_AMP_CTRL0_ON,
	GPIO_EXT_SPK_AMP_CTRL0_OFF,
	GPIO_EXT_SPK_AMP_CTRL1_ON,
	GPIO_EXT_SPK_AMP_CTRL1_OFF,
	GPIO_EXT_SPK_AMP_CTRLP_ON,
	GPIO_EXT_SPK_AMP_CTRLP_OFF,
	GPIO_EXT_HP_AMP_ON,
	GPIO_EXT_HP_AMP_OFF,
	GPIO_EXT_HP_AMP_CTRL0_ON,
	GPIO_EXT_HP_AMP_CTRL0_OFF,
	GPIO_EXT_HP_AMP_CTRL1_ON,
	GPIO_EXT_HP_AMP_CTRL1_OFF,
	GPIO_EXT_HP_AMP_CTRLP_ON,
	GPIO_EXT_HP_AMP_CTRLP_OFF,
	GPIO_NUM
};
...
static struct audio_gpio_attr aud_gpios[GPIO_NUM] = {
	...
	// added for external amplifier for speaker & headphone
	[GPIO_EXT_SPK_AMP_ON] = {"ext_spk_amp_on", false, NULL},
	[GPIO_EXT_SPK_AMP_OFF] = {"ext_spk_amp_off", false, NULL},
	[GPIO_EXT_SPK_AMP_CTRL0_ON] = {"ext_spk_amp_ctrl0_on", false, NULL},
	[GPIO_EXT_SPK_AMP_CTRL0_OFF] = {"ext_spk_amp_ctrl0_off", false, NULL},
	[GPIO_EXT_SPK_AMP_CTRL1_ON] = {"ext_spk_amp_ctrl1_on", false, NULL},
	[GPIO_EXT_SPK_AMP_CTRL1_OFF] = {"ext_spk_amp_ctrl1_off", false, NULL},
	[GPIO_EXT_SPK_AMP_CTRLP_ON] = {"ext_spk_amp_ctrlp_on", false, NULL},
	[GPIO_EXT_SPK_AMP_CTRLP_OFF] = {"ext_spk_amp_ctrlp_off", false, NULL},
	[GPIO_EXT_HP_AMP_ON] = {"ext_hp_amp_on", false, NULL},
	[GPIO_EXT_HP_AMP_OFF] = {"ext_hp_amp_off", false, NULL},
	[GPIO_EXT_HP_AMP_CTRL0_ON] = {"ext_hp_amp_ctrl0_on", false, NULL},
	[GPIO_EXT_HP_AMP_CTRL0_OFF] = {"ext_hp_amp_ctrl0_off", false, NULL},
	[GPIO_EXT_HP_AMP_CTRL1_ON] = {"ext_hp_amp_ctrl1_on", false, NULL},
	[GPIO_EXT_HP_AMP_CTRL1_OFF] = {"ext_hp_amp_ctrl1_off", false, NULL},
	[GPIO_EXT_HP_AMP_CTRLP_ON] = {"ext_hp_amp_ctrlp_on", false, NULL},
	[GPIO_EXT_HP_AMP_CTRLP_OFF] = {"ext_hp_amp_ctrlp_off", false, NULL},
};
#endif
...
static int _AudDrv_GPIO_EXT_AMP_Select(int bEnable, int mode, int base)
{
	int retval = 0;
	int i;
	mutex_lock(&gpio_request_mutex);
	if ((bEnable == 1) && aud_gpios[GPIO_EXT_AMP_ON(base)].gpio_prepare)
	{
		// set the EXT AMP switch
		retval = pinctrl_select_state(pinctrlaud, aud_gpios[GPIO_EXT_AMP_ON(base)].gpioctrl);
		if (retval)
			pr_err("could not set aud_gpios[GPIO_EXT_AMP_ON] pins\n");

		// there are 2 kinds of mode setting method, by GPIO direct pulling or by pulse generating
		// we need to set ctrlp in the dts if the target PA chip is controlled by pulses
		// otherwise just set the ctrl0/ctrl1 pins
		mode &= EXT_AMP_CTRL_MASK;
		if (aud_gpios[GPIO_EXT_AMP_CTRLP_ON(base)].gpio_prepare)
		{
			// issue pulse with mode times
			for (i = 0; i < (mode & EXT_AMP_CTRL_MASK); i++)
			{
				retval = pinctrl_select_state(pinctrlaud, aud_gpios[GPIO_EXT_AMP_CTRLP_OFF(base)].gpioctrl);
				if (retval)
					pr_err("could not set aud_gpios[GPIO_EXT_AMP_CTRLP_OFF] pins\n");
				udelay(2);
				retval = pinctrl_select_state(pinctrlaud, aud_gpios[GPIO_EXT_AMP_CTRLP_ON(base)].gpioctrl);
				if (retval)
					pr_err("could not set aud_gpios[GPIO_EXT_AMP_CTRLP_ON] pins\n");
				udelay(2);
			}
		}
		else
		{
			// set ctrl0 pin first if it is configured
			if ((mode & EXT_AMP_CTRL0_BIT) && aud_gpios[GPIO_EXT_AMP_CTRL0_ON(base)].gpio_prepare)
			{
				retval = pinctrl_select_state(pinctrlaud, aud_gpios[GPIO_EXT_AMP_CTRL0_ON(base)].gpioctrl);
				if (retval)
					pr_err("could not set aud_gpios[GPIO_EXT_AMP_CTRL0_ON] pins\n");
			}
			else if ((mode & EXT_AMP_CTRL0_BIT) == 0 && aud_gpios[GPIO_EXT_AMP_CTRL0_OFF(base)].gpio_prepare)
			{
				retval = pinctrl_select_state(pinctrlaud, aud_gpios[GPIO_EXT_AMP_CTRL0_OFF(base)].gpioctrl);
				if (retval)
					pr_err("could not set aud_gpios[GPIO_EXT_AMP_CTRL0_OFF] pins\n");
			}
			// set ctrl1 pin if it is also configured
			if ((mode & EXT_AMP_CTRL1_BIT) && aud_gpios[GPIO_EXT_AMP_CTRL1_ON(base)].gpio_prepare)
			{
				retval = pinctrl_select_state(pinctrlaud, aud_gpios[GPIO_EXT_AMP_CTRL1_ON(base)].gpioctrl);
				if (retval)
					pr_err("could not set aud_gpios[GPIO_EXT_AMP_CTRL1_ON] pins\n");
			}
			else if ((mode & EXT_AMP_CTRL1_BIT) == 0 && aud_gpios[GPIO_EXT_AMP_CTRL1_OFF(base)].gpio_prepare)
			{
				retval = pinctrl_select_state(pinctrlaud, aud_gpios[GPIO_EXT_AMP_CTRL1_OFF(base)].gpioctrl);
				if (retval)
					pr_err("could not set aud_gpios[GPIO_EXT_AMP_CTRL1_OFF] pins\n");
			}
		}
	}
	else if (aud_gpios[GPIO_EXT_AMP_OFF(base)].gpio_prepare)
	{
		// clear the EXT AMP switch
		retval = pinctrl_select_state(pinctrlaud, aud_gpios[GPIO_EXT_AMP_OFF(base)].gpioctrl);
		if (retval)
			pr_err("could not set aud_gpios[GPIO_EXT_AMP_OFF] pins\n");
	}
	mutex_unlock(&gpio_request_mutex);
	return retval;
}
...
int AudDrv_GPIO_EXT_SPK_AMP_Select(int bEnable, int mode)
{
	int retval = 0;
	if (bEnable == 1) {
		retval = _AudDrv_GPIO_EXT_AMP_Select(bEnable, mode, GPIO_EXT_SPK_AMP_ON);
...
{
	return _AudDrv_GPIO_EXT_AMP_Select(bEnable, mode, GPIO_EXT_HP_AMP_ON);
}
