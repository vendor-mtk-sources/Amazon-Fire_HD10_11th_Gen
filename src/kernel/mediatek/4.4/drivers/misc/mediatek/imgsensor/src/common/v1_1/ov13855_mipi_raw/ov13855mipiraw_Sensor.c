...
#define LOG_DEB(fmt, args...)   pr_debug(PFX "[%s] " fmt, __func__, ##args)
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.checksum_value = 0x78779e1c,	/* checksum value for Camera Auto Test */
	.pre = {
		.pclk = 110000000,	/* record different mode's pclk */
		.linelength = 1140,	/* record different mode's linelength */
	...
	.mclk = 6,		/* mclk value, suggest 24 or 26 for 24Mhz or 26Mhz */
...
{4256, 3168, 0, 8, 4256, 3152, 4256, 3152, 16, 8, 4224, 3136, 0, 0, 4224, 3136}, /* video */
{4256, 3168, 64, 32, 4128, 3104, 1032, 776, 4, 4, 1024, 768, 0, 0, 1024, 768},	 /* hs video */
{4256, 3168, 0, 8, 4256, 3152, 2128, 1576, 8, 4, 2112, 1568, 0, 0, 2112, 1568}  /*slim video*/
};
...
static void set_shutter(kal_uint16 shutter)
{
	...
	LOG_DEB("Enter set_shutter! shutter =%d\n", shutter);
	...
	LOG_DEB("Exit! shutter =%d, framelength =%d\n", shutter, imgsensor.frame_length);
}				/*    set_shutter */
...
static void set_shutter_frame_length(kal_uint16 shutter, kal_uint16 frame_length)
{
	...
	imgsensor.shutter = shutter;
	spin_unlock_irqrestore(&imgsensor_drv_lock, flags);
	LOG_DEB("Enter set_shutter_frame_length! shutter =%d\n", shutter);
...
{
	LOG_DEB("le:0x%x, se:0x%x, gain:0x%x\n", le, se, gain);
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
	...
	if (enable) {
		...
		write_cmos_sensor(0x5000, 0xda);	/* disable lenc and otp_dpc */
		write_cmos_sensor(0x4503, 0x80);
	} else {
		/* 0x5E00[8]: 1 enable,  0 disable */
		/* 0x5E00[1:0]; 00 Color bar, 01 Random Data, 10 Square, 11 BLACK */
		write_cmos_sensor(0x5000, 0xfd);	/* enable otp_dpc */
		write_cmos_sensor(0x4503, 0x00);
	}
	spin_lock(&imgsensor_drv_lock);
...
static void capture_init(kal_bool cap_val)
{
	LOG_INF("E\n");
	...
	if (KAL_TRUE == cap_val) {
		...
		LOG_INF("current_fps %d fps, use cap setting: %d fps!\n",
...
static void normal_video_init(void)
{
	LOG_INF("E\n");
...
static void hs_video_init(void)
{
	LOG_INF("E\n");
...
static void slim_video_init(void)
{
	LOG_INF("E\n");
...
static kal_uint32 get_sensor_temperature(void)
{
	...
	/* Writing 0x4d12[0] to '1' to trigger temperature calculating */
	write_cmos_sensor(0x4d12, temp | 0x01);
	/* Latched Temperature Value, Integer Part */
	temperature = read_cmos_sensor(0x4d13);
	mutex_unlock(&get_temperature_mutex);

	/* Positive temperature, use as it is */
	if (temperature < 0xc0)
		temperature_convert = temperature;
	/* Negative temperature, conversion is needed */
	else
		temperature_convert = 0xc0 - temperature;
