...
void mt_usb_swconnect(void)
{
	...
	if (!wake_lock_active(&_mu3d_musb->usb_wakelock))
		wake_lock(&_mu3d_musb->usb_wakelock);

	musb_start(_mu3d_musb);
}
...
void mt_usb_swdisconnect(void)
{
	...
	musb_stop(_mu3d_musb);
	if (wake_lock_active(&_mu3d_musb->usb_wakelock))
		wake_unlock(&_mu3d_musb->usb_wakelock);
}
...
/* build time force on  and to avoid build error due to PMIC module not ready*/
#if defined(CONFIG_FPGA_EARLY_PORTING)
	|| !defined(CONFIG_MTK_SMART_BATTERY)
#define BYPASS_PMIC_LINKAGE  1
#else
#define BYPASS_PMIC_LINKAGE  0
#endif
...
#if BYPASS_PMIC_LINKAGE
	os_printk(K_INFO, "force on");
	chg_type = STANDARD_HOST;
#else
	chg_type = mtk_vbusd_get_linetype();
#endif
	return chg_type;
}
...
#ifdef CONFIG_USB_VBUSDET_IN_GPIO
	vbus_exist = mtk_vbusd_vbus_status();
#else
...
static bool __usb_cable_connected(int ops)
{
	...
	if (mu3d_force_on) {
		...
		/* TYPE CHECK*/
		_mu3d_musb->charger_mode = mu3d_hal_get_charger_type();
		chg_type = _mu3d_musb->charger_mode;
