...
int32_t cmdq_task_secure_enable_dapc(struct cmdqRecStruct *handle,
...
int32_t cmdq_task_secure_enable_port_security(struct cmdqRecStruct *handle,
	const uint64_t engineFlag)
{
#ifdef CMDQ_SECURE_PATH_SUPPORT
	if (handle == NULL)
							      ...
							      struct cmdqRecStruct *handle)
{
	...
	pDesc->secData.srcHandle = handle->secData.srcHandle;
#endif
#endif
...
int32_t cmdqRecSecureEnableDAPC(struct cmdqRecStruct *handle,
	const uint64_t engineFlag)
{
...
int32_t cmdqRecSecureEnablePortSecurity(struct cmdqRecStruct *handle,
	const uint64_t engineFlag)
{
			...
			struct cmdqRecStruct *handle)
{
	...
	if ((!secData->addrMetadataCount) || (handle->secData.addrMetadataCount >= CMDQ_IWC_MAX_ADDR_LIST_LENGTH)) {
		CMDQ_ERR(
