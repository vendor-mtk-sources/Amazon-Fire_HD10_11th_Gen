...
#if CFG_SUPPORT_ANT_DIVERSITY
	cnmTimerInitTimer(prAdapter,
			  &prAdapter->rWifiVar.rAisAntSwitchInfo.rSwapTimer,
			  (PFN_MGMT_TIMEOUT_FUNC)aisFsmAntSwDelayTimeOut, (ULONG) NULL);
#endif
}				/* end of aisFsmInit() */
	...
	do {
		...
		case AIS_STATE_SEARCH:
			...
			if (prAisFsmInfo->ucJoinFailCntAfterScan >= 5) {
				prBssDesc = NULL;
				DBGLOG(AIS, WARN,
					"Failed to connect ssid XXXXXX more than 5 times after last scan, scan again\n");
			} else
...
VOID aisFsmRunEventScanDone(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
...
#endif
#if CFG_SUPPORT_ANT_DIVERSITY
	struct AIS_ANT_SWITCH_INFO_T *prAisAntSwInfo = &prAdapter->rWifiVar.rAisAntSwitchInfo;
#endif
	...
	if (ucSeqNumOfCompMsg != prAisFsmInfo->ucSeqNumOfScanReq) {
		...
		default:
#if CFG_SUPPORT_ANT_DIVERSITY
			DBGLOG(AIS, INFO, "[AntS] eState:%d, detectTargetAnt:%d, scanIndex:%d\n",
					prAisAntSwInfo->eState,
					prAisAntSwInfo->fgIsDetectTargetAnt,
					prAisAntSwInfo->ucAntScanIndex);
			if (prAisAntSwInfo->eState == AIS_ANT_SW_DETECT ||
					prAisAntSwInfo->ucTestCase == ENUM_AS_TEST_SCAN) {
				prConnSettings->fgIsScanReqIssued = FALSE;
				aisFsmAntSwitchAdjustRCPI(prAdapter, prAisAntSwInfo);

				DBGLOG(AIS, INFO, "[AntS]CurRCPI:%u, TarRCPI:%u \n", prAisAntSwInfo->ucCurrentAntRCPI, prAisAntSwInfo->ucTargetAntRCPI);
				if (prAisAntSwInfo->ucTestCase == ENUM_AS_TEST_SCAN) {
					aisFsmAntSwOidCallbcak(prAdapter);
				} else {
					aisFsmAntSwitchSteps(prAdapter, AIS_ANT_SW_DETECT_DONE);
				}
				break;
			}
#endif
...
#if CFG_SUPPORT_ANT_DIVERSITY
	if (prBcnRmParam->eState == RM_WAITING && prAisAntSwInfo->eState == AIS_ANT_SW_IDLE) {
#else
...
#if CFG_SUPPORT_ANT_DIVERSITY
		if (prAisAntSwInfo->eState != AIS_ANT_SW_IDLE)
			return;
	...
	} else if (prBcnRmParam->eState == RM_ON_GOING) /* Radio Measurement is on-going, schedule to next Measurement Element */
...
#else
			if ((prAdapter->rWifiVar.rRoamingInfo.fgDrvRoamingAllow))
#endif
...
VOID aisFsmRunEventSecModeChangeTimeout(IN P_ADAPTER_T prAdapter, ULONG ulParamPtr)
{
	...
	prBssInfo->u2DeauthReason = REASON_CODE_BEACON_SECURITY_CHANGE;
	...
	IN UINT_32 u4IeLength,  IN UINT_8 ucSetChannel, IN struct _PARAM_SCAN_RANDOM_MAC_ADDR_T *prScanRandMacAddr)
{
	...
	if (!prConnSettings->fgIsScanReqIssued) {
		...
		if (prAisFsmInfo->eCurrentState == AIS_STATE_NORMAL_TR) {
			...
			prAdapter->rWifiVar.rAisAntSwitchInfo.eState !=  AIS_ANT_SW_IDLE) {
#else
	} else if (prAdapter->rWifiVar.rRmReqParams.rBcnRmParam.eState == RM_ON_GOING) {
#endif
...
static VOID aisRemoveDisappearedBlacklist(P_ADAPTER_T prAdapter)
{
	...
	LINK_FOR_EACH_ENTRY_SAFE(prEntry, prNextEntry, prBlackList, rLinkEntry, struct AIS_BLACKLIST_ITEM) {
		...
		if (!fgDisappeared || (u8Current - prEntry->u8DisapperTime) < 600 * USEC_PER_SEC)
			continue;
		DBGLOG(AIS, INFO, "Remove disappeared blacklist %pM\n",
			prEntry->aucBSSID);
		LINK_MGMT_RETURN_ENTRY(&prConnSettings->rBlackList, prEntry);
	}
}
...
VOID aisFsmRunEventBssTransition(IN P_ADAPTER_T prAdapter, IN P_MSG_HDR_T prMsgHdr)
{
	...
	prRoamingFsmInfo->oldApRssi = cConnectedRssi;
...
static VOID aisPostScan(P_ADAPTER_T prAdapter)
{
	...
	LINK_FOR_EACH_ENTRY(prBssDesc, prLink, rLinkEntry, BSS_DESC_T) {
		...
		if (prBssDesc->fgRptCfg80211 && prBssDesc->rUpdateTime + 10000 < rScanStartTime &&
			...
			fgNeedYellow = TRUE;
			DBGLOG(AIS, INFO, "%pM scan update mismatch, return line %d, Up@ %u, Add@ %u\n",
				prBssDesc->aucBSSID, prBssDesc->u2ReturnLine,
...
#if CFG_SUPPORT_ANT_DIVERSITY
extern INT_32 wmt_plat_wifi_ant_switch(UINT_8 ant_num);

VOID aisFsmAntSwitchSteps(P_ADAPTER_T prAdapter,
		enum AIS_ANT_SWITCH_STATE_T eNextState)
{
	struct AIS_ANT_SWITCH_INFO_T *prASInfo = NULL;
	struct AIS_ANT_SWITCH_STATISTIC_T *prASData = NULL;
	...
	do {
		DBGLOG(AIS, INFO, "[AntS] TRANSITION: [%d] -> [%d]"
				"IsDetectTargetAnt: %d SwitchAnt:%d\n",
				prASInfo->eState, eNextState,
				prASInfo->fgIsDetectTargetAnt,
				prASInfo->ucSwitchAntNum);
		prASInfo->eState = eNextState;
		switch (eNextState) {
		case AIS_ANT_SW_IDLE:
			fgIsTransition = FALSE;
			prASInfo->fgIsDetectTargetAnt = FALSE;
			if (prASInfo->ucTestCase >= ENUM_AS_TEST_SW_SUCCESS &&
					prASInfo->ucTestCase <=
					ENUM_AS_TEST_RANDOM_RCPI) {
				aisFsmAntSwOidCallbcak(prAdapter);
			}
			break;
		case AIS_ANT_SW_DETECT:
			aisFsmAntSwitchScan(prAdapter);
			fgIsTransition = FALSE;
			break;
		case AIS_ANT_SW_DETECT_DONE:
			eNextState = AIS_ANT_SW_IDLE;
			fgIsTransition = TRUE;

			if (!prASInfo->fgIsDetectTargetAnt) {
				if (prASInfo->ucCurrentAntRCPI == 0) {
					prASData->u4ASScanCurrentNoRsp++;
					break;
				}
				if (prASInfo->ucCurrentAntRCPI >=
						AIS_ANT_SW_RCPI_THRESHOLD) {
					prASData->u4ASOrientationStrong++;
					break;
				}
				prASInfo->ucSwitchAntNum =
					prASInfo->ucTargetAntenna;
				prASInfo->fgIsDetectTargetAnt = TRUE;
				eNextState = AIS_ANT_SW_SWITCH;
				break;
			}

			prASInfo->fgIsDetectTargetAnt = FALSE;
			if (prASInfo->ucTargetAntRCPI == 0) {
				prASData->u4ASScanTargetNoRsp++;
			}
			if (prASInfo->ucCurrentAntRCPI +
					prASInfo->ucAntRCPIDelta >=
					prASInfo->ucTargetAntRCPI) {
				prASInfo->ucSwitchAntNum =
						prASInfo->ucCurrentAntenna;
				prASData->u4ASOrientationSwitchBack++;
				eNextState = AIS_ANT_SW_SWITCH;
				break;
			}
			prASData->u4ASOrientationSuccess++;
			prASInfo->ucCurrentAntenna = prASInfo->ucTargetAntenna;
			break;
		case AIS_ANT_SW_SWITCH:
			aisFsmAntSwitchQuerySwitch(prAdapter,
				prASInfo->ucSwitchAntNum);
			fgIsTransition = FALSE;
			break;
		case AIS_ANT_SW_SWITCH_DONE:
			eNextState = AIS_ANT_SW_IDLE;
			fgIsTransition = TRUE;
			DBGLOG(AIS, INFO, "[AntS] ant switch to [%d:%d]\n",
				prASInfo->ucSwitchAntNum,
				prASInfo->rSwitchStatus);
			if (prASInfo->rSwitchStatus != WLAN_STATUS_SUCCESS) {
				DBGLOG(AIS, INFO, "[AntS] switch fail\n");
				break;
			}
			if (prAdapter->prGlueInfo->fgWas1stSwitch) {
				prAdapter->prGlueInfo->fgWas1stSwitch = FALSE;
			}
			if (prASInfo->fgIsDetectTargetAnt) {
				eNextState = AIS_ANT_SW_DETECT;
			}
			if (prASInfo->ucTestCase ==
				ENUM_AS_TEST_QUERY_ANT_CAP ||
				prAdapter->prGlueInfo->fgIsEnableAntSwQuery) {
				break;
			}
			if (wmt_plat_wifi_ant_switch(prASInfo->ucSwitchAntNum)) {
				DBGLOG(AIS, INFO, "[AntS] ant switch fail!\n");
			}
			break;
		default:
			break;
...
VOID aisFsmAntSwitchQuerySwitch(P_ADAPTER_T prAdapter,
		UINT_8 ucAntNum)
{
	struct CMD_GET_ANT_SW_CAP rAntSwCap;
	...
	if (prAntSwInfo->ucTestCase != ENUM_AS_TEST_QUERY_ANT_CAP &&
			!prAdapter->prGlueInfo->fgIsEnableAntSwQuery) {
		prAntSwInfo->rSwitchStatus = WLAN_STATUS_SUCCESS;
		aisFsmAntSwitchSteps(prAdapter, AIS_ANT_SW_SWITCH_DONE);
		return;
	}
	...
	rStatus = wlanSendSetQueryCmd(prAdapter,
				      ...
				      TRUE,	/* fgNeedResp */
				      TRUE,	/* fgIsOid */
				      aisFsmAntSwitchQuerySwDone,
				      aisFsmAntSwQrySwTimeOut,
				      sizeof(struct CMD_GET_ANT_SW_CAP),
				      (PUINT_8) &rAntSwCap,
...
VOID
aisFsmAntSwitchQuerySwDone(IN P_ADAPTER_T prAdapter,
		IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf)
{
	struct CMD_GET_ANT_SW_CAP *prAntSwCap = NULL;
	struct AIS_ANT_SWITCH_STATISTIC_T *prAntSwData = NULL;
	struct AIS_ANT_SWITCH_INFO_T *prAntSwInfo = NULL;
	...
	DBGLOG(AIS, INFO, "[AntS]switch result:%d", prAntSwCap->rStatus);
	prAntSwInfo->rSwitchStatus = prAntSwCap->rStatus;
	switch(prAntSwInfo->rSwitchStatus) {
	case WLAN_STATUS_INVALID_DATA:
		prAntSwData->u4ASSwitchWrongAnt++;
		break;
	case WLAN_STATUS_RESOURCES:
		prAntSwData->u4ASSwitchBtBusy++;
		break;
	default:
		prAntSwData->u4ASSwitchOther++;
		break;
	}
	if (prAntSwInfo->ucTestCase == ENUM_AS_TEST_QUERY_ANT_CAP) {
		aisFsmAntSwOidCallbcak(prAdapter);
...
VOID
aisFsmAntSwQrySwTimeOut(IN P_ADAPTER_T prAdapter,
		IN P_CMD_INFO_T prCmdInfo)
{
	...
	prAntSwData->u8AntSwQueryDoneTime = kalGetTimeTick();
	prAntSwData->u4ASSwitchTimeout++;
	prAntSwInfo->rSwitchStatus = WLAN_STATUS_PENDING;
	if (prAntSwInfo->ucTestCase == ENUM_AS_TEST_QUERY_ANT_CAP) {
		aisFsmAntSwOidCallbcak(prAdapter);
	} else {
		aisFsmAntSwitchSteps(prAdapter, AIS_ANT_SW_SWITCH_DONE);
...
VOID aisFsmAntSwitchScan(P_ADAPTER_T prAdapter)
{
	struct _MSG_SCN_SCAN_REQ_V3_T *prScanReqMsg;
	UINT_32 u4ScnReqMsgLen = 0;
	struct AIS_ANT_SWITCH_INFO_T *prAntSwInfo = NULL;
	...
	prScanReqMsg->rMsgHdr.eMsgId = MID_AIS_SCN_SCAN_REQ_V3;
	prScanReqMsg->ucSeqNum = ++prAdapter->rWifiVar.rAisFsmInfo.ucSeqNumOfScanReq;
	prScanReqMsg->ucBssIndex = prAdapter->prAisBssInfo->ucBssIndex;
	...
	prScanReqMsg->ucSSIDNum = 1;
	prScanReqMsg->prSsid = &prAdapter->rWlanInfo.rCurrBssId.rSsid;

	prScanReqMsg->u2ProbeDelay = AIS_ANT_SW_SCN_PROBE_DELAY;
	prScanReqMsg->u2ChannelDwellTime = AIS_ANT_SW_SCN_DWELL_TIME;
	...
	for (int i = 0; i < AIS_ANT_SW_SCN_CHANNEL_COUNT; i++) {
		...
		prScanReqMsg->arChnlInfoList[i].ucChannelNum = prAdapter->prAisBssInfo->ucPrimaryChannel;
	}

	mboxSendMsg(prAdapter, MBOX_ID_0, (P_MSG_HDR_T) prScanReqMsg, MSG_SEND_METHOD_BUF);
}

UINT_8 aisFsmAntSwitchCalcRCPI(P_ADAPTER_T prAdapter, UINT_8 ucAntScanIndex,
				UINT_8 aucAntScanRCPI[])
{
#define MAX_RCPI_DIFF           16
#define RCPI_PERCENTAGE         90

	UINT_32 u4RCPISum = 0;
	UINT_8 ucAverageRCPI = 0;
	UINT_8 aucDiffCnt[MAX_RCPI_DIFF] = {0};
	UINT_8 ucRCPIDiff = 0;
	UINT_8 ucDiffLimit = 0;
	UINT_8 ucIdx = 0;
	UINT_32 ucFinalRCPI = 0;

	if (ucAntScanIndex == 0) {
		return ucFinalRCPI;
	}

	for (ucIdx = 0; ucIdx < ucAntScanIndex; ucIdx++) {
		u4RCPISum += aucAntScanRCPI[ucIdx];
	}

	ucAverageRCPI = u4RCPISum / ucAntScanIndex;
	for (ucIdx = 0; ucIdx < ucAntScanIndex; ucIdx++) {
		UINT_8 ucRCPI = aucAntScanRCPI[ucIdx];
		ucRCPIDiff = ucRCPI > ucAverageRCPI ? ucRCPI - ucAverageRCPI : ucAverageRCPI - ucRCPI;
		if (ucRCPIDiff < MAX_RCPI_DIFF) {
			aucDiffCnt[ucRCPIDiff]++;
		}
	}

	u4RCPISum = 0;
	for (ucIdx = 0; ucIdx < MAX_RCPI_DIFF; ucIdx++) {
		u4RCPISum += aucDiffCnt[ucIdx];
		if (u4RCPISum * 100 > ucAntScanIndex * RCPI_PERCENTAGE) {
			break;
		}
	}
	...
	for (ucIdx = 0; ucIdx < ucAntScanIndex; ucIdx++) {
		UINT_8 ucRCPI = aucAntScanRCPI[ucIdx];
		ucRCPIDiff = ucRCPI > ucAverageRCPI ? ucRCPI - ucAverageRCPI : ucAverageRCPI - ucRCPI;
		if (ucRCPIDiff > ucDiffLimit) {
			continue;
		}
		ucFinalRCPI = ucFinalRCPI == 0 ? ucRCPI * 1000 : (ucFinalRCPI * 7 + ucRCPI * 1000) >> 3;
	}
	return (UINT_8)(ucFinalRCPI / 1000);

}

VOID aisFsmAntSwitchAdjustRCPI(P_ADAPTER_T prAdapter,
		struct AIS_ANT_SWITCH_INFO_T *prAisAntSwInfo) {
	if (prAisAntSwInfo->fgIsDetectTargetAnt) {
		prAisAntSwInfo->ucTargetAntRCPI = aisFsmAntSwitchCalcRCPI(prAdapter,
				prAisAntSwInfo->ucAntScanIndex, prAisAntSwInfo->aucAntScanRCPI);
	} else {
		prAisAntSwInfo->ucCurrentAntRCPI = aisFsmAntSwitchCalcRCPI(prAdapter,
				prAisAntSwInfo->ucAntScanIndex, prAisAntSwInfo->aucAntScanRCPI);
	}
	switch (prAisAntSwInfo->ucTestCase) {
		case ENUM_AS_TEST_SW_SUCCESS:
			prAisAntSwInfo->ucTargetAntRCPI = 100;
			prAisAntSwInfo->ucCurrentAntRCPI = 80;
			break;
		case ENUM_AS_TEST_SW_FAIL:
			prAisAntSwInfo->ucTargetAntRCPI = 90;
			prAisAntSwInfo->ucCurrentAntRCPI = 80;
			break;
		case ENUM_AS_TEST_RANDOM_RCPI:
			prAisAntSwInfo->ucTargetAntRCPI += (kalRandomNumber() % 50 - 25);
			prAisAntSwInfo->ucCurrentAntRCPI += (kalRandomNumber() % 50 - 25);
			break;
		case ENUM_AS_TEST_CUR_ANT_NO_RSP:
			prAisAntSwInfo->ucCurrentAntRCPI = 0;
			break;
		case ENUM_AS_TEST_TARGET_ANT_NO_RSP:
			prAisAntSwInfo->ucCurrentAntRCPI = 80;
			prAisAntSwInfo->ucTargetAntRCPI = 0;
			break;
		default:
			break;
	}
...
{
	struct AIS_ANT_SWITCH_INFO_T *prASInfo;
	struct PARAM_ANT_SWITCH_TYPE *prASType;
	UINT_8 ucBufIdx;
	UINT_8 ucIdx;
	INT_8 iRet;

	prASInfo = &prAdapter->rWifiVar.rAisAntSwitchInfo;
	prASType = (struct PARAM_ANT_SWITCH_TYPE *)
			prAdapter->prGlueInfo->OidEntry.pvInfoBuf;

	if (prASType == NULL) {
		prASInfo->ucTestCase = ENUM_AS_ERROR_NULL_PARAM;
	}

	switch (prASInfo->ucTestCase) {
	case ENUM_AS_TEST_SCAN:
		ucBufIdx = kalSnprintf(prASType->aucRet, prASType->u4RetSize,
				"RCPI [%u] [", prASInfo->ucCurrentAntRCPI);
		for (ucIdx = 0; ucIdx < prASInfo->ucAntScanIndex; ucIdx++) {
			iRet = kalSnprintf(prASType->aucRet + ucBufIdx,
					prASType->u4RetSize - ucBufIdx, " %u",
					prASInfo->aucAntScanRCPI[ucIdx]);
			if (iRet < 0)
				break;
			ucBufIdx += iRet;
		}
		kalSnprintf(prASType->aucRet + ucBufIdx,
				prASType->u4RetSize - ucBufIdx, "%s", " ]");
		break;
	case ENUM_AS_TEST_SW_SUCCESS:
	case ENUM_AS_TEST_SW_FAIL:
	case ENUM_AS_TEST_CUR_ANT_NO_RSP:
	...
	case ENUM_AS_TEST_RANDOM_RCPI:
		if (prASInfo->ucCurrentAntRCPI == 0) {
			kalSnprintf(prASType->aucRet, prASType->u4RetSize,
					"%s", "[No Response]");
			break;
		}
		if (prASInfo->ucCurrentAntRCPI > AIS_ANT_SW_RCPI_THRESHOLD) {
			kalSnprintf(prASType->aucRet, prASType->u4RetSize,
					"%s", "[Strong Signal]");
			break;
		}
		if (prASInfo->ucTargetAntRCPI > prASInfo->ucCurrentAntRCPI +
				prASInfo->ucAntRCPIDelta) {
			kalSnprintf(prASType->aucRet, prASType->u4RetSize,
					"%s", "[Switch Success]");
			break;
	...
	case ENUM_AS_TEST_QUERY_ANT_CAP:
		switch (prASInfo->rSwitchStatus) {
		case WLAN_STATUS_RESOURCES:
			kalSnprintf(prASType->aucRet, prASType->u4RetSize,
					"%s", "BT was busy during switch");
			break;
		case WLAN_STATUS_PENDING:
			kalSnprintf(prASType->aucRet, prASType->u4RetSize,
					"%s", "Query cmd to FW Timeout");
			break;
		case WLAN_STATUS_SUCCESS:
			kalSnprintf(prASType->aucRet, prASType->u4RetSize,
					"antenna switched to %d",
					prASInfo->ucCurrentAntenna);
			break;
		case WLAN_STATUS_INVALID_DATA:
			kalSnprintf(prASType->aucRet, prASType->u4RetSize,
		...
		default:
			kalSnprintf(prASType->aucRet, prASType->u4RetSize,
					"unexpect case [%d] in FW query",
					prASInfo->rSwitchStatus);
			break;
		}
		break;
	case ENUM_AS_ERROR_NULL_PARAM:
		DBGLOG(AIS, ERROR, "[AntS] didn't get para from oid entry\n");
		break;
	default:
		kalSnprintf(prASType->aucRet, prASType->u4RetSize,
				"unexpect case [%u] in oid callback",
				prASInfo->ucTestCase);
		break;
	}

	prASInfo->ucTestCase = 0;
	kalOidComplete(prAdapter->prGlueInfo, FALSE,
			sizeof(struct PARAM_ANT_SWITCH_TYPE),
			WLAN_STATUS_SUCCESS);
}

VOID aisFsmAntSwDelayTimeOut(IN P_ADAPTER_T prAdapter,
		ULONG ulParamPtr)
{
	...
	aisFsmAntSwitchSteps(prAdapter, AIS_ANT_SW_DETECT);
}
