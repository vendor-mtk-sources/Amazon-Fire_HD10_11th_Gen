...
int ovl_switch_to_nonsec(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	if (ovl_is_sec[ovl_idx] == 1) {
		...
		/*disable_ovl_layers(module, nonsec_switch_handle);*/
		/* in fact, dapc/port_sec will be disabled by cmdq */
		cmdqRecSecureDisablePortSecurity(nonsec_switch_handle, (1LL << cmdq_engine));
		cmdqRecSecureDisableDAPC(nonsec_switch_handle, (1LL << cmdq_engine));
#if 0
		if (handle != NULL) {
			/* Async Flush method */
			 ...
			 struct disp_ddp_path_config *pConfig, void *handle)
{
	...
	if (has_sec_layer == 1)
		...
		ret = ovl_switch_to_nonsec(module, handle); /* hadle = NULL, use the sync flush method */
