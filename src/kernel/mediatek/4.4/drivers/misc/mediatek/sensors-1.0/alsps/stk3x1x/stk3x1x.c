...
uint32_t stk_alscode2lux(struct stk3x1x_priv *obj, uint32_t alscode);
...
/*----------------------------------------------------------------------------*/
static ssize_t stk3x1x_store_config(struct device_driver *ddri,
...
/*----------------------------------------------------------------------------*/
static ssize_t stk3x1x_store_send(struct device_driver *ddri,
		const char *buf, size_t count)
{
	int addr, cmd;
	u8 dat;
...
/*----------------------------------------------------------------------------*/
static ssize_t stk3x1x_store_recv(struct device_driver *ddri,
					const char *buf, size_t count)
{
	int addr;
	u8 dat;
...
/*----------------------------------------------------------------------------*/
static ssize_t stk3x1x_set_ps_offset(struct device_driver *ddri,
				const char *buf, size_t count)
{
	int ret;
...
/*----------------------------------------------------------------------------*/
static ssize_t stk3x1x_show_pthreshold_calibration(struct device_driver *ddri,
							char *buf)
{
	return 0;
}

static ssize_t stk3x1x_store_ps_offset(struct device_driver *ddri,
			const char *buf, size_t count)
{
	return count;
}
...
/*----------------------------------------------------------------------------*/
static ssize_t stk3x1x_show_als_enable(struct device_driver *ddri, char *buf)
{
	int32_t enable;
	u8 r_buf;
	int ret;
...
static ssize_t stk3x1x_store_als_enable(struct device_driver *ddri,
...
/*----------------------------------------------------------------------------*/
static ssize_t stk3x1x_show_als_test(struct device_driver *ddri, char *buf)
{
	int32_t als_reading;
...
static DRIVER_ATTR(config, S_IWUSR | S_IRUGO,
		stk3x1x_show_config, stk3x1x_store_config);
static DRIVER_ATTR(trace, S_IWUSR | S_IRUGO,
		stk3x1x_show_trace, stk3x1x_store_trace);
static DRIVER_ATTR(status, S_IWUSR | S_IRUGO,
		stk3x1x_show_status, NULL);
static DRIVER_ATTR(send, S_IWUSR | S_IRUGO,
		stk3x1x_show_send, stk3x1x_store_send);
static DRIVER_ATTR(recv, S_IWUSR | S_IRUGO,
		stk3x1x_show_recv, stk3x1x_store_recv);
static DRIVER_ATTR(reg, S_IWUSR | S_IRUGO,
		stk3x1x_show_reg, NULL);
static DRIVER_ATTR(allreg, S_IWUSR | S_IRUGO,
		stk3x1x_show_allreg, NULL);
static DRIVER_ATTR(pscalibration, S_IWUSR | S_IRUGO,
		stk3x1x_show_ps_offset, stk3x1x_set_ps_offset);
static DRIVER_ATTR(pthredcalibration, S_IWUSR | S_IRUGO,
		stk3x1x_show_pthreshold_calibration, NULL);
static DRIVER_ATTR(ps_offset, S_IWUSR | S_IRUGO, NULL,
		stk3x1x_store_ps_offset);
/*----------------------------------------------------------------------------*/
static DRIVER_ATTR(enable, S_IWUSR | S_IRUGO,
		stk3x1x_show_als_enable, stk3x1x_store_als_enable);
static DRIVER_ATTR(alstest, S_IWUSR | S_IRUGO, stk3x1x_show_als_test, NULL);
static DRIVER_ATTR(alscali, S_IWUSR | S_IRUGO, stk3x1x_show_als_cali, NULL);
/*----------------------------------------------------------------------------*/
