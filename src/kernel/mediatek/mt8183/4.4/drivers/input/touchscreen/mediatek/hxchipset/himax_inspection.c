...
static int himax_self_test_data_init(void)
{
	...
	int i = 0;
#if HX_MULT_BIN
	char temp_file_name[MAX_MODULE_NAME_LEN];
	...
	for (i = 0; i < module_num; i++) {
		...
		if (temp_id == info->id) {
			snprintf(temp_file_name, sizeof(temp_file_name),
				info->csv_name);
			I("%s: csv file name = %s\n", __func__, temp_file_name);
			break;
		}
	}
	if (i == module_num) {
		I("%s: There is no matching module id. use default scv\n",
			__func__);
		snprintf(temp_file_name, sizeof(temp_file_name),
			file_name_2);
	}
...
#if HX_MULT_BIN
		err = request_firmware(&file_entry, temp_file_name, ts->dev);
		if (err < 0) {
			E("%s,Fail to get %s\n", __func__, temp_file_name);
