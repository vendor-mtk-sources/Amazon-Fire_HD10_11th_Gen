...
static void gtp_work_func(struct goodix_ts_data *ts)
{
	...
	if (g_debugfs_enable_latency_check) {
		...
		snprintf(buffer, sizeof(buffer),
				"Report event:latency=%ldus;@%ldus;num=%d;TYPE_A=%d",
				(unsigned long)(ktime_to_ns(g_ts_report) - ktime_to_ns(g_ts_irq_in))/1000L,
				(unsigned long)ktime_to_ns(g_ts_report)/1000L,
				point_state & 0x0f,
...
static irqreturn_t gtp_irq_handler(int irq, void *dev_id)
{
	...
	if (g_debugfs_enable_latency_check) {
		...
		snprintf(buffer, sizeof(buffer),
				"Irq:@%ldus", (unsigned long)ktime_to_ns(g_ts_irq_in)/1000L);
		log_to_metrics(ANDROID_LOG_INFO, "Touch", buffer);
	}
#endif
	gtp_work_func(ts);
...
static int gtp_power_init(struct goodix_ts_data *ts)
{
	...
	if (IS_ERR(ts->vdd_ana)) {
		...
		ts->vdd_ana = NULL;
		TP_LOGI("Optional regulator get failed vdd ret=%d\n", ret);
	}
	...
	if (IS_ERR(ts->vcc_i2c)) {
		...
		ts->vcc_i2c = NULL;
		TP_LOGI("Optional regulator get failed vcc_i2c ret=%d for dual power\n",
			ret);
	}
	return 0;
}
