	...
	*pu4SetInfoLen = 0;
	...
	for (i = 0; i < prScanRequest->u4SsidNum; i++) {
		if (prScanRequest->rSsid[i].u4SsidLen > ELEM_MAX_LEN_SSID) {
			DBGLOG(OID, ERROR,
			       "[%s] SSID(XXXXXX) Length(%u) is over than ELEM_MAX_LEN_SSID(%d)\n",
			       __func__, prScanRequest->rSsid[i].u4SsidLen, ELEM_MAX_LEN_SSID);
...
wlanoidSetConnect(IN P_ADAPTER_T prAdapter, IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	DBGLOG(OID, INFO, "ssid XXXXXX, bssid " MACSTR ", conn policy %d, disc reason %d\n",
			...
			IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	ASSERT(prAdapter);
	ASSERT(pvSetBuffer);
    if (kalStrnCmp(pucSetBuf, "roam_param ", 11) == 0)
		pucSetBuf += 11;
	else
		return WLAN_STATUS_INVALID_DATA;

	i4Status = sscanf(pucSetBuf, "%d,%d,%d,%d,%d", &prRoamInfo->rRoamCtrl.fgEnable,
...
/*----------------------------------------------------------------------------*/
/*!
* \brief This routine is called to enter the state machine of antenna switch flow
*
* \param[in] prAdapter Pointer to the Adapter structure.
* \param[in] pvSetBuffer A pointer to the buffer that holds the data to be set.
* \param[in] u4SetBufferLen The length of the set buffer.
...
/*----------------------------------------------------------------------------*/
WLAN_STATUS
wlanoidSwitchAntenna(IN P_ADAPTER_T prAdapter,
		     IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen,
		     OUT PUINT_32 pu4SetInfoLen)
{
	WLAN_STATUS rRetStatus = WLAN_STATUS_SUCCESS;
	struct AIS_ANT_SWITCH_INFO_T *prAisAntSwInfo = NULL;
	struct AIS_ANT_SWITCH_STATISTIC_T *prAisAntSwData = NULL;
	UINT_32 u4PreferAntNum = 0;
	UINT_32 u4AntNum = 0;
	...
	if (u4PreferAntNum == u4AntNum &&
		!prAdapter->prGlueInfo->fgWas1stSwitch) {
		DBGLOG(OID, INFO, "[AntS] antenta already was %u\n", u4AntNum);
		prAisAntSwData->u4ASOrientationRemain++;
		return WLAN_STATUS_INVALID_DATA;
	}
	if (prAdapter->rWifiVar.rAisFsmInfo.eCurrentState !=
			AIS_STATE_NORMAL_TR) {
		prAisAntSwData->u4ASOrientationDisconnect++;
		return WLAN_STATUS_MEDIA_DISCONNECT;
	}
	if (prAdapter->rWifiVar.rConnSettings.fgIsScanReqIssued) {
		prAisAntSwData->u4ASOrientationScan++;
		return WLAN_STATUS_RESOURCES;
	}

	DBGLOG(OID, INFO, "[AntS]Switch Ant[%u->%u]!\n", u4AntNum,
			 u4PreferAntNum);
	prAisAntSwInfo->ucTestCase = 0;
	prAisAntSwInfo->ucCurrentAntRCPI = 0;
	prAisAntSwInfo->ucTargetAntRCPI = 0;
	prAisAntSwInfo->ucTargetAntenna = (UINT_8) u4PreferAntNum;
	prAisAntSwData->u4PreviousSwitchCnt = prAisAntSwData->u4AntSwitchCount;
	prAisAntSwData->u4PreviousSwitchSucCnt =
			 prAisAntSwData->u4AntSwitchSuccessCount;

	if (prAdapter->prGlueInfo->fgIsEnableDelaySwap) {
		cnmTimerStopTimer(prAdapter, &prAisAntSwInfo->rSwapTimer);
		cnmTimerStartTimer(prAdapter, &prAisAntSwInfo->rSwapTimer,
				prAdapter->prGlueInfo->u4AsDelayMSec);
	} else {
...
/*----------------------------------------------------------------------------*/
/*!
* \brief This routine is called to enter antenna test case dispatch flow
*
* \param[in] prAdapter Pointer to the Adapter structure.
* \param[in] pvSetBuffer A pointer to the buffer that holds the data to be set.
* \param[in] u4SetBufferLen The length of the set buffer.
...
/*----------------------------------------------------------------------------*/
WLAN_STATUS
wlanoidAntSwitchTest(IN P_ADAPTER_T prAdapter,
		     IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen,
		     OUT PUINT_32 pu4SetInfoLen)
{
	struct PARAM_ANT_SWITCH_TYPE *prCmdBuf = NULL;
	struct AIS_ANT_SWITCH_INFO_T *prAisAntSwInfo = NULL;
	struct AIS_ANT_SWITCH_STATISTIC_T *prAisAntSwData = NULL;
	ENUM_AIS_STATE_T eCurrentState = AIS_STATE_IDLE;
	WLAN_STATUS rStatus = WLAN_STATUS_SUCCESS;
	UINT_32 u4AntNum = 0;
	UINT_8 ucLen = 0;
	...
	DBGLOG(OID, INFO, "[AntS] prCmdBuf[%u, %u]!\n", prCmdBuf->u4Cmd,
			 prCmdBuf->u4Data);

	prAisAntSwInfo->ucTestCase = prCmdBuf->u4Cmd;
	switch (prCmdBuf->u4Cmd) {
	case ENUM_QUERY_CURRENT_ANT:
		kalSnprintf(prCmdBuf->aucRet, prCmdBuf->u4RetSize,
				"current antenna was %u", u4AntNum);
		break;
	case ENUM_SET_ANT_NUM: /* test ant switch */
		if (u4AntNum == prCmdBuf->u4Data) {
			kalSnprintf(prCmdBuf->aucRet, prCmdBuf->u4RetSize,
				"antenna already was %u", u4AntNum);
			break;
		}
		DBGLOG(OID, INFO, "[AntS] test switch ant from %u to %u!\n",
				u4AntNum, prCmdBuf->u4Data);
		rStatus = wmt_plat_wifi_ant_switch((UINT_8)prCmdBuf->u4Data);
		if (rStatus) {
			kalSnprintf(prCmdBuf->aucRet, prCmdBuf->u4RetSize,
				"wmt_plat_wifi_ant_switch err:%u", rStatus);
			break;
		}
		prAisAntSwInfo->ucCurrentAntenna = (UINT_8)prCmdBuf->u4Data;
		kalSnprintf(prCmdBuf->aucRet, prCmdBuf->u4RetSize,
				"antenna switched to %u", prCmdBuf->u4Data);
		break;
	case ENUM_AS_TEST_SCAN:
		if (eCurrentState != AIS_STATE_NORMAL_TR) {
			kalSnprintf(prCmdBuf->aucRet, prCmdBuf->u4RetSize,
				"%s", "media disconnect");
			break;
		}
	...
	case ENUM_AS_TEST_RANDOM_RCPI:
		...
		prAisAntSwInfo->ucTargetAntenna = (UINT_8) (u4AntNum + 1) % 2;
		prAisAntSwInfo->ucCurrentAntRCPI = 0;
		prAisAntSwInfo->ucTargetAntRCPI = 0;
	...
	case ENUM_AS_DUMP_OTHER_STATISTICS:
		kalSnprintf(prCmdBuf->aucRet, prCmdBuf->u4RetSize,
				...
				prAdapter->prGlueInfo->rNewOrientationStatus,
				prAdapter->prGlueInfo->idme_board_id,
				prAisAntSwInfo->eState,
				prAisAntSwInfo->ucCurrentAntenna,
				prAisAntSwInfo->ucTargetAntenna,
				prAisAntSwInfo->ucCurrentAntRCPI,
				prAisAntSwInfo->ucTargetAntRCPI,
				prAisAntSwInfo->ucAntScanIndex,
				prAisAntSwInfo->fgIsDetectTargetAnt,
				prAisAntSwInfo->ucTestCase,
				...
				prAisAntSwData->u4PreviousSwitchSucCnt,
				prAisAntSwData->u4AntSwitchSuccessCount,
				prAisAntSwData->u8AntSwQueryTime,
				prAisAntSwData->u8AntSwQueryDoneTime,
				prAisAntSwData->u4NoProbeRspCount);
		break;
	case ENUM_AS_SET_DELTA_RSSI:
		prAisAntSwInfo->ucAntRCPIDelta = prCmdBuf->u4Data * 2;
		kalSnprintf(prCmdBuf->aucRet, prCmdBuf->u4RetSize,
				"RSSI Delta was set to %u",
				prCmdBuf->u4Data);
		break;
	case ENUM_AS_TEST_QUERY_ANT_CAP:
		if (u4AntNum == prCmdBuf->u4Data) {
			kalSnprintf(prCmdBuf->aucRet, prCmdBuf->u4RetSize,
				"[FW] antenna already was %u", u4AntNum);
			break;
		}
		DBGLOG(OID, INFO, "[AntS] test switch ant [FW] to %u!\n",
				prCmdBuf->u4Data);
		aisFsmAntSwitchQuerySwitch(prAdapter,
				(UINT_8) prCmdBuf->u4Data);
		rStatus = WLAN_STATUS_PENDING;
		break;
	case ENUM_AS_DUMP_MATRIX:
		ucLen = 0;
		if (eCurrentState != AIS_STATE_NORMAL_TR) {
			ucLen = kalSnprintf(prCmdBuf->aucRet,
				prCmdBuf->u4RetSize, "\n%s",
				"Status: Disconnect");
		...
		kalSnprintf(prCmdBuf->aucRet + ucLen,
			prCmdBuf->u4RetSize - ucLen, "\n"
			"AS_ORIENTATION_TOTAL = %u\n"
			"AS_ORIENTATION_REMAIN = %u\n"
			"AS_ORIENTATION_DISCONNECT = %u\n"
			"AS_ORIENTATION_SCAN = %u\n"
			"AS_ORIENTATION_STRONG = %u\n"
			"AS_ORIENTATION_SUCCESS = %u\n"
			"AS_ORIENTATION_SWITCHBACK = %u\n"
			"AS_SCAN_TOTAL = %u\n"
			"AS_SCAN_CURRENT_NORSP = %u\n"
			"AS_SCAN_TARGET_NORSP = %u\n"
			"AS_SWITCH_TOTAL = %u\n"
			"AS_SWITCH_BTBUSY = %u\n"
			"AS_SWITCH_TIMEOUT = %u\n"
			"AS_SWITCH_WRONGANT = %u\n"
			"AS_SWITCH_OTHER = %u",
			prAisAntSwData->u4ASOrientationTotal,
			prAisAntSwData->u4ASOrientationRemain,
			prAisAntSwData->u4ASOrientationDisconnect,
			prAisAntSwData->u4ASOrientationScan,
			prAisAntSwData->u4ASOrientationStrong,
			prAisAntSwData->u4ASOrientationSuccess,
			prAisAntSwData->u4ASOrientationSwitchBack,
			prAisAntSwData->u4ASScanTotal,
			prAisAntSwData->u4ASScanCurrentNoRsp,
			prAisAntSwData->u4ASScanTargetNoRsp,
			prAisAntSwData->u4ASSwitchTotal,
			prAisAntSwData->u4ASSwitchBtBusy,
			prAisAntSwData->u4ASSwitchTimeout,
			prAisAntSwData->u4ASSwitchWrongAnt,
			prAisAntSwData->u4ASSwitchOther);
		break;
	case ENUM_AS_CLEAR_METRICS:
		prAisAntSwData->u4ASOrientationTotal = 0;
		prAisAntSwData->u4ASOrientationRemain = 0;
		prAisAntSwData->u4ASOrientationDisconnect = 0;
		prAisAntSwData->u4ASOrientationScan = 0;
		prAisAntSwData->u4ASOrientationStrong = 0;
		prAisAntSwData->u4ASOrientationSuccess = 0;
		prAisAntSwData->u4ASOrientationSwitchBack = 0;
		prAisAntSwData->u4ASScanTotal = 0;
		prAisAntSwData->u4ASScanCurrentNoRsp = 0;
		prAisAntSwData->u4ASScanTargetNoRsp = 0;
		prAisAntSwData->u4ASSwitchTotal = 0;
		prAisAntSwData->u4ASSwitchBtBusy = 0;
		prAisAntSwData->u4ASSwitchTimeout = 0;
		prAisAntSwData->u4ASSwitchWrongAnt = 0;
		prAisAntSwData->u4ASSwitchOther = 0;
		kalSnprintf(prCmdBuf->aucRet, prCmdBuf->u4RetSize, "%s",
				"Metrics was clear out");
		break;
	default:
		kalSnprintf(prCmdBuf->aucRet, prCmdBuf->u4RetSize,
...
#if CFG_SHOW_NVRAM
	prRegInfo->au2CountryCode[0] = ((UINT_16) pucCountryCode[0]);
	prRegInfo->au2CountryCode[1] = ((UINT_16) pucCountryCode[1]);
#endif
...
#if CFG_SHOW_NVRAM
		prRegInfo->prOldEfuseMapping->uc11AcTxPwrValid = pCountryPwrTable->uc11AcTxPwrValid;
		kalMemCopy(&prRegInfo->prOldEfuseMapping->r11AcTxPwr, &pCountryPwrTable->r11AcTxPwr, sizeof(AC_PWR_SETTING_STRUCT));
#endif
...
#if CFG_SHOW_NVRAM
		if (0 != pCountryPwrTable->r5GBandEdgePwr.uc5GBandEdgePwrUsed && 0 != pCountryPwrTable->ucSupport5GBand) {
			prRegInfo->prOldEfuseMapping->r5GBandEdgePwr.uc5GBandEdgePwrUsed = pCountryPwrTable->r5GBandEdgePwr.uc5GBandEdgePwrUsed;
		}
		prRegInfo->prOldEfuseMapping->r5GBandEdgePwr.c5GBandEdgeMaxPwrOFDM20 = rCmdEdgeTxPwrLimit.cBandEdgeMaxPwrOFDM20;
		prRegInfo->prOldEfuseMapping->r5GBandEdgePwr.c5GBandEdgeMaxPwrOFDM40 = rCmdEdgeTxPwrLimit.cBandEdgeMaxPwrOFDM40;
		prRegInfo->prOldEfuseMapping->r5GBandEdgePwr.c5GBandEdgeMaxPwrOFDM80 = rCmdEdgeTxPwrLimit.cBandEdgeMaxPwrOFDM80;
#endif
