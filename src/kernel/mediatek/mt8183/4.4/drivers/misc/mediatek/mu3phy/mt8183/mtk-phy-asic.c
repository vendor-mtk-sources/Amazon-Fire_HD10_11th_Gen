...
int mt_usb_phychk_extconn(void)
{
	...
	val  = U3PhyReadReg32((u3phy_addr_t)U3D_USBPHYACR6);
	val &= ~(RG_USB20_BC11_SW_EN);
	U3PhyWriteReg32((u3phy_addr_t)U3D_USBPHYACR6, val);
	...
	val  = U3PhyReadReg32((u3phy_addr_t)U3D_U2PHYDTM0);
	val |=  (FORCE_DP_PULLDOWN | FORCE_DM_PULLDOWN);
	val &= ~(RG_DPPULLDOWN | RG_DMPULLDOWN);
	U3PhyWriteReg32((u3phy_addr_t)U3D_U2PHYDTM0, val);

	val  = U3PhyReadReg32((u3phy_addr_t)U3D_U2PHYACR4);
	val |=  RG_USB20_DM_100K_EN;
	U3PhyWriteReg32((u3phy_addr_t)U3D_USBPHYACR4, val);
	...
	/* Read linestate
	  ...
	  */
	line_state  = U3PhyReadReg32((u3phy_addr_t)U3D_U2PHYDMON1);
	...
	if ((line_state & 0x02) == 0)
		...
		U3PhyWriteReg32((u3phy_addr_t)U3D_U2PHYACR4, val);
	...
	os_printk(K_INFO, "\n Final extended connector type: %s since line state: 0x%x\n",
				 string_usb_extconn_type[usb_extconn], line_state);
