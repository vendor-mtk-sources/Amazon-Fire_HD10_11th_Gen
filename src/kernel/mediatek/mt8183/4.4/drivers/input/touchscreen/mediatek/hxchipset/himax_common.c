...
struct upgrade_module himax_module_list[] = {
	{VID_HX_TG_BOE, CID_TG_BOE, "TG_BOE", "AMAZON_TG_BOE_HX.bin", "hx_criteria_TG_BOE.csv"},
	{VID_HX_KD_INX, CID_KD_INX, "KD_INX", "AMAZON_KD_INX_HX.bin", "hx_criteria_KD_INX.csv"},
};
int module_num = sizeof(himax_module_list)/sizeof(struct upgrade_module);
...
int himax_common_proc_init(void)
{
...
#if defined(HX_HIGH_SENSE)
	himax_proc_HSEN_file = proc_create(HIMAX_PROC_HSEN_FILE, 0664,
...
#endif
#if defined(HX_SMART_WAKEUP)
	himax_proc_SMWP_file = proc_create(HIMAX_PROC_SMWP_FILE, 0664,
	...
	himax_proc_GESTURE_file = proc_create(HIMAX_PROC_GESTURE_FILE, 0664,
...
#if defined(HX_ULTRA_LOW_POWER)
	himax_proc_psensor_file = proc_create(HIMAX_PROC_PSENSOR_FILE, 0664,
...
static int himax_auto_update_check(void)
{
	...
	int i = 0;
	struct upgrade_module *info = &himax_module_list[0];
#endif
	...
	if (g_core_fp.fp_fw_ver_bin() == 0) {
...
#if HX_MULT_BIN
			temp_id = hx83102_incell_compare_id();
			I("%s: Panel_ID=%x,bin_cid=%x\n", __func__,
				temp_id, g_i_CID_MAJ);
			for (i = 0; i < module_num; i++) {
				info = &himax_module_list[i];
				if ((temp_id == info->id) && (g_i_CID_MAJ != info->cid)) {
					E("Panel=%s,pannel cid=%x,FW bin ID not match need not update!\n",
						info->module_name, info->cid);
					return 1;
				}
			}
#endif
		...
		I("%s:bin_fw_ver=%d,bin_CFG_ver=%d\n",
			__func__, g_i_FW_VER, g_i_CFG_VER);
		I("%s:ic_fw_ver=%d,ic_config_ver=%d\n",
			__func__, ic_data->vendor_fw_ver, ic_data->vendor_config_ver);
	} else {
		E("%s: FW bin fail!\n", __func__);
		ret = 1;
	}
...
static int i_get_FW(void)
{
	...
	for (i = 0; i < module_num; i++) {
		info = &himax_module_list[i];
		if (temp_id == info->id) {
			I("%s: file name = %s\n", __func__, info->fw_name);
			ret = request_firmware(&hxfw, info->fw_name,
			private_ts->dev);
			break;
		}
	}
	if (i == module_num) {
		E("%s: There is no matching module id.\n", __func__);
		return MODULE_FAIL;
	}
#else
		...
		int ts_path, int ts_status)
{
	...
	&& p_y <= ts->pdata->abs_y_max) {
		...
		ts->p_point_num = 1;
		ts->pre_pen_data[0] = p_x;
		ts->pre_pen_data[1] = p_y;
...
static void himax_pen_report(struct himax_ts_data *ts)
{
	bool valid = false;


	if (g_ts_dbg != 0) {
		I("%s:start ic_data->HX_PEN_FUNC=%d\n",
...
static void himax_finger_leave(struct himax_ts_data *ts)
{
	...
	if (g_ts_dbg != 0)
		I("%s: pen_finger_start!\n", __func__);
		/* Pen Leave only*/
	if ((ic_data->HX_PEN_FUNC) && (!ts->p_point_num)
		&& (ts->pre_pen_data[0]) && (ts->pre_pen_data[1]))
		goto pen_leave_only;
#if defined(HX_PALM_REPORT)
