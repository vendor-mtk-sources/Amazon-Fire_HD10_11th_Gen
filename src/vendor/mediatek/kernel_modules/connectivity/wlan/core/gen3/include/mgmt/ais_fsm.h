...
#define AIS_ANT_SW_SCN_PROBE_DELAY                      10
#define AIS_ANT_SW_SCN_DWELL_TIME                       50
#define AIS_ANT_SW_SCN_CHANNEL_COUNT                    6
#define AIS_ANT_SW_SCN_RCPI_NUM                         100
#define AIS_ANT_SW_RCPI_THRESHOLD                       90
#define AIS_ANT_SW_RCPI_DELTA                           12

/* the device rotation and user cover device antenna with
 * hand may didn't happy at the same time, so we delay
 * the antenna signal measure to make the better decision
 * of antenna switch.
 */
#define AIS_ANT_SW_DELAY_MSEC                           1500
#endif
/*******************************************************************************
*                             D A T A   T Y P E S
********************************************************************************
...
#if CFG_SUPPORT_ANT_DIVERSITY
enum AIS_ANT_SWITCH_STATE_T {
	AIS_ANT_SW_IDLE,
	AIS_ANT_SW_DETECT,
	AIS_ANT_SW_DETECT_DONE,
	AIS_ANT_SW_SWITCH,
	AIS_ANT_SW_SWITCH_DONE,
};

enum ENUM_ANT_SWITCH_TEST_T {
	ENUM_QUERY_BORAD_ID = 0,
	ENUM_QUERY_CURRENT_ANT,            // 1
	ENUM_SET_ANT_NUM,                  // 2
	ENUM_ANT_SW_ENABLE,                // 3
	ENUM_AS_TEST_SCAN,                 // 4
	ENUM_AS_TEST_SW_SUCCESS,           // 5
	ENUM_AS_TEST_SW_FAIL,              // 6
	ENUM_AS_TEST_CUR_ANT_NO_RSP,       // 7
	ENUM_AS_TEST_TARGET_ANT_NO_RSP,    // 8
	ENUM_AS_TEST_RANDOM_RCPI,          // 9
	ENUM_AS_TEST_RCPI_MOVING_ALG,      // 10
	ENUM_AS_TEST_RCPI_MOVING_ALG_N,    // 11
	ENUM_AS_DUMP_DELAY_SCN_INFO,       // 12
	ENUM_AS_DUMP_RCPI_INFO,            // 13
	ENUM_AS_DUMP_OTHER_STATISTICS,     // 14
	ENUM_AS_DUMP_NEW_ORIENTATION,      // 15
	ENUM_AS_SET_DELTA_RSSI,            // 16
	ENUM_AS_TEST_QUERY_ANT_CAP,        // 17
	ENUM_AS_TEST_ENABLE_AS_QUERY,      // 18
	ENUM_AS_ERROR_NULL_PARAM,          // 19
	ENUM_AS_SET_PANEL_IDME,            // 20
	ENUM_AS_SET_GOOD_PANEL,            // 21
	ENUM_AS_SET_PANEL_TYPE,            // 22
	ENUM_AS_SET_HW_SUPPORT,            // 23
	ENUM_AS_SET_IDME_SUPPORT,          // 24
	ENUM_AS_DUMP_MATRIX,               // 25
	ENUM_AS_CLEAR_METRICS,             // 26
};

struct AIS_ANT_SWITCH_INFO_T {
	enum AIS_ANT_SWITCH_STATE_T eState;
	UINT_8 ucCurrentAntRCPI;
	UINT_8 ucTargetAntRCPI;
	UINT_8 ucCurrentAntenna;
	UINT_8 ucTargetAntenna;
	UINT_8 ucAntScanIndex;
	UINT_8 aucAntScanRCPI[100];
	BOOLEAN fgIsDetectTargetAnt;
	UINT_8 ucTestCase;
	UINT_8 ucAntRCPIDelta;
	WLAN_STATUS rSwitchStatus;
...
struct AIS_ANT_SWITCH_STATISTIC_T {
	UINT_32 u4AntSwitchCount;
	UINT_32 u4AntSwitchSuccessCount;
	UINT_32 u4NoProbeRspCount;
	UINT_32 u4PreviousSwitchCnt;
	UINT_32 u4PreviousSwitchSucCnt;
	UINT_64 u8AntSwQueryTime;
	UINT_64 u8AntSwQueryDoneTime;

	UINT_32 u4ASOrientationTotal;
	UINT_32 u4ASOrientationRemain;
	UINT_32 u4ASOrientationDisconnect;
	UINT_32 u4ASOrientationScan;
	UINT_32 u4ASOrientationStrong;
	UINT_32 u4ASOrientationSuccess;
	UINT_32 u4ASOrientationSwitchBack;

	UINT_32 u4ASScanTotal;
	UINT_32 u4ASScanCurrentNoRsp;
	UINT_32 u4ASScanTargetNoRsp;

	UINT_32 u4ASSwitchTotal;
	UINT_32 u4ASSwitchBtBusy;
	UINT_32 u4ASSwitchTimeout;
	UINT_32 u4ASSwitchWrongAnt;
	UINT_32 u4ASSwitchOther;
};

struct CMD_GET_ANT_SW_CAP{
	UINT_8 ucAntNum;
	WLAN_STATUS rStatus;
};
#endif
...
#if CFG_SUPPORT_ANT_DIVERSITY
VOID aisFsmAntSwitchSteps(P_ADAPTER_T prAdapter, enum AIS_ANT_SWITCH_STATE_T eNextState);
VOID aisFsmAntSwitchScan(P_ADAPTER_T prAdapter);
UINT_8 aisFsmAntSwitchCalcRCPI(P_ADAPTER_T prAdapter, UINT_8 ucAntScanIndex, UINT_8 aucAntScanRCPI[]);
VOID aisFsmAntSwitchAdjustRCPI(P_ADAPTER_T prAdapter, struct AIS_ANT_SWITCH_INFO_T *prAisAntSwInfo);
VOID aisFsmAntSwitchQuerySwitch(P_ADAPTER_T prAdapter, UINT_8 ucAntNum);
VOID aisFsmAntSwitchQuerySwDone(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo, IN PUINT_8 pucEventBuf);
VOID aisFsmAntSwQrySwTimeOut(IN P_ADAPTER_T prAdapter, IN P_CMD_INFO_T prCmdInfo);
