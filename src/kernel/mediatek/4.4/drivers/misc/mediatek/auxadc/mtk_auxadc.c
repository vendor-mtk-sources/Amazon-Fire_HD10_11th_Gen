...
static const struct iio_chan_spec mtk_auxadc_iio_channels[] = {
	MTK_AUXADC_CHANNEL(0),
	MTK_AUXADC_CHANNEL(1),
	MTK_AUXADC_CHANNEL(2),
	MTK_AUXADC_CHANNEL(3),
	MTK_AUXADC_CHANNEL(4),
	MTK_AUXADC_CHANNEL(5),
	MTK_AUXADC_CHANNEL(6),
	MTK_AUXADC_CHANNEL(7),
	MTK_AUXADC_CHANNEL(8),
	MTK_AUXADC_CHANNEL(9),
	MTK_AUXADC_CHANNEL(10),
	MTK_AUXADC_CHANNEL(11),
	MTK_AUXADC_CHANNEL(12),
	MTK_AUXADC_CHANNEL(13),
	MTK_AUXADC_CHANNEL(14),
	MTK_AUXADC_CHANNEL(15),
};
				  ...
				  long info)
{
	...
	case IIO_CHAN_INFO_RAW:
		IMM_auxadc_GetOneChannelValue(chan->channel, data, val);
		if (*val < 0) {
...
static const struct iio_info mtk_auxadc_info = {
	.driver_module = THIS_MODULE,
	.read_raw = &mtk_auxadc_read_raw,
};
...
static int mt_auxadc_probe(struct platform_device *dev)
{
	...
	indio_dev->info = &mtk_auxadc_info;
	indio_dev->modes = INDIO_DIRECT_MODE;
	indio_dev->channels = mtk_auxadc_iio_channels;
	indio_dev->num_channels = ARRAY_SIZE(mtk_auxadc_iio_channels);

	ret = iio_device_register(indio_dev);
	if (ret < 0)
		dev_err(&dev->dev, "failed to register iio device\n");
