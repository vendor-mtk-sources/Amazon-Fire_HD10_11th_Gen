...
********************************************************************************
*/
static VOID roamingWaitCandidateTimeout(IN P_ADAPTER_T prAdapter, ULONG ulParamPtr)
{
...
VOID roamingFsmInit(IN P_ADAPTER_T prAdapter)
{
	...
	cnmTimerInitTimer(prAdapter, &prRoamingFsmInfo->rWaitCandidateTimer,
			  (PFN_MGMT_TIMEOUT_FUNC)roamingWaitCandidateTimeout, (ULONG) NULL);
#if CFG_SUPPORT_DEBUG_FS
...
VOID roamingFsmSteps(IN P_ADAPTER_T prAdapter, IN ENUM_ROAMING_STATE_T eNextState)
{
	...
	do {
		...
		case ROAMING_STATE_DISCOVERY:
			{
				...
				if ((!prRoamingFsmInfo->rRoamingDiscoveryUpdateTime || CHECK_FOR_TIMEOUT
		...
		case ROAMING_STATE_REQ_CAND_LIST:
		{
			...
			struct BSS_TRANSITION_MGT_PARAM_T *prBtm = &prAdapter->rWifiVar.rAisSpecificBssInfo.rBTMParam;
			...
			*/
			if (prBssInfo && prBssInfo->prStaRecOfAP) {
				if (prBssInfo->prStaRecOfAP->fgSupportBTM) {
					prBtm->ucDialogToken = wnmGetBtmToken();
					prBtm->ucQueryReason = BSS_TRANSITION_LOW_RSSI;
					wnmSendBTMQueryFrame(prAdapter, prBssInfo->prStaRecOfAP);
					cnmTimerStartTimer(prAdapter, &prRoamingFsmInfo->rWaitCandidateTimer, 500);
					break;
				} else if (prBssDesc &&
					(prBssDesc->aucRrmCap[0] & BIT(RRM_CAP_INFO_NEIGHBOR_REPORT_BIT))) {
...
VOID roamingFsmRunEventFail(IN P_ADAPTER_T prAdapter, IN UINT_32 u4Param)
{
	...
	DBGLOG(ROAMING, STATE, "EVENT-ROAMING FAIL: reason %x Current Time = %u\n", u4Param, kalGetTimeTick());
