...
#define SPI_TICK_DELAY_OFFSET		29
#define SPI_TICK_DELAY_MASK		7ul
...
int spi_set_tick_delay(struct spi_master *master, int tick_delay)
{
	u32 reg_val;
	struct mtk_spi *mdata;

	if (tick_delay & ~SPI_TICK_DELAY_MASK)
		return -EINVAL;

	mdata = spi_master_get_devdata(master);

	reg_val = readl(mdata->base + SPI_CFG1_REG);
	reg_val &= ~(SPI_TICK_DELAY_MASK << SPI_TICK_DELAY_OFFSET);
	reg_val |= (tick_delay << SPI_TICK_DELAY_OFFSET);
	writel(reg_val, mdata->base + SPI_CFG1_REG);

	return 0;
}
EXPORT_SYMBOL(spi_set_tick_delay);

static void mtk_spi_prepare_transfer(struct spi_master *master,
				     struct spi_transfer *xfer)
{
	u32 spi_clk_hz, div, sck_time, cs_time, reg_val = 0;
	u32 cs_setup_time, cs_hold_time, cs_idle_time;
	struct mtk_spi *mdata = spi_master_get_devdata(master);
	...
	sck_time = (div + 1) / 2;
	cs_setup_time = cs_hold_time = cs_idle_time = sck_time * 2;
	...
	if (mdata->dev_comp->adjust_reg) {
		...
		if (mdata->spi_cs_hold_ns > 0) {
			cs_time = (mdata->spi_cs_hold_ns / (1000000000UL / spi_clk_hz)) + 1;
			if (cs_time > cs_hold_time)
				cs_hold_time = cs_time;
		}
		reg_val |= (((cs_hold_time - 1) & 0xffff)
			   << SPI_ADJUST_CFG0_CS_HOLD_OFFSET);
		if (mdata->spi_cs_setup_ns > 0) {
			cs_time = (mdata->spi_cs_setup_ns / (1000000000UL / spi_clk_hz)) + 1;
			if (cs_time > cs_setup_time)
				cs_setup_time = cs_time;
		}
		...
		reg_val |= (((cs_hold_time - 1) & 0xff) << SPI_CFG0_CS_HOLD_OFFSET);
		if (mdata->spi_cs_setup_ns > 0) {
			cs_time = (mdata->spi_cs_setup_ns / (1000000000UL / spi_clk_hz)) + 1;
			if (cs_time > cs_setup_time)
				cs_setup_time = cs_time;
		}
		reg_val |= (((cs_setup_time - 1) & 0xff) << SPI_CFG0_CS_SETUP_OFFSET);
		writel(reg_val, mdata->base + SPI_CFG0_REG);
	}

	reg_val = readl(mdata->base + SPI_CFG1_REG);
	reg_val &= ~SPI_CFG1_CS_IDLE_MASK;
	if (mdata->spi_cs_idle_ns > 0) {
		cs_time = (mdata->spi_cs_idle_ns / (1000000000UL / spi_clk_hz)) + 1;
		if (cs_time > cs_idle_time)
			cs_idle_time = cs_time;
	}
	reg_val |= (((cs_idle_time - 1) & 0xff) << SPI_CFG1_CS_IDLE_OFFSET);
	writel(reg_val, mdata->base + SPI_CFG1_REG);
}
...
static int mtk_spi_probe(struct platform_device *pdev)
{
	...
	master->rt = of_property_read_bool(pdev->dev.of_node, "task-rt");
	...
	if (!of_property_read_u32(pdev->dev.of_node, "spi-tick-delay",
				  &tick_delay)){
		ret = spi_set_tick_delay(master, tick_delay);
		if (ret) {
			dev_err(&pdev->dev, "error: invalid tick_delay: %d\n",
				tick_delay);
			goto err_disable_runtime_pm;
		}
	}

	if (of_property_read_u32(pdev->dev.of_node, "spi-cs-setup-ns",
				  &mdata->spi_cs_setup_ns))
		mdata->spi_cs_setup_ns = 0;

	if (of_property_read_u32(pdev->dev.of_node, "spi-cs-hold-ns",
				  &mdata->spi_cs_hold_ns))
		mdata->spi_cs_hold_ns = 0;

	if (of_property_read_u32(pdev->dev.of_node, "spi-cs-idle-ns",
				  &mdata->spi_cs_idle_ns))
		mdata->spi_cs_idle_ns = 0;
