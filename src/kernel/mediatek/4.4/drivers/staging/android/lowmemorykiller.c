...
unsigned long long lmk_stats[NUM_LMK_BUCKETS];

static void add_oom_kill(int selected_oom_score_adj)
{
	int array_size, i, idx;

	array_size = ARRAY_SIZE(lowmem_adj);
	...
	for (i = array_size - 1; i >= 0; i--) {
		if (selected_oom_score_adj >= lowmem_adj[i]) {
			/* if not exactly equal, assume next higher bucket */
			if (selected_oom_score_adj > lowmem_adj[i])
				i++;
			break;
		}
	...
	if (i >= NUM_LMK_BUCKETS)
		idx = NUM_LMK_BUCKETS - 1;
	else
		idx = i + 1;
...
/* fosmod_fireos_crash_reporting begin */
	...
	if (lmk_log_buffer && selected && selected_oom_score_adj == 0) {
		...
		lowmem_print(1, "Free memory other_free: %d, other_file:%d pages\n", other_free, other_file);
		lowmem_print(1, "Kernel memory slab_unreclaimable:%lu\n", global_page_state(NR_SLAB_UNRECLAIMABLE));
...
/* fosmod_fireos_crash_reporting begin */
		add_oom_kill(selected_oom_score_adj);
...
static int lmk_stats_show(struct seq_file *m, void *v)
{
	seq_printf(m, "Persistent: %llu\n", lmk_stats[PERSISTENT_IDX]);
	seq_printf(m, "Fg: %llu\n", lmk_stats[FG_IDX]);
	seq_printf(m, "Visible: %llu\n", lmk_stats[VISIBLE_IDX]);
	seq_printf(m, "Perceptible: %llu\n", lmk_stats[PERCEPTIBLE_IDX]);
	seq_printf(m, "Backup: %llu\n", lmk_stats[BACKUP_IDX]);
	seq_printf(m, "Other: %llu\n", lmk_stats[OTHER_IDX]);
	return 0;
}

static int lmk_stats_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, lmk_stats_show, NULL);
}

static const struct file_operations lmk_stats_fops = {
	.open       = lmk_stats_proc_open,
	.read       = seq_read,
	.llseek     = seq_lseek,
	.release    = single_release
};
...
static int __init lowmem_init(void)
{
	...
	proc_create("lmk_stats", 0, NULL, &lmk_stats_fops);
