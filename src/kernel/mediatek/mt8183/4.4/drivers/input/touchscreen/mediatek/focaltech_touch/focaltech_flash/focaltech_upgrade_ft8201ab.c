 ...
 *
 * Copyright (c) 2021, Focaltech Ltd. All rights reserved.
 *
...
static int fts_ft8201ab_upgrade_mode(enum FW_FLASH_MODE mode, u8 *buf, u32 len)
{
	...
	if ((buf == NULL) || (len < FTS_MIN_LEN)) {
		FTS_ERROR("buffer/len(0x%x) is invalid", len);
		return -EINVAL;
	}
...
static int fts_ft8201ab_upgrade(u8 *buf, u32 len)
{
	...
	if ((len < FTS_MIN_LEN) || (len > FTS_MAX_LEN_FILE)) {
		FTS_ERROR("fw buffer len(0x%x) fail", len);
		return -EINVAL;
	}
...
static int fts_ft8201ab_lic_upgrade(u8 *buf, u32 len)
{
	...
	if ((len < FTS_MIN_LEN) || (len > FTS_MAX_LEN_FILE)) {
		FTS_ERROR("lcd initial code buffer len(0x%x) fail", len);
		return -EINVAL;
	}
