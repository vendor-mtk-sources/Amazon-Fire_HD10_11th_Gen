...
static int ti_lmu_check_id(struct ti_lmu *lmu, enum ti_lmu_id id)
{
	int ret = 0;
	unsigned int val;
	...
	if (id == LM36274) {
		ret = regmap_read(lmu->regmap, LM36274_REG_REV, &val);
...
static int ti_lmu_probe(struct i2c_client *cl, const struct i2c_device_id *id)
{
	...
	ret = ti_lmu_check_id(lmu, id->driver_data);
	if (ret)
	{
