...
#define AMAZON_CAM_CAL_OFFSET 256
...
/* Others */
	...
	{OV13855_SENSOR_ID, 0xA8, CMD_AUTO, cam_cal_check_amazon_cid},
	/*  ADD before this line */
...
/* For amazon cam_cal, we use different eeprom mapping table */
unsigned int cam_cal_check_amazon_cid(struct i2c_client *client, cam_cal_cmd_func readCamCalData)
{
	unsigned int calibrationID = 0, ret = 0;
	int j = 0;

	if (readCamCalData != NULL) {
		readCamCalData(client, AMAZON_CAM_CAL_OFFSET + 1, (unsigned char *)&calibrationID, 4);
		CAM_CALDB("calibrationID = %x\n", calibrationID);
	}

	if (calibrationID != 0)
		for (j = 0; j < ARRAY_SIZE(amazonCidList); j++) {
			CAM_CALDB("amazonCidList[%d] == %x\n", j, mtkCidList[j]);
			if (amazonCidList[j] == calibrationID) {
				ret = 1;
				break;
			}
		}
