...
static void fts_swap_xy(int *x, int *y);
static void fts_rotate_90(int *x, int *y, int max_y);
static void fts_rotate_180(int *x, int *y, int max_x, int max_y);
static void fts_rotate_270(int *x, int *y, int max_x);
...
static int fts_get_ic_information(struct fts_ts_data *ts_data)
{
    ...
    do {
        ...
        {
            if((chip_id[0] == 0x58) && (chip_id[1] == 0x22))
            {
                fts_read_reg(0xA0, &value);
                FTS_DEBUG("packid = %x",value);
                if((value == 0x24)||(value == 0xff))
                {
                    chip_id[0] = 0x58;
                    chip_id[1] = 0x2E;
                }
...
static int fts_read_parse_touchdata(struct fts_ts_data *data)
{
    ...
    u32 rotate_degree = data->pdata->rotate_degree;
    u8 *buf = data->point_buf;
    ...
    for (i = 0; i < max_touch_num; i++) {
        ...
        if (rotate_degree == 90) {
            fts_rotate_90(&events[i].x, &events[i].y, data->pdata->y_max);
        } else if (rotate_degree == 180) {
            fts_rotate_180(&events[i].x, &events[i].y,
                data->pdata->x_max, data->pdata->y_max);
        } else if (rotate_degree == 270) {
            fts_rotate_270(&events[i].x, &events[i].y, data->pdata->x_max);
        } else if (rotate_degree != 0) {
            pr_warn("rotate_degree=%u is not supported", rotate_degree);
        }
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    ret = of_property_read_u32(np, "focaltech,rotate_degree", &pdata->rotate_degree);
    if (ret < 0)
        FTS_ERROR("Rotate degree not defined!");
...
static int fts_ts_suspend(struct device *dev)
{
    ...
    if (ts_data->gesture_mode) {
        ...
        if (!fts_is_boot_mode_supported() || fts_is_kpoc_boot_mode()) {
            fts_irq_disable();
