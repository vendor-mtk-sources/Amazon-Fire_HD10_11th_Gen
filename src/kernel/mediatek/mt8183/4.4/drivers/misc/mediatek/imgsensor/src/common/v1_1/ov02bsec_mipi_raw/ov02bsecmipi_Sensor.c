...
#include "ov02bsecmipi_Sensor.h"

/************************** Modify Following Strings for Debug **************************/
#define PFX "ov02bsec_camera_sensor"
#define LOG_1 LOG_DBG("OV02B,MIPI 1LANE\n")
/****************************   Modify end    *******************************************/
...
static kal_uint16 curr_framelength = 1221;

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV02BSEC_SENSOR_ID,
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	...
	{1600, 1200, 160, 240, 1280, 720, 1280, 720, 0000, 0000, 1280, 720, 0, 0, 1280, 720}, /* HS video */
...
static kal_uint16 ov02bsec_table_write_cmos_sensor(kal_uint16 *para, kal_uint32 len)
{
...
static void set_dummy(void)
{
	...
	write_cmos_sensor(0x14, (imgsensor.dummy_line >> 8) & 0x7f);
...
static void set_max_framerate(UINT16 framerate, kal_bool min_framelength_en)
{
	...
	imgsensor.dummy_line = imgsensor.frame_length - curr_framelength;
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	if (gain < BASEGAIN)
		reg_gain = BASEGAIN;
	else if (gain > OV02BSEC_SENSOR_DGAIN_MAX)
		reg_gain = OV02BSEC_SENSOR_DGAIN_MAX;
...
{
	kal_uint16 reg_gain, ana_gain;

	reg_gain = gain2reg(gain);

	ana_gain = (((reg_gain*3) >> 2)/3);
	LOG_DBG("Total gain = %d, Analog Gain = 0x%x", reg_gain, ana_gain & 0xff);
	write_cmos_sensor(0xfd, 0x01);
	write_cmos_sensor(0x22, ana_gain & 0xff);
	LOG_DBG("Digital Gain = 0x40");
...
{
	LOG_DBG("Warining:Do not supportIHDR, Return. le:0x%x, se:0x%x, gain:0x%x\n", le, se, gain);
}
...
kal_uint16 addr_data_pair_init_ov02bsec[] = {
	0xfc, 0x01,
	0xfd, 0x00,
	0xfd, 0x00,
	0x24, 0x02,
...
kal_uint16 addr_data_pair_hs_video_ov02bsec[] = {
	0xfd, 0x01,
	0x02, 0x00,
	0x03, 0xc0,
	0x04, 0x01,
...
{
	LOG_INF("E\n");
	ov02bsec_table_write_cmos_sensor(
	    addr_data_pair_init_ov02bsec,
		sizeof(addr_data_pair_init_ov02bsec) /
		sizeof(kal_uint16));
}
...
static void hs_video_setting(void)
{
	LOG_DBG("E\n");
	ov02bsec_table_write_cmos_sensor(
		addr_data_pair_hs_video_ov02bsec,
		sizeof(addr_data_pair_hs_video_ov02bsec) /
		sizeof(kal_uint16));
}
		...
		do {
			*sensor_id = return_sensor_id() + 1;
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				if (ret == 1) {
					LOG_INF("FFC_ID_PIN = %d, it is the main source\n", ret);
...
static kal_uint32 set_max_framerate_by_scenario(MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
		...
		imgsensor.dummy_line = (frame_length > imgsensor_info.pre.framelength) ?
			(frame_length - imgsensor_info.pre.framelength) : 1;
		imgsensor.frame_length = imgsensor_info.pre.framelength + imgsensor.dummy_line;
		imgsensor.min_frame_length = imgsensor.frame_length;
		curr_framelength = imgsensor_info.pre.framelength;
		spin_unlock(&imgsensor_drv_lock);
		set_dummy();
		break;
	case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
		...
		imgsensor.dummy_line = (frame_length > imgsensor_info.normal_video.framelength) ?
			(frame_length - imgsensor_info.normal_video.framelength) : 1;
		imgsensor.frame_length = imgsensor_info.normal_video.framelength + imgsensor.dummy_line;
		imgsensor.min_frame_length = imgsensor.frame_length;
		curr_framelength = imgsensor_info.normal_video.framelength;
		spin_unlock(&imgsensor_drv_lock);
		set_dummy();
		break;
	case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
			...
			imgsensor.dummy_line = (frame_length > imgsensor_info.cap1.framelength) ?
				(frame_length - imgsensor_info.cap1.framelength) : 1;
			imgsensor.frame_length = imgsensor_info.cap1.framelength + imgsensor.dummy_line;
			curr_framelength = imgsensor_info.cap1.framelength;
			imgsensor.min_frame_length = imgsensor.frame_length;
			spin_unlock(&imgsensor_drv_lock);
		} else {
			if (imgsensor.current_fps != imgsensor_info.cap.max_framerate)
			...
			imgsensor.dummy_line = (frame_length > imgsensor_info.cap.framelength) ?
				(frame_length - imgsensor_info.cap.framelength) : 1;
			imgsensor.frame_length = imgsensor_info.cap.framelength + imgsensor.dummy_line;
			curr_framelength = imgsensor_info.cap.framelength;
			imgsensor.min_frame_length = imgsensor.frame_length;
			spin_unlock(&imgsensor_drv_lock);
		}
		set_dummy();
	...
	case MSDK_SCENARIO_ID_HIGH_SPEED_VIDEO:
		...
		imgsensor.dummy_line = (frame_length > imgsensor_info.hs_video.framelength) ?
			(frame_length - imgsensor_info.hs_video.framelength) : 1;
		imgsensor.frame_length = imgsensor_info.hs_video.framelength + imgsensor.dummy_line;
		curr_framelength = imgsensor_info.hs_video.framelength;
		imgsensor.min_frame_length = imgsensor.frame_length;
		spin_unlock(&imgsensor_drv_lock);
		set_dummy();
		break;
	case MSDK_SCENARIO_ID_SLIM_VIDEO:
		...
		imgsensor.dummy_line = (frame_length > imgsensor_info.slim_video.framelength) ?
			(frame_length - imgsensor_info.slim_video.framelength) : 1;
		imgsensor.frame_length = imgsensor_info.slim_video.framelength + imgsensor.dummy_line;
		curr_framelength = imgsensor_info.slim_video.framelength;
		imgsensor.min_frame_length = imgsensor.frame_length;
		spin_unlock(&imgsensor_drv_lock);
		set_dummy();
		break;
...
UINT32 OV02BSEC_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
