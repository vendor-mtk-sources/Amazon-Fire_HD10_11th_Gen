...
static int compat_get_disp_caps_info(struct compat_disp_caps_info __user *data32, struct disp_caps_info __user *data)
{
	...
	for (j = 0; j < ARRAY_SIZE(data32->rsz_in_res_list); j++) {
		for (i = 0; i < ARRAY_SIZE(data32->rsz_in_res_list[j]); i++) {
			err |= get_user(u, &(data32->rsz_in_res_list[j][i]));
			err |= put_user(u, &(data->rsz_in_res_list[j][i]));
		}
	}
...
static int compat_put_disp_caps_info(struct compat_disp_caps_info __user *data32, struct disp_caps_info __user *data)
{
	...
	for (j = 0; j < ARRAY_SIZE(data->rsz_in_res_list); j++) {
		for (i = 0; i < ARRAY_SIZE(data->rsz_in_res_list[j]); i++) {
			err |= get_user(u, &(data->rsz_in_res_list[j][i]));
			err |= put_user(u, &(data32->rsz_in_res_list[j][i]));
		}
	}
	...
	for (j = 0; j < ARRAY_SIZE(data->rsz_in_max); j++) {
		...
		struct disp_ccorr_config __user *data)
{
	...
	err |= get_user(u, &(data32->is_dirty));
	err |= put_user(u, &(data->is_dirty));
...
int _compat_ioctl_wait_disp_self_refresh(struct file *file, unsigned long arg)
{
	int ret = 0;
	compat_uint_t __user *data32;
	unsigned int data;
	...
	data32 = compat_ptr(arg);
	if (get_user(data, data32)) {
		DISPWARN("self_refresh get_user failed\n");
		ret = -EFAULT;
	}

	ret = file->f_op->unlocked_ioctl(file, DISP_IOCTL_WAIT_DISP_SELF_REFRESH, (unsigned long)data);
...
int _compat_ioctl_wait_all_jobs_done(struct file *file, unsigned long arg)
{
	...
	DISPMSG("_compat_ioctl_wait_all_jobs_done begin\n");
	ret = file->f_op->unlocked_ioctl(file, DISP_IOCTL_WAIT_ALL_JOBS_DONE, arg);
	DISPMSG("_compat_ioctl_wait_all_jobs_done end\n");
