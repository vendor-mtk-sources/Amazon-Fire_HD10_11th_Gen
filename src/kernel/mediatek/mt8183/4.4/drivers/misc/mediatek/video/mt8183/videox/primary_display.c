...
extern m4u_client_t *g_client;
#endif
static int _primary_display_free_fb_buf_nolock(void);
#endif
...
#ifdef FREE_FB_BUFFER
int primary_display_set_frame_buffer_size(unsigned int size)
{
...
static int _primary_display_free_fb_buf_nolock(void)
{
	...
	if (!is_fb_freed) {
		...
		if (ret)
			DISPPR_ERROR("m4u_dealloc_mva returns fail: %d\n", ret);
#endif
		...
		pgc->framebuffer_va = 0x0;
		pgc->framebuffer_mva = 0x0;
			     ...
			     struct cmdqRecStruct *cmdq_handle)
{
	...
	for (i = 0; i < cfg->input_layer_num; i++) {
...
#ifdef FREE_FB_BUFFER
		/* no risk unholding fb_free_lock now */
		...
		if ((unsigned long)pgc->framebuffer_mva < ovl_cfg->addr + layer_size
			&& ovl_cfg->addr < (unsigned long)pgc->framebuffer_mva + framebuffer_size) {
			ovl_is_config_fb = 1;
			DISPMSG("ovl used framebuffer addr: 0x%lx\n", ovl_cfg->addr);
		}
#endif
	}
#ifdef FREE_FB_BUFFER
		...
		unsigned int size, unsigned char force_update)
{
	...
	ret = cmdqRecCreate(CMDQ_SCENARIO_DISP_ESD_CHECK,
		&cmdq_handle_backlight);
	if (ret) {
		DISPPR_ERROR("fail to create primary cmdq handle\n");
		return -1;
	}
		...
		unsigned int size, unsigned char force_update)
{
	...
	ret = cmdqRecCreate(CMDQ_SCENARIO_DISP_ESD_CHECK,
		&cmdq_handle_backlight);
	if (ret) {
		DISPPR_ERROR("fail to create cmdq handle for backlight\n");
		return -1;
	}
