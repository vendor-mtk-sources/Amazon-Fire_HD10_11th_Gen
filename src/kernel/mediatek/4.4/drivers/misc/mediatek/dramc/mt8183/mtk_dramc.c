...
{
	static int lp4_rough_data_rate[] =    {3588, 3198, 2392, 1599};
	static int lp4_standard_data_rate[] = {3600, 3200, 2400, 1600};
	int i;
	unsigned int arr_size = ARRAY_SIZE(lp4_rough_data_rate);

	if (dram_type == TYPE_LPDDR4 || dram_type == TYPE_LPDDR4X) {
		int upper, lower;

		/* if rough data rate is the standard data rate. */
		...
		for (i = 0; i < arr_size; ++i)
			if (lp4_standard_data_rate[i] >= lower
			    && lp4_standard_data_rate[i] <= upper)
				return lp4_standard_data_rate[i];
	}
...
unsigned int get_dram_data_rate(void)
{
	...
	if (DRAM_TYPE == TYPE_LPDDR3) {
		...
		u4DataRate = get_mapped_dram_rate(TYPE_LPDDR4,
						u4DataRate, 130, -130);
	} else
		u4DataRate = 0;
...
void zqcs_timer_callback(unsigned long data)
{
	...
	if ((get_dram_data_rate() >= 3200) || (low_freq_counter >= 10))
		...
		low_freq_counter++;
		mod_timer(&zqcs_timer, jiffies + msecs_to_jiffies(SW_CHECK_PERIOD));
		return;
	}
...
{
	mod_timer(&zqcs_timer, jiffies + msecs_to_jiffies(SW_CHECK_PERIOD)); /* add_timer(&zqcs_timer); */
}
...
static int dram_probe(struct platform_device *pdev)
{
	...
	if ((DRAM_TYPE == TYPE_LPDDR4) || (DRAM_TYPE == TYPE_LPDDR4X)) {
		...
		zqcs_timer.data = 0;
		if (mod_timer(&zqcs_timer, jiffies + msecs_to_jiffies(SW_CHECK_PERIOD)))
