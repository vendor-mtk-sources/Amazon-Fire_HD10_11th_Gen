...
#define ALSPS_VER(fmt, args...)		pr_debug(ALSPS_TAG"%s: "fmt,\
						__func__, ##args)
...
struct ps_control_path {
	...
	/* open data rerport to HAL */
	int (*access_data_fifo)(void);
	/* version2.used for flush operate */
	...
	bool is_report_input_direct;
	bool is_support_batch;
	/* version2.used for batch mode support flag */
	bool is_polling_mode;
	bool is_use_common_factory;
};
...
struct alsps_drv_obj {
	void *self;
	int polling;
	int (*alsps_operate)(void *self, uint32_t command,
...
struct alsps_context {
	...
	atomic_t			trace;
	atomic_t			delay_als;
	/*als polling period for reporting input event*/
	atomic_t				delay_ps;
	/*ps polling period for reporting input event*/
	atomic_t			wake;
