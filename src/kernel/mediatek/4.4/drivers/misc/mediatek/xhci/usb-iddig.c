...
static enum usb_swmode mtk_usb_swmode_cur = DUAL_PROP_IDLE;

static struct delayed_work mtk_xhci_delaywork;
static bool manual_drd_enabled;
...
static int otg_iddig_probe(struct platform_device *pdev)
{
	...
	manual_drd_enabled = of_property_read_bool(node, "enable-manual-drd");
#ifdef CONFIG_SYSFS
	if (manual_drd_enabled) {
		...
		mt_usb_swconnect();
		mtk_usb_swmode_cur = DUAL_PROP_DEVICE;
	}
#endif
	if (!manual_drd_enabled) {
	...
	drvvbus_not_supported = of_property_read_bool(node, "drvvbus-not-supported");
...
static ssize_t otg_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (mtk_usb_swmode_cur == DUAL_PROP_HOST)
		dev_err(dev, "current mode: host\n");
	else if (mtk_usb_swmode_cur == DUAL_PROP_DEVICE)
		dev_err(dev, "current mode: device\n");
	else if (mtk_usb_swmode_cur == DUAL_PROP_IDLE)
		dev_err(dev, "current mode: idle\n");
	return 0;
}
			 ...
			 const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "host", 4)) {
		if (mtk_usb_swmode_cur == DUAL_PROP_IDLE) {
			mtk_xhci_driver_load(!drvvbus_not_supported);
			mtk_usb_swmode_cur = DUAL_PROP_HOST;
		} else {
			dev_err(dev, "current mode is %s\n",
				(mtk_usb_swmode_cur == DUAL_PROP_HOST)
				? "host" : "device");
		}
		...
		if (mtk_usb_swmode_cur == DUAL_PROP_HOST)
			mtk_xhci_driver_unload(!drvvbus_not_supported);
		else if (mtk_usb_swmode_cur == DUAL_PROP_DEVICE)
			mt_usb_swdisconnect();

		mtk_usb_swmode_cur = DUAL_PROP_IDLE;
	}
