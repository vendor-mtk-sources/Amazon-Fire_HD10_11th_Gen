	...
	enum mt6370_adc_sel adc_sel, int *adc_val)
{
	...
	if (adc_sel == MT6370_ADC_TEMP_JC)
		dev_dbg(chg_data->dev, "%s: Select ADC channel to TEMP_JC\n",
			__func__);
	...
	if (adc_sel == MT6370_ADC_TEMP_JC)
		dev_dbg(chg_data->dev, "%s: Start ADC conversion\n", __func__);
	...
	if (adc_sel == MT6370_ADC_TEMP_JC)
		dev_dbg(chg_data->dev, "%s: wait_times = %d\n", __func__, i);
...
static int mt6370_get_ts_bat(struct charger_device *chg_dev, u32 *ts)
{
	struct mt6370_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);
	u32 chg_vddp = 0, ts_bat = 0;
	int ret = 0;

	/* Get adc chg_vddp reference voltage */
	ret = mt6370_get_adc(chg_data, MT6370_ADC_CHG_VDDP, &chg_vddp);
	if (ret < 0) {
		dev_err(chg_data->dev, "%s: get chg_vddp fail\n", __func__);
		return ret;
	}

	/* Get ts_bat(0.01%) */
	ret = mt6370_get_adc(chg_data, MT6370_ADC_TS_BAT, &ts_bat);
	if (ret < 0) {
		dev_err(chg_data->dev, "%s: get ts_bat fail\n", __func__);
		return ret;
	}
	*ts = ts_bat;
	dev_info(chg_data->dev, "%s: chg_vddp = %dmV, ts_bat = %d\n",
			__func__, chg_vddp / 1000, ts_bat);
	return ret;
}
...
extern void mtk_xhci_rerun_dock_detection(void);
#endif
...
static struct charger_ops mt6370_chg_ops = {
	...
	.get_ts_bat_adc = mt6370_get_ts_bat,
...
static ssize_t bat_fault_check_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	chg_data = dev_get_drvdata(&chg_dev->dev);

	mt6370_enable_jeita(chg_data, true);
	mt6370_get_ts_bat(chg_dev, &ts_bat);
	ret = mt6370_pmu_reg_read(chg_data->chip, MT6370_PMU_REG_CHGNTC);
	if (ret < 0)
		return scnprintf(buf, PAGE_SIZE, "-1\n");
	bat_fault = ret;
	mt6370_enable_jeita(chg_data, false);
	return scnprintf(buf, PAGE_SIZE, "0x%02x(%d)\n", bat_fault, ts_bat);
}
static const DEVICE_ATTR_RO(bat_fault_check);
...
static int mt6370_pmu_charger_probe(struct platform_device *pdev)
{
	...
	ret = device_create_file(chg_data->dev, &dev_attr_bat_fault_check);
	if (ret < 0) {
		dev_notice(&pdev->dev, "create bat_fault_check attr fail\n");
		goto err_register_ls_dev;
	}
