...
int disp_lcm_get_vendor_id(void)
{
	const char *node_path = "/odm/lcm_panels";
	struct device_node *node = NULL;
	int lcm_vendor_id = -1;

	node = of_find_node_by_path(node_path);
	if (!node) {
		DISPPR_ERROR("Not found lcm node: %s\n", node_path);
		return -1;
	}
	if (of_property_read_u32(node, "lcm_vendor_id", &lcm_vendor_id)) {
		DISPPR_ERROR("Not found chosen-id property\n");
		return -1;
	}
	if (lcm_vendor_id < 0 || lcm_vendor_id >= 0xFF) {
		DISPPR_ERROR("Invalid LCM vendor id %d\n", lcm_vendor_id);
		return -1;
	}
	return lcm_vendor_id;
}
EXPORT_SYMBOL(disp_lcm_get_vendor_id);
