...
*****************************************************************************/
int g_debugfs_enable_latency_check;
...
static ssize_t ft_fw_read_proc(struct file *file, char __user *buff,
				    size_t count, loff_t *ppos)
{
	...
	if (ret < 0)
		FTS_ERROR("read fw version fail\n");

	ret = fts_read_reg(FTS_REG_LIC_NEW_VER, &lic_ver);
	if (ret < 0)
		FTS_ERROR("read lic_ver fail\n");

	ret = fts_read_reg(FTS_REG_VENDOR_ID, &vendor_id);
	if (ret < 0)
		FTS_ERROR("read vendor_id fail\n");

	ret = fts_read_reg(FTS_REG_PANEL_ID, &panel_id);
	if (ret < 0)
		FTS_ERROR("read panel_id fail\n");

#if FTS_ESDCHECK_EN
	fts_esdcheck_proc_busy(0);
#endif
	len += scnprintf(tmpbuf + len, PROC_BUF_SIZE - len,
		"fw_ver:0x%02X ", fw_ver);
	len += scnprintf(tmpbuf + len, PROC_BUF_SIZE - len,
		"lic_ver:0x%02X ", lic_ver);
	len += scnprintf(tmpbuf + len, PROC_BUF_SIZE - len,
		"pid:0x%02X ", panel_id);
	len += scnprintf(tmpbuf + len, PROC_BUF_SIZE - len,
		"vid:0x%02X\n", vendor_id);
	len = simple_read_from_buffer(buff, count, ppos, tmpbuf, len);
	return len;
}

static struct proc_dir_entry *fw_proc_entry;
...
int fts_create_apk_debug_channel(struct fts_ts_data *ts_data)
{
	...
	fw_proc_entry = proc_create(PROC_FW_NAME, 0444, NULL, &ft_fw_proc_fops);
	if (fw_proc_entry == NULL) {
	...
	struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	fts_read_reg(FTS_REG_LIC_NEW_VER, &val);
	count += snprintf(buf + count, PAGE_SIZE,
		"LCD Initcode new Ver:0x%02x\n", val);
...
static DEVICE_ATTR(fts_fw_version, S_IRUGO, fts_tpfwver_show, NULL);
...
static DEVICE_ATTR(fts_upgrade_bin, S_IWUSR | S_IWGRP, NULL, fts_fwupgradebin_store);
static DEVICE_ATTR(fts_force_upgrade, S_IWUSR, NULL, fts_fwforceupg_store);
static DEVICE_ATTR(fts_driver_info, S_IRUGO, fts_driverinfo_show, NULL);
static DEVICE_ATTR(fts_dump_reg, S_IRUGO, fts_dumpreg_show, NULL);
static DEVICE_ATTR(fts_hw_reset, S_IRUGO, fts_hw_reset_show, NULL);
...
static DEVICE_ATTR(fts_touch_point, S_IRUGO, fts_tpbuf_show, NULL);
...
static int gtp_debugfs_power_get(void *data, u64 *val)
{
	struct fts_ts_data *ts_data = data;
	*val = !(ts_data->suspended);
	FTS_INFO("tp_power = %d", !(ts_data->suspended));
	return 0;
};

static int gtp_debugfs_tp_power_set(void *data, u64 val)
{
	if (val == 1) {
		tpd_resume(NULL);
		FTS_INFO("tp_power_set tpd_resume");
	} else if (val == 0) {
		tpd_suspend(NULL);
		FTS_INFO("tp_power_set suspended");
	}
	return 0;
};

DEFINE_SIMPLE_ATTRIBUTE(debug_fs_tp_power_fops, gtp_debugfs_power_get,
		gtp_debugfs_tp_power_set, "%llu\n");
...
int fts_create_sysfs(struct fts_ts_data *ts_data)
{
	...
	/* Add sysfs Link*/
	fts_touchscreen_link = kobject_create_and_add("touchscreen", NULL);
	if (fts_touchscreen_link != NULL) {
		ret = sysfs_create_link(fts_touchscreen_link,
				&ts_data->dev->kobj,
				"link_input_dev");
		if (ret < 0)
			FTS_ERROR("[focal]Failed to create sysfs link for input_dev\n");
	}

	/* Create debugfs*/
	ts_data->debug_dir_touchscreen =
		debugfs_create_dir("touchscreen", NULL);
	if (ts_data->debug_dir_touchscreen) {
		debugfs_create_file("latency_enable", S_IRUSR | S_IWUSR,
				ts_data->debug_dir_touchscreen, ts_data,
		...
		FTS_ERROR("Cannot create debugfs\n");
	}
...
{
	sysfs_remove_group(&ts_data->dev->kobj, &fts_attribute_group);
	debugfs_remove_recursive(ts_data->debug_dir_touchscreen);
	return 0;
}
