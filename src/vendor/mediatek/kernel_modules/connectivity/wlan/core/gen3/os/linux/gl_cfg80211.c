	...
	if (request->n_ssids == 0) {
		...
		for (i = 0; i < request->n_ssids; i++) {
			...
			if (u4ValidIdx == SCN_SSID_MAX_NUM) {
				DBGLOG(REQ, TRACE, "i=%d, u4ValidIdx is SCN_SSID_MAX_NUM\n", i);
				break;
			}
		}
...
int mtk_cfg80211_connect(struct wiphy *wiphy, struct net_device *ndev, struct cfg80211_connect_params *sme)
{
	...
	rNewSsid.u4CenterFreq = sme->channel_hint ? sme->channel_hint->center_freq : 0;
...
mtk_cfg80211_testmode_get_sta_statistics(IN struct wiphy *wiphy, IN void *data, IN int len, IN P_GLUE_INFO_T prGlueInfo)
{
	...
	if (data && (len >= sizeof(NL80211_DRIVER_GET_STA_STATISTICS_PARAMS))) {
		prParams = (P_NL80211_DRIVER_GET_STA_STATISTICS_PARAMS) data;
	} else {
		DBGLOG(QM, WARN, "%s data is null or len is invalid\n", __func__);
		return  -EINVAL;
	}
...
INT_32 mtk_cfg80211_process_str_cmd(struct wiphy *wiphy, struct wireless_dev *wdev, PUINT_8 data, INT_32 len)
{
	WLAN_STATUS rStatus = WLAN_STATUS_SUCCESS;
	UINT_32 u4SetInfoLen = 0;
