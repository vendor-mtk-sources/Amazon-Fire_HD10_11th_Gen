...
static char sodi_status[300];
char *sodi_map_status(u32 step)
{
	char *p = sodi_status;
	char *buff_end = sodi_status + sizeof(sodi_status);

	memset(sodi_status, 0, sizeof(sodi_status));
	if (step == 0)
		return "leave sodi successful !!";

	if (step & (1 << SPM_SODI_ENTER))
		p += scnprintf(p, buff_end-p, "Enter sodi |");
	if (step & (1 << SPM_SODI_ENTER_SSPM_ASYNC_IPI_BEFORE_WFI))
		p += scnprintf(p, buff_end-p, "Enter sspm async ipi before wfi |");
	if (step & (1 << SPM_SODI_ENTER_SPM_FLOW))
		p += scnprintf(p, buff_end-p, "Enter spm flow |");
	if (step & (1 << SPM_SODI_ENTER_UART_SLEEP))
		p += scnprintf(p, buff_end-p, "Enter uart sleep |");
	if (step & (1 << SPM_SODI_B4))
		p += scnprintf(p, buff_end-p, "Enter sodi b4 |");
	if (step & (1 << SPM_SODI_B5))
		p += scnprintf(p, buff_end-p, "Enter sodi b5 |");
	if (step & (1 << SPM_SODI_B6))
		p += scnprintf(p, buff_end-p, "Enter sodi b6 |");
	if (step & (1 << SPM_SODI_ENTER_WFI))
		p += scnprintf(p, buff_end-p, "Enter wfi |");
	if (step & (1 << SPM_SODI_LEAVE_WFI))
		p += scnprintf(p, buff_end-p, "Leave wfi |");
	if (step & (1 << SPM_SODI_ENTER_UART_AWAKE))
		p += scnprintf(p, buff_end-p, "Leave uart sleep |");
	if (step & (1 << SPM_SODI_LEAVE_SSPM_ASYNC_IPI_AFTER_WFI))
		p += scnprintf(p, buff_end-p, "Leave sspm async ipi after wfi |");
	if (step & (1 << SPM_SODI_LEAVE_SPM_FLOW))
		p += scnprintf(p, buff_end-p, "Leave spm flow |");
	if (step & (1 << SPM_SODI_LEAVE))
		p += scnprintf(p, buff_end-p, "Leave sodi");
