...
int charger_dev_enable(struct charger_device *charger_dev, bool en)
{
	if (charger_dev != NULL && charger_dev->ops != NULL && charger_dev->ops->enable) {
		dev_info(&charger_dev->dev, "%s: en=%d\n", __func__, en);
...
int charger_dev_get_vbus(struct charger_device *charger_dev, u32 *vbus)
{
	if (charger_dev != NULL && charger_dev->ops != NULL
		&& charger_dev->ops->get_vbus_adc)
		return charger_dev->ops->get_vbus_adc(charger_dev, vbus);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_get_vbus);

int charger_dev_get_ts_bat(struct charger_device *chg_dev, u32 *ts)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
			chg_dev->ops->get_ts_bat_adc)
		return chg_dev->ops->get_ts_bat_adc(chg_dev, ts);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_get_ts_bat);
...
int charger_dev_get_mivr(struct charger_device *charger_dev, u32 *uV)
{
	if (charger_dev != NULL && charger_dev->ops != NULL && charger_dev->ops->get_mivr)
		return charger_dev->ops->get_mivr(charger_dev, uV);
...
int wireless_charger_dev_set_vout_en(struct charger_device *chg_dev, bool en)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
			chg_dev->ops->set_vout_en)
		return chg_dev->ops->set_vout_en(chg_dev, en);
