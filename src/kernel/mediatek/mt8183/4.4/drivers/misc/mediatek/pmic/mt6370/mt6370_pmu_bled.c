...
void mt6370_bled_set_brightness(uint32_t level)
{
	struct led_classdev *led_cdev = &mt6370_pmu_bled_dev;
	if (IS_ERR_OR_NULL(led_cdev->dev)) {
		dev_err(led_cdev->dev, "%s: mt6370_bled device is not initialized.\n",
				__func__);
		return;
	}
	mt6370_pmu_bled_bright_set(led_cdev, level);
}
EXPORT_SYMBOL(mt6370_bled_set_brightness);
	...
	uint32_t ramptime)
{
	...
	ret = mt6370_pmu_reg_update_bits(bled_data->chip, MT6370_PMU_REG_BLCTRL,
			MT6370_RAMP_MASK, ramptime << MT6370_BLED_RAMPTSHFT);
	if (ret < 0)
...
void mt6370_bled_set_ramptime(uint32_t ramptime)
{
	struct led_classdev *led_cdev = &mt6370_pmu_bled_dev;
	...
	mt6370_pmu_bled_ramptime_set(led_cdev, ramptime);
}
EXPORT_SYMBOL(mt6370_bled_set_ramptime);

static void mt6370_pmu_bled_pwm_hys_set(struct led_classdev *led_cdev,
	uint32_t hys_en, uint32_t hys_val)
{
	struct mt6370_pmu_bled_data *bled_data =
				dev_get_drvdata(led_cdev->dev->parent);
	int ret = 0;

	dev_dbg(led_cdev->dev, "%s: %d %d\n", __func__, hys_en, hys_val);

	ret = mt6370_pmu_reg_update_bits(bled_data->chip, MT6370_PMU_REG_BLPWM,
		MT6370_PWM_HYS_VAL_MASK, hys_val << MT6370_PWM_HYS_VAL_SHIFT);
	if (ret < 0)
		goto out_pwm_hys_set;

	ret = mt6370_pmu_reg_update_bits(bled_data->chip, MT6370_PMU_REG_BLPWM,
		MT6370_PWM_HYS_EN_MASK, hys_en << MT6370_PWM_HYS_EN_SHIFT);
	if (ret < 0)
		goto out_pwm_hys_set;
...
void mt6370_bled_set_pwm_hys(uint32_t hys_en, uint32_t hys_val)
{
	struct led_classdev *led_cdev = &mt6370_pmu_bled_dev;
	...
	mt6370_pmu_bled_pwm_hys_set(led_cdev, hys_en, hys_val);
}
EXPORT_SYMBOL(mt6370_bled_set_pwm_hys);
