...
struct last_reboot_reason {
	...
	uint32_t rgidle_footprint[AEE_MTK_CPU_NUMS];
};
...
void aee_rr_show_deepidle_cpu(struct seq_file *m, int cpu);
void aee_rr_show_sodi3_cpu(struct seq_file *m, int cpu);
void aee_rr_show_sodi_cpu(struct seq_file *m, int cpu);
void aee_rr_show_spm_suspend_cpu(struct seq_file *m, int cpu);
void aee_rr_show_rgidle_footprint(struct seq_file *m, int cpu);
...
{
	return "Target not support to track deepidle status";
}

char __attribute__ ((weak)) *get_dvfsrc_latch_info(void)
{
	return "Target not support to track dvfsrc latch info";
}

char __attribute__ ((weak)) *sodi3_map_status(u32 step)
{
	return "Target not support to track sodi3 status";
}

char __attribute__ ((weak)) *sodi_map_status(u32 step)
{
	return "Target not support to track sodi status";
}

char __attribute__ ((weak)) *spm_suspend_map_status(u32 step)
{
	return "Target not support to track spm suspend status";
}
...
void aee_rr_rec_rgidle_val(int id, u32 val)
{
	if (!ram_console_init_done || !ram_console_buffer)
		return;
	LAST_RR_SET_WITH_ID(rgidle_footprint, id, val);
}
...
{
	if ((LAST_RRR_VAL(deepidle_data) >> 24) == cpu)
		aee_rr_show_deepidle(m);
}

void aee_rr_show_sodi3(struct seq_file *m)
{
	seq_printf(m, "  sodi3            : %s (0x%x)\n",
			sodi3_map_status(LAST_RRR_VAL(sodi3_data)),
			LAST_RRR_VAL(sodi3_data));
}
...
{
	seq_printf(m, "  spm_suspend      : %s (0x%x)\n",
			spm_suspend_map_status(LAST_RRR_VAL(spm_suspend_data)),
			LAST_RRR_VAL(spm_suspend_data));
}
...
{
	if (LAST_RRR_VAL(mcdi_footprint[cpu]))
		seq_printf(m, "  mcdi             : Stuck in CPU/CLUSTER off (0x%x)\n", LAST_RRR_VAL(mcdi_footprint[cpu]));
	else
		seq_printf(m, "  mcdi             : leave CPU/CLUSTER off successful !! (0x%x)\n", LAST_RRR_VAL(mcdi_footprint[cpu]));
}

void aee_rr_show_rgidle_footprint(struct seq_file *m, int cpu)
{
	if (LAST_RRR_VAL(rgidle_footprint[cpu]))
		seq_printf(m, "  rgidle           : Stuck in wfi (0x%x)\n", LAST_RRR_VAL(rgidle_footprint[cpu]));
	else
		seq_printf(m, "  rgidle           : leave wfi successful !! (0x%x)\n", LAST_RRR_VAL(rgidle_footprint[cpu]));
}
