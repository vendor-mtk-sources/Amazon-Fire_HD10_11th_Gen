...
#define KEY_GESTURE_ONE						   KEY_POWER
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	mutex_lock(&ts_data->input_dev->mutex);
	if (!ts_data->gesture_filter) {
		FTS_ERROR("cannot switch gesture mode during suspend");
		mutex_unlock(&ts_data->input_dev->mutex);
		return -EINVAL;
	}
...
static void fts_gesture_report(struct input_dev *input_dev, int gesture_id)
{
	...
	switch (gesture_id) {
	case GESTURE_ONECLICK:
		gesture = KEY_GESTURE_ONE;
		break;
	case GESTURE_LEFT:
	...
	if (gesture != -1) {
		...
		snprintf(buffer, sizeof(buffer),
				"touch_event:def:gesture_wakeup_code=%d;CT;1:NR",
				gesture);
