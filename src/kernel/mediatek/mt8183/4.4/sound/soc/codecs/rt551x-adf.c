...
#ifdef CONFIG_SND_SOC_RT551X_PINCTRL
struct pinctrl *pinctrl_hwreset;
struct rt551x_gpio_attr {
	const char *name;
	bool gpio_prepare;
	struct pinctrl_state *gpioctrl;
};
enum rt551x_gpio_type {
	GPIO_RT551X_HWRESET_DISABLE = 0,
	GPIO_RT551X_HWRESET_ENABLE,
	GPIO_NUM
};
static struct rt551x_gpio_attr rt551x_gpios[GPIO_NUM] = {
	[GPIO_RT551X_HWRESET_DISABLE] = {"rt551x_hwreset_disable", false, NULL},
	[GPIO_RT551X_HWRESET_ENABLE] = {"rt551x_hwreset_enable", false, NULL},
};
#endif
...
{
	int ret = 0;
#ifndef CONFIG_SND_SOC_RT551X_PINCTRL
...
err_gpio:
	return ret;
#else
	pinctrl_hwreset = devm_pinctrl_get(&rt551x->i2c->dev);
	if (IS_ERR(pinctrl_hwreset)) {
		ret = PTR_ERR(pinctrl_hwreset);
		dev_err(&rt551x->i2c->dev,
			"%s Cannot find pinctrl_hwreset\n", __func__);
	}
	for (index_gpio = 0; index_gpio < ARRAY_SIZE(rt551x_gpios);
		 index_gpio++) {
		rt551x_gpios[index_gpio].gpioctrl =
			pinctrl_lookup_state(pinctrl_hwreset,
			rt551x_gpios[index_gpio].name);
		if (IS_ERR(rt551x_gpios[index_gpio].gpioctrl)) {
			ret = PTR_ERR(
				rt551x_gpios[index_gpio].gpioctrl);
			dev_err(&rt551x->i2c->dev,
				"%s pinctrl_lookup_state fail\n",
				__func__);
		} else {
			rt551x_gpios[index_gpio].gpio_prepare = true;
			dev_dbg(&rt551x->i2c->dev,
				"%s pinctrl_lookup_state success!\n", __func__);
		}
	}
	return ret;
#endif
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] = rt551x->dsp_mode;
...
int rt551x_parse_rtk_header(struct snd_soc_codec *codec, const u8 *buf)
{
#ifdef CONFIG_SND_SOC_RT551X_CHECK
	struct rt551x_priv *rt551x = snd_soc_codec_get_drvdata(codec);
	u8 *cmp_buf = NULL;
#endif
	...
	for (i = 0 ; i < sMicFWHeader.NumBin; i++) {
		...
		pr_info("sMicFWHeader.BinArray[%d].Offset = %08x\n",
			i, sMicFWHeader.BinArray[i].Offset);

		offset += 4;
		sMicFWHeader.BinArray[i].Size =
			rt551x_4byte_le_to_uint(buf + offset);
		pr_info("sMicFWHeader.BinArray[%d].Size = %08x\n",
			i, sMicFWHeader.BinArray[i].Size);

		offset += 4;
		sMicFWHeader.BinArray[i].Addr =
			rt551x_4byte_le_to_uint(buf + offset);
		pr_info("sMicFWHeader.BinArray[%d].Addr = %08x\n",
		...
		if (rt551x->dsp_fw_check) {
			...
			if (memcmp(buf + sMicFWHeader.BinArray[i].Offset, cmp_buf,
				...
				pr_info("sMicFWHeader.BinArray[%d].Addr = %08x firmware check failed\n",
					i, sMicFWHeader.BinArray[i].Addr);
				kfree(cmp_buf);
				ret = -1;
				goto exit_BinArray;
				...
				pr_info("sMicFWHeader.BinArray[%d].Addr = %08x firmware check successful\n",
					i, sMicFWHeader.BinArray[i].Addr);
				kfree(cmp_buf);
			}
		}
...
bool rt551x_bin_cpy(void *dest, void *src, size_t len)
{
	uintptr_t destaddr = (uintptr_t)dest;
	if (rt551x_spi_burst_write((u32)((destaddr << 32) >> 32), src, (len + 7) & (~0x7)) == 0)
		return true;
	else
		return false;
}

bool rt551x_bin_read(void *src, void *dest, size_t len)
{
	uintptr_t srcAddr = (uintptr_t)src;
	return rt551x_spi_burst_read((u32)((srcAddr << 32) >> 32), dest, (len + 7) & (~0x7));
}

void rt551x_parse_header(struct snd_soc_codec *codec, const u8 *src, u32 size)
{
#ifdef CONFIG_SND_SOC_RT551X_CHECK
	struct rt551x_priv *rt551x = snd_soc_codec_get_drvdata(codec);
#endif
	...
	if (magic == ADF_FW_MAGIC) {
		pr_info("%s header magic amzn! 0x%x\n", __func__, magic);
		adfLoad_init((void *)src, NULL, size, rt551x_bin_cpy, RT551X_CORE_NO);
#ifdef CONFIG_SND_SOC_RT551X_CHECK
		if (rt551x->dsp_fw_check) {
			if (adfLoad_initCheck((void *)src, NULL, rt551x_bin_read))
				rt551x->dsp_fw_check = 2;
			else
				rt551x->dsp_fw_check = 3;
		}
#endif
	} else if (magic == SMICFW_SYNC) {
		pr_info("%s header magic rtk! 0x%x\n", __func__, magic);
		rt551x_parse_rtk_header(codec, src);
	} else
		pr_err("%s header magic unknown! 0x%x\n", __func__, magic);
	return ;
}
...
static int rt551x_set_dsp_mode(struct snd_soc_codec *codec, int DSPMode)
{
	...
	if (snd_soc_codec_get_bias_level(codec) == SND_SOC_BIAS_OFF) {
		if ((rt551x->dsp_mode != DSP_MODE_DISABLE) && (DSPMode != DSP_MODE_DISABLE))
			restart = true;

		rt551x->dsp_mode = DSPMode;

		if (rt551x->dsp_mode != DSP_MODE_DISABLE) {
			if (restart) {
			...
			if (rt551x->chip_id == RT5514_DEVICE_ID)
				...
				request_firmware(&fw, rt551x_dsp_binary_name[rt551x->dsp_mode], codec->dev);
			if (fw) {
				rt551x_parse_header(codec, fw->data, fw->size);
				release_firmware(fw);
				fw = NULL;
			} else {
...
#else
			if (rt551x_gpios[
				GPIO_RT551X_HWRESET_ENABLE].gpio_prepare) {
				pinctrl_select_state(pinctrl_hwreset,
					rt551x_gpios[
					GPIO_RT551X_HWRESET_ENABLE].gpioctrl);
				pinctrl_select_state(pinctrl_hwreset,
					rt551x_gpios[
					GPIO_RT551X_HWRESET_DISABLE].gpioctrl);
				dev_dbg(codec->dev,
				"%s hw reset pin pull low and pull high success!\n",
				__func__);
			} else {
				dev_err(codec->dev,
					"%s GPIO_RT551X_HWRESET_ENABLE are not prepared!\n",
					__func__);
			}
#endif
		} else {
...
#else
			if (rt551x_gpios[
				GPIO_RT551X_HWRESET_DISABLE].gpio_prepare) {
				pinctrl_select_state(pinctrl_hwreset,
					rt551x_gpios[
					GPIO_RT551X_HWRESET_DISABLE].gpioctrl);
			} else {
				dev_err(codec->dev,
					"%s GPIO_RT551X_HWRESET_DISABLE are not prepared!\n",
					__func__);
			}
#endif
		}
...
static int rt551x_dsp_set_idle_mode(struct snd_soc_codec *codec, int IdleMode)
{
	...
	} else if (rt551x->dsp_mode != DSP_MODE_DISABLE) {
			...
			enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_PREPARE:
		if (snd_soc_codec_get_bias_level(codec) == SND_SOC_BIAS_STANDBY) {
			if (rt551x->dsp_mode != DSP_MODE_DISABLE) {
				rt551x->dsp_mode = DSP_MODE_DISABLE;
				rt551x_reset(rt551x);
				regcache_mark_dirty(rt551x->regmap);
				regcache_sync(rt551x->regmap);
			}
	...
	if (adfDspPriv != NULL) {
		...
		if (adfDspPriv->dspHeader == NULL)
			...
			if (logInfo) {
				...
				if (!ret)
					pr_err("%s logData spi_burst_read failed! %d\n", __func__, ret);
				else
