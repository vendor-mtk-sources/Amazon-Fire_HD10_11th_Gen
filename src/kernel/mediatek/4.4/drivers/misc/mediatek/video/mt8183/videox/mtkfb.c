...
extern struct mutex fb_free_lock;
#endif
...
#define MTK_FB_BL_MAX_BRIGHTNESS  1023
...
static int _convert_fb_layer_to_disp_input(struct fb_overlay_layer *src, struct disp_input_config *dst)
{
	...
	/* begin add for supporting lk logo's color format */
	...
	case MTK_FB_FORMAT_RGBA8888:
		...
		break;
	/* end add for supporting lk logo's color format */
	...
	if (MTK_FB_FORMAT_ARGB8888 == src->src_fmt || MTK_FB_FORMAT_ABGR8888 == src->src_fmt
		|| MTK_FB_FORMAT_BGRA8888 == src->src_fmt || MTK_FB_FORMAT_RGBA8888 == src->src_fmt)
...
static int mtkfb_set_par(struct fb_info *fbi)
{
	...
	case 32:
		...
		/* align pan_display's setting: (var->blue.offset == 0) ? DISP_FORMAT_BGRA8888 : DISP_FORMAT_RGBX8888; */
		fb_layer.src_fmt = (var->blue.offset == 0) ?
...
#ifdef FREE_FB_BUFFER
		/* DO NOT support get framebuffer mva when FREE_FB_BUFFER feature enable.
		 * If return this mva, may cause use-after-free
		 */
		return -EPERM;
#else
...
#ifdef FREE_FB_BUFFER
			mutex_lock(&fb_free_lock);
			if (is_fb_freed) {
				DISPERR("framebuffer has been freed !!!\n");
				mutex_unlock(&fb_free_lock);
				return -EPERM;
			} else {
...
#ifdef FREE_FB_BUFFER
void mtkfb_reset_fb_base(void)
{
	fb_base = 0x0;
}
EXPORT_SYMBOL(mtkfb_reset_fb_base);
void mtkfb_reset_fb_size(void)
{
	vramsize = 0;
}
EXPORT_SYMBOL(mtkfb_reset_fb_size);
#endif
...
#ifdef FREE_FB_BUFFER
	primary_display_set_frame_buffer_size(vramsize);
#endif
