...
#include "gc2375secmipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "GC2375sec_camera_sensor"
#define LOG_1 LOG_INF("GC2375SEC,MIPI 1LANE\n")
/******************************   Modify end    *******************************************/
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC2375SEC_SENSOR_ID,	/* record sensor id defined in Kd_imgsensor.h */
...
static kal_uint32 get_id_pin(void)
{
	...
	if (gpionum == -1) {
		node = of_find_compatible_node(NULL, NULL, "mediatek,rfc_id_pin");
		gpionum = of_get_named_gpio(node, "rfc_id_pin", 0);
	}

	gpio_request(gpionum, "rfc_id_pin");
		...
		do {
			*sensor_id = return_sensor_id() + 2;
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				ret = get_id_pin();
				LOG_INF("rfc_id_pin: %d\n", ret);
				if (ret == 0) {
					LOG_INF("RFC_ID_PIN = %d, it is the main source\n", ret);
...
UINT32 GC2375SEC_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
