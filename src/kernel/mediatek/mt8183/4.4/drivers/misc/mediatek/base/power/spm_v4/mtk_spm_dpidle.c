...
static char idle_status[64];
char *dpidle_map_status(u32 step)
{
	char *p = idle_status;
	char *buff_end = idle_status + sizeof(idle_status);

	memset(idle_status, 0, sizeof(idle_status));
	if (step == 0)
		return "leave deepidle successful !!";

	if (step & SPM_DEEPIDLE_SLEEP_DPIDLE)
		p += scnprintf(p, buff_end-p, "sleep deepidle: ");
	else
		p += scnprintf(p, buff_end-p, "deepidle: ");

	switch (step & ((1U << CPU_FOOTPRINT_SHIFT) - 1)) {
	case SPM_DEEPIDLE_ENTER:
		p += scnprintf(p, buff_end-p,
			"Stuck in deepidle function entry");
		break;
	case SPM_DEEPIDLE_ENTER_UART_SLEEP:
		p += scnprintf(p, buff_end-p, "Stuck in sleep UAR");
		break;
	case SPM_DEEPIDLE_ENTER_SSPM_ASYNC_IPI_BEFORE_WFI:
		p += scnprintf(p, buff_end-p,
			"Stuck in sspm async ipi before wfi");
		break;
	...
	case SPM_DEEPIDLE_LEAVE_WFI:
		...
		break;
	case SPM_DEEPIDLE_LEAVE_SSPM_ASYNC_IPI_AFTER_WFI:
		p += scnprintf(p, buff_end-p,
			"Stuck in sspm async ipi after wfi");
		break;
	case SPM_DEEPIDLE_ENTER_UART_AWAKE:
		p += scnprintf(p, buff_end-p, "Stuck in awake UART");
		break;
	default:
