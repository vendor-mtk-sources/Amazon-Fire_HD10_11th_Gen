...
static void msdc_add_device_attrs(struct msdc_host *host, struct device_attribute *attrs[])
{
	...
	for (i = 0; attrs[i]; ++i) {
		ret = device_create_file(&host->pdev->dev, attrs[i]);
		if (ret)
			dev_err(&host->pdev->dev, "failed to register attribute: %s; err=%d\n",
...
#endif
		dev_info(&host->pdev->dev, "crc/total %d/%d invalcrc %d datato %d cmdto %d reqto %d total_pc %d pc_sus %d\n",
...
int msdc_do_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	host->req_count++;
	dev_dbg(&host->pdev->dev, "crc/total %d/%d invalcrc %d datato %d cmdto %d reqto %d total_pc %d pc_sus %d\n",
			host->crc_count, host->req_count, host->crc_invalid_count, host->datatimeout_count,
			host->cmdtimeout_count, host->reqtimeout_count, host->pc_count, host->pc_suspend);
