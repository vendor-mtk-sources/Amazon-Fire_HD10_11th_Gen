...
static int hx_panel_coords[4] = {0, 1200, 0, 1920};
static int hx_display_coords[4] = {0, 1200, 0, 1920};
...
#else
	if (value)
		hx83102_share_tp_resetpin_high();
	else
		hx83102_share_tp_resetpin_low();
#endif
}
#endif
...
static int himax_common_probe_spi(struct spi_device *spi)
{
	...
	if (hx83102_incell_compare_id() == VID_NULL) {
		E("%s:lcm_id does not match hx83102 incell IC, exit.", __func__);
		return -ENODEV;
	}
	...
	ret = hx83102_share_request_powerpin_resource(ts->dev);
	if (ret < 0)
