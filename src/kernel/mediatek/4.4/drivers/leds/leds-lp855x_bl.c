...
#undef LOG_ERR
#endif
#define LOG_ERR(dev, fmt, args...)	dev_dbg(dev, fmt, ##args)
...
static int lp855x_read_byte(struct lp855x *lp, u8 reg, u8 *data)
{
	...
	if (ret < 0) {
		LOG_ERR(lp->dev, "failed to read 0x%.2x\n", reg);
		return ret;
	}
...
static int lp855x_init_device(struct lp855x *lp)
{
		...
		if (pd->load_new_rom_data && pd->size_program) {
			for (i = 0; i < pd->size_program; i++) {
				...
				dev_dbg(lp->dev,
						...
						addr,
						val);
				pr_debug("[KE/BL] %s Load new rom data 0x%02X 0x%02X\n",
						__func__,
						addr,
						val);
	...
	if (lp->cfg->post_init_device) {
		...
		if (ret) {
			LOG_ERR(lp->dev, "post init device err: %d\n", ret);
			goto err;
		}
	}
...
static int lp855x_probe(struct i2c_client *cl, const struct i2c_device_id *id)
{
	...
	if (!pdata) {
		...
		if (ret < 0) {
			LOG_ERR(&cl->dev, "no platform data supplied\n");
			return ret;
		}
	...
	if (lp->pdata->gpio_en >= 0 && load_switch_support) {
		...
		if (ret != 0) {
			LOG_ERR(lp->dev, "backlight gpio request failed, %d\n", lp->pdata->gpio_en);
	...
	if (ret) {
		LOG_ERR(lp->dev, "device config err: %d\n", ret);
		goto err_dev;
	}
	...
	if (ret) {
		LOG_ERR(lp->dev, " failed to register sysfs. err: %d\n", ret);
		goto err_sysfs;
	}
