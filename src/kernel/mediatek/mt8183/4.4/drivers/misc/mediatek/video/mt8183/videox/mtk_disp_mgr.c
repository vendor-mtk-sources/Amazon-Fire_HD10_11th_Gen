...
static void compat_get_valid_layer(struct disp_layer_info *data, struct compat_disp_layer_info *data32)
{
	int j;

	for (j = 0; j < ARRAY_SIZE(data32->disp_mode); j++)
		data->disp_mode[j] = data32->disp_mode[j];

	for (j = 0; j < ARRAY_SIZE(data32->layer_num); j++)
		data->layer_num[j] = data32->layer_num[j];

	for (j = 0; j < ARRAY_SIZE(data32->gles_head); j++)
		data->gles_head[j] = data32->gles_head[j];

	for (j = 0; j < ARRAY_SIZE(data32->gles_tail); j++)
		data->gles_tail[j] = data32->gles_tail[j];

	data->hrt_num = data32->hrt_num;
	data->res_idx = data32->res_idx;
}
static void compat_put_valid_layer(struct disp_layer_info *data, struct compat_disp_layer_info *data32)
{
	int j;

	for (j = 0; j < ARRAY_SIZE(data->disp_mode); j++)
		data32->disp_mode[j] = data->disp_mode[j];

	for (j = 0; j < ARRAY_SIZE(data->layer_num); j++)
		data32->layer_num[j] = data->layer_num[j];

	for (j = 0; j < ARRAY_SIZE(data->gles_head); j++)
		data32->gles_head[j] = data->gles_head[j];

	for (j = 0; j < ARRAY_SIZE(data->gles_tail); j++)
		data32->gles_tail[j] = data->gles_tail[j];

	data32->hrt_num = data->hrt_num;
	data32->res_idx = data->res_idx;
}

static long _compat_ioctl_query_valid_layer(unsigned long arg)
{
	struct disp_layer_info disp_info_user;
	struct compat_disp_layer_info compat_disp_info_user;

	void __user *argp = (void __user *)arg;

	if (copy_from_user(&compat_disp_info_user, argp, sizeof(compat_disp_info_user))) {
		DISPWARN("[FB]: copy_to_user failed! line:%d\n", __LINE__);
		return -EFAULT;
	}
	compat_get_valid_layer(&disp_info_user, &compat_disp_info_user);
	/* data end */

	disp_info_user.input_config[0] = (void *)(u64)compat_disp_info_user.input_config[0];
	disp_info_user.input_config[1] = (void *)(u64)compat_disp_info_user.input_config[1];

	/* check data from userspace is legal */
	if (disp_info_user.layer_num[0] < 0 || disp_info_user.layer_num[0] > 0x300 ||
		disp_info_user.layer_num[1] < 0 || disp_info_user.layer_num[1] > 0x300) {
		DISPWARN("[FB]: disp_info_user.layer_num[0]= %d, disp_info_user.layer_num[1]= %d!\n",
			disp_info_user.layer_num[0], disp_info_user.layer_num[1]);
		return -EINVAL;
	}
	...
	compat_put_valid_layer(&disp_info_user, &compat_disp_info_user);

	if (copy_to_user(argp, &compat_disp_info_user, sizeof(compat_disp_info_user))) {
		DISPWARN("[FB]: copy_to_user failed! line:%d\n", __LINE__);
		return -EFAULT;
	}
	return 0;
...
static long mtk_disp_mgr_compat_ioctl(struct file *file, unsigned int cmd,  unsigned long arg)
{
	...
	case COMPAT_DISP_IOCTL_QUERY_VALID_LAYER:
		{
		   return  _compat_ioctl_query_valid_layer(arg);
		}
	case COMPAT_DISP_IOCTL_SCREEN_FREEZE:
		{
		   return _compat_ioctl_screen_freeze(file, arg);
		}
	case COMPAT_DISP_IOCTL_WAIT_DISP_SELF_REFRESH:
		{
		   return _compat_ioctl_wait_disp_self_refresh(file, arg);
		}
