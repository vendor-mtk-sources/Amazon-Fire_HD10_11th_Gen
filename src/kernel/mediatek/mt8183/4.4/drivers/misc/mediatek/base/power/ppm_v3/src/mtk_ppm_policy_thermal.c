...
static unsigned int custom_limited_power = INT_MAX;
static unsigned int mtk_limited_power = INT_MAX;
...
static void mt_ppm_cpu_thermal_protect_do(void)
{
	unsigned int limited_power;
	FUNC_ENTER(FUNC_LV_POLICY);

	if (custom_limited_power >= mt_ppm_thermal_get_max_power())
		custom_limited_power = 0;

	if (mtk_limited_power >= mt_ppm_thermal_get_max_power())
		mtk_limited_power = 0;

	ppm_ver("custom_limited_power = %d\n", custom_limited_power);
	ppm_ver("mtk_limited_power = %d\n", mtk_limited_power);

	limited_power = (custom_limited_power == 0 || mtk_limited_power == 0) ? max(custom_limited_power, mtk_limited_power) : min(custom_limited_power, mtk_limited_power);
...
{
	mtk_limited_power = limited_power;
	mt_ppm_cpu_thermal_protect_do();
}

void custom_ppm_cpu_thermal_protect(unsigned int limited_power)
{
	custom_limited_power = limited_power;
	mt_ppm_cpu_thermal_protect_do();
}
