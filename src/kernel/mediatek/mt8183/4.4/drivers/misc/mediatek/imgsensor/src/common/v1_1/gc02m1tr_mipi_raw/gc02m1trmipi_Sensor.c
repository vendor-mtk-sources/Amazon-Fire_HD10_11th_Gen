...
#include "gc02m1trmipi_Sensor.h"

/************************** Modify Following Strings for Debug **************************/
#define PFX "gc02m1tr_camera_sensor"
#define LOG_1 LOG_INF("GC02M1TR, MIPI 1LANE\n")
/****************************   Modify end    *******************************************/
...
static kal_uint16 gc02m1tr_table_write_cmos_sensor(kal_uint16 *para, kal_uint32 len)
{
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	if (reg_gain < GC02M1TR_SENSOR_GAIN_BASE)
		reg_gain = GC02M1TR_SENSOR_GAIN_BASE;
	else if (reg_gain > GC02M1TR_SENSOR_GAIN_MAX)
		reg_gain = GC02M1TR_SENSOR_GAIN_MAX;
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	kal_uint16 GC02M1TR_AGC_Param[GC02M1TR_SENSOR_GAIN_MAX_VALID_INDEX][2] = {
		{  1024,  0 },
		{  1536,  1 },
		{  2035,  2 },
		{  2519,  3 },
	...
	for (gain_index = GC02M1TR_SENSOR_GAIN_MAX_VALID_INDEX - 1; gain_index > 0; gain_index--)
		if (reg_gain >= GC02M1TR_AGC_Param[gain_index][0])
			break;

	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xb6, GC02M1TR_AGC_Param[gain_index][1]);
	temp_gain = reg_gain * GC02M1TR_SENSOR_DGAIN_BASE / GC02M1TR_AGC_Param[gain_index][0];
	write_cmos_sensor(0xb1, (temp_gain >> 8) & 0x1f);
	write_cmos_sensor(0xb2, temp_gain & 0xff);
	LOG_DBG("GC02M1TR_AGC_Param[gain_index][1] = 0x%x, temp_gain = 0x%x, reg_gain = %d\n",
		GC02M1TR_AGC_Param[gain_index][1], temp_gain, reg_gain);
...
kal_uint16 addr_data_pair_init_gc02m1tr[] = {
	/*system*/
	0xfc, 0x01,
	0xf4, 0x41,
	0xf5, 0xc0,
...
static void sensor_init(void)
{
	...
	gc02m1tr_table_write_cmos_sensor(
	    addr_data_pair_init_gc02m1tr,
		sizeof(addr_data_pair_init_gc02m1tr) /
		sizeof(kal_uint16));
}
...
UINT32 GC02M1TR_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
