...
#include <mtk_sleep_reg_md_reg_mt8183.h>
#include <mtk_eem.h>
...
#define LATCH_BUFF_SIZE         350
static char latch_info[LATCH_BUFF_SIZE];
char *get_dvfsrc_latch_info(void)
{
	return latch_info;
}

char __attribute__ ((weak)) *dvfs_get_opp_info(int opp)
{
	return "Unknown opp information";
}
/* current_dvfs_level: bit[47:44] */
/* target_level: bit[37:34] */
#define CURRENT_LEVEL(data)	((data >> (44-32)) & 0xf)
#define TARGET_LEVEL(data)	((data >> (34-32)) & 0xf)
static void spm_vcorefs_last_records_footprint(void)
{
	uint32_t dvfs_record[8];
	uint32_t index, last_index;
	uint32_t curl, targl, last_targl;
	char *p = latch_info;

	memset(latch_info, 0, sizeof(latch_info));
	index = spm_read(DVFSRC_LAST_L);
	if (index > 7)
		return;

	last_index = index ? index - 1 : 7;

	dvfs_record[0] = spm_read(DVFSRC_RECORD_0_L_1);
	dvfs_record[1] = spm_read(DVFSRC_RECORD_1_L_1);
	dvfs_record[2] = spm_read(DVFSRC_RECORD_2_L_1);
	dvfs_record[3] = spm_read(DVFSRC_RECORD_3_L_1);
	dvfs_record[4] = spm_read(DVFSRC_RECORD_4_L_1);
	dvfs_record[5] = spm_read(DVFSRC_RECORD_5_L_1);
	dvfs_record[6] = spm_read(DVFSRC_RECORD_6_L_1);
	dvfs_record[7] = spm_read(DVFSRC_RECORD_7_L_1);

	curl  = CURRENT_LEVEL(dvfs_record[index]);
	targl = TARGET_LEVEL(dvfs_record[index]);
	last_targl = TARGET_LEVEL(dvfs_record[last_index]);

	if (targl == 0 && last_targl == curl) {
		p += scnprintf(p, latch_info+LATCH_BUFF_SIZE-p,
			"Current dvfs info: %s(0x%8x)\n",
			dvfs_get_opp_info(3-curl),
			dvfs_record[index]);
	} else {
		p += scnprintf(p, latch_info+LATCH_BUFF_SIZE-p,
			"Hang in adjusting dvfs level(0x%8x)\n",
			dvfs_record[index]);
		p += scnprintf(p, latch_info+LATCH_BUFF_SIZE-p,
			"Current dvfs info: %s\n", dvfs_get_opp_info(3-curl));
		p += scnprintf(p, latch_info+LATCH_BUFF_SIZE-p,
			"Target dvfs info : %s\n", dvfs_get_opp_info(3-targl));
	}
	...
	p += scnprintf(p, latch_info+LATCH_BUFF_SIZE-p,
	"  DVFSRC_RECORD_0_L_1~3_L_1\t: 0x%08x, 0x%08x, 0x%08x, 0x%08x\n",
	dvfs_record[0], dvfs_record[1], dvfs_record[2], dvfs_record[3]);

	p += scnprintf(p, latch_info+LATCH_BUFF_SIZE-p,
	"  DVFSRC_RECORD_4_L_1~7_L_1\t: 0x%08x, 0x%08x, 0x%08x, 0x%08x\n",
	dvfs_record[4], dvfs_record[5], dvfs_record[6], dvfs_record[7]);
