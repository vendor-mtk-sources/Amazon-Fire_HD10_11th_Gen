...
int32_t CTP_I2C_READ(struct i2c_client *client, uint16_t address, uint8_t *buf, uint16_t len)
{
	...
	if (atomic_read(&ts->i2c_enable) == 0) {
		mutex_unlock(&ts->xbuf_lock);
		NVT_ERR("I2c is disabled, failed to read.\n");
		return -EIO;
	}
...
int32_t CTP_I2C_WRITE(struct i2c_client *client, uint16_t address, uint8_t *buf, uint16_t len)
{
	...
	if (atomic_read(&ts->i2c_enable) == 0) {
		mutex_unlock(&ts->xbuf_lock);
		NVT_ERR("I2c is disabled, failed to write\n");
		return -EIO;
	}
...
int32_t nvt_check_fw_status(void)
{
	...
	if (i >= retry) {
		NVT_ERR("failed, i=%d, buf[2]=0x%02X\n", i, buf[2]);
		return -1;
	} else {
		return 0;
	}
...
static ssize_t nvt_flash_read(struct file *file, char __user *buff, size_t count, loff_t *offp)
{
	...
	if ((str[1] == 0) || (str[1] > (count-2))) {
		NVT_ERR("error, str[1] len=%d\n", str[1]);
		return -EFAULT;
	}
...
#if WAKEUP_GESTURE
static ssize_t nvt_gesture_wakeup_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	NVT_LOG("ts->gesture_switch=%d\n", ts->gesture_switch);
	return scnprintf(buf, PAGE_SIZE, "%d\n", ts->gesture_switch);
}

static ssize_t nvt_gesture_wakeup_store(struct device *dev,
			struct device_attribute *attr,
			const char *buf, size_t count)
{
	ssize_t ret = count;
	NVT_LOG("current ts->gesture_switch=%d\n", ts->gesture_switch);
	...
	if (ts->TouchIsAwake) {
		if (strncmp(buf, "1", 1) == 0) {
			ts->gesture_switch = true;
			NVT_LOG("set ts->gesture_switch to %d\n", ts->gesture_switch);
		} else if (strncmp(buf, "0", 1) == 0) {
			ts->gesture_switch = false;
			NVT_LOG("set ts->gesture_switch to %d\n", ts->gesture_switch);
		} else {
			NVT_LOG("invalid input \"%s\"!\n", buf);
			ret = -EINVAL;
			goto err_exit;
		}
	} else {
		NVT_LOG("gesture switch should only be changed during wake up\n");
		ret = -EINVAL;
		goto err_exit;
	}
err_exit:
...
static DEVICE_ATTR(gesture_wakeup, S_IRUGO | S_IWUSR | S_IWGRP,
	nvt_gesture_wakeup_show, nvt_gesture_wakeup_store);
...
static struct kobject *nvt_touchscreen_link;
#define NVT_TOUCH_SYSFS_LINK "link_input_dev"

static int nvt_debugfs_tp_latency_get(void *data, u64 *val)
{
	*val = g_debugfs_enable_latency_check;
	return 0;
};

static int nvt_debugfs_tp_latency_set(void *data, u64 val)
{
	g_debugfs_enable_latency_check = val;
	return 0;
};

DEFINE_SIMPLE_ATTRIBUTE(debug_fs_tp_latency_fops, nvt_debugfs_tp_latency_get,
		nvt_debugfs_tp_latency_set, "%llu\n");


static int nvt_debugfs_power_get(void *data, u64 *val)
{
	*val = ts->TouchIsAwake;
	NVT_LOG("tp_power = %d", ts->TouchIsAwake);
	return 0;
};
static int32_t nvt_ts_resume(struct device *dev);
static int32_t nvt_ts_suspend(struct device *dev);

static int nvt_debugfs_tp_power_set(void *data, u64 val)
{
	if (val == 1) {
		nvt_ts_resume(&ts->client->dev);
		NVT_LOG("tp_power_set tpd_resume");
	} else if (val == 0) {
		nvt_ts_suspend(&ts->client->dev);
		NVT_LOG("tp_power_set suspended");
	}
	return 0;
};

DEFINE_SIMPLE_ATTRIBUTE(debug_fs_tp_power_fops, nvt_debugfs_power_get,
		nvt_debugfs_tp_power_set, "%llu\n");
...
static int32_t nvt_touch_sysfs_init(void)
{
	...
	/* Add sysfs Link*/
	nvt_touchscreen_link = kobject_create_and_add("touchscreen", NULL);
	if (nvt_touchscreen_link != NULL) {
		ret = sysfs_create_link(nvt_touchscreen_link,
				&ts->input_dev->dev.kobj,
				NVT_TOUCH_SYSFS_LINK);
		if (ret < 0)
			NVT_ERR("[focal]Failed to create sysfs link for input_dev\n");
	}
	...
	if (ts->debug_dir_touchscreen) {
		...
		debugfs_create_file("power", S_IRUSR | S_IWUSR,
				ts->debug_dir_touchscreen, ts,
				&debug_fs_tp_power_fops);

	} else {
		NVT_ERR("Cannot create debugfs\n");
	}
...
static void nvt_touch_sysfs_deinit(void)
{
	...
	sysfs_delete_link(nvt_touchscreen_link, &ts->input_dev->dev.kobj, NVT_TOUCH_SYSFS_LINK);

	kobject_put(nvt_touchscreen_link);
...
#if NVT_SW_DEBOUNCE_EN
		if (ts->single_click_gesture_flag == 1) {
			NVT_LOG("Gesture : First Effective Single Click.\n");
			keycode = gesture_key_array[13];
			ts->single_click_gesture_timer = jiffies;
			ts->single_click_gesture_flag = 0;
		} else {
			if (time_after(jiffies, ts->single_click_gesture_timer +
				msecs_to_jiffies(NVT_GESTURE_DEBOUNCE_PERIOD))) {
				NVT_LOG("Gesture : Effective Single Click.\n");
				keycode = gesture_key_array[13];
				ts->single_click_gesture_timer = jiffies;
			} else
				NVT_LOG("Gesture : Single Click during debounce.\n");
		}
#else
...
static void nvt_parse_dt(struct device *dev)
{
	...
	int32_t ret = 0;
	ts->pctrl = devm_pinctrl_get(dev);
	if (IS_ERR(ts->pctrl)) {
		ret = PTR_ERR(ts->pctrl);
		NVT_ERR("[NVT][ERROR] Cannot find pctrl!\n");
	}
	ts->pin_state_suspend = pinctrl_lookup_state(ts->pctrl, "nvt_i2c0_suspend");
	if (IS_ERR(ts->pin_state_suspend)) {
		ret = PTR_ERR(ts->pin_state_suspend);
		NVT_ERR("[NVT][ERROR] Cannot find pin_state_suspend %d!\n", ret);
	}
	ts->pin_state_resume = pinctrl_lookup_state(ts->pctrl, "nvt_i2c0_rusume");
	if (IS_ERR(ts->pin_state_resume)) {
		ret = PTR_ERR(ts->pin_state_resume);
		NVT_ERR("[NVT][ERROR] Cannot find pin_state_resume %d!\n", ret);
	}
...
static irqreturn_t nvt_ts_work_func(int irq, void *data)
{
	...
	pr_debug("[NVT]nvt_ts_work_func enter\n");
#if defined(CONFIG_AMAZON_METRICS_LOG) || defined(CONFIG_AMZN_METRICS_LOG)
	...
	if (atomic_read(&ts->i2c_enable) == 0) {
		NVT_ERR("I2c is disabled, failed to read touch points.\n");
		goto XFER_ERROR;
	}
	...
	if (g_debugfs_enable_latency_check) {
		snprintf(buffer, sizeof(buffer),
				"touch_event:def:read_latency_us=%lld;CT;1:NR",
				(ktime_to_ns(ktime_get())-ktime_to_ns(g_ts_irq_in))/1000LL);
	...
	//--- dump I2C buf ---
	for (i = 0; i < 10; i++) {
		pr_debug("[NVT]%02X %02X %02X %02X %02X %02X  ",
	...
	for (i = 0; i < ts->max_touch_num; i++) {
		...
		if (((point_data[position] & 0x07) == 0x01) || ((point_data[position] & 0x07) == 0x02)) {
			...
			finger_cnt++;
			pr_debug("[NVT]P%d c:%d(x:%d, y:%d)[p:%d,w:%d]DOWN!\n",
				  input_id, finger_cnt,
				  input_x, input_y,
				  input_p, input_w);
...
*/
		...
		if (pen_format_id != 0xFF) {
			...
			/* pen_format_id = 0xFF, i.e. no pen present */
		...
		if (g_debugfs_enable_latency_check) {
			snprintf(buffer, sizeof(buffer),
					"touch_event:def:num=%d;CT;1,TYPE_B=%d;CT;1,"
					"report_latency_us=%lld;CT;1:NR",
					finger_cnt,
					MT_PROTOCOL_B,
					(ktime_to_ns(ktime_get())-ktime_to_ns(g_ts_irq_in))/1000LL);
			log_to_metrics(ANDROID_LOG_INFO, "Touch", buffer);
...
static int32_t nvt_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	pinctrl_select_state(ts->pctrl, ts->pin_state_resume);
	atomic_set(&ts->i2c_enable, 1);
	mutex_init(&ts->lock);
	...
	queue_delayed_work(nvt_fwu_wq, &ts->nvt_fwu_work,
		msecs_to_jiffies(NVT_FW_UPDATE_WORK_DELAY));
#endif
	...
	nvt_irq_enable(true);
	ret = nt36523_share_request_powerpin_resource(&ts->client->dev);
	if (ret < 0)
		goto err_share_powerpin_resource_failed;
...
#else
err_share_powerpin_resource_failed:
...
#if WAKEUP_GESTURE
	if (ts->gesture_switch == false) {
		nt36523_share_powerpin_off();
		atomic_set(&ts->i2c_enable, 0);
		pinctrl_select_state(ts->pctrl, ts->pin_state_suspend);
	}
#else
	nt36523_share_powerpin_off();
	atomic_set(&ts->i2c_enable, 0);
