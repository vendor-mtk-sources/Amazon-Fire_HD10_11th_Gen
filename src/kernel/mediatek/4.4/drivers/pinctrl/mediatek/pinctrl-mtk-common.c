		...
		unsigned int pin, unsigned char driving)
{
	...
	if (driving >= MTK_I2C_EH_DRIVE_000 && driving <= MTK_I2C_EH_DRIVE_111) {
		if (pctl->devdata->pin_i2c_eh_grps)
		pctl->devdata->mtk_pctl_set_i2c_eh(pctl, pctl->devdata->pin_i2c_eh_grps,
						pctl->devdata->n_pin_i2c_eh, pin, driving);
		return 0;
	}
