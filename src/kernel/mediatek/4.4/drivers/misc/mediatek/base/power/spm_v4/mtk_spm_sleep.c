...
static char suspend_status[64];
char *spm_suspend_map_status(u32 step)
{
	char *p = suspend_status;
	char *buff_end = suspend_status + sizeof(suspend_status);

	memset(suspend_status, 0, sizeof(suspend_status));
	if (step == 0)
		return "leave suspend successful !!";

	switch (step & ((1U << CPU_FOOTPRINT_SHIFT) - 1)) {
	case SPM_SUSPEND_ENTER:
		p += scnprintf(p, buff_end-p, "Stuck in suspend entry");
		break;
	case SPM_SUSPEND_ENTER_UART_SLEEP:
		p += scnprintf(p, buff_end-p, "Stuck in sleep UAR");
		break;
	case SPM_SUSPEND_ENTER_WFI:
		p += scnprintf(p, buff_end-p, "Stuck in enter wfi");
		break;
	case SPM_SUSPEND_LEAVE_WFI:
		p += scnprintf(p, buff_end-p, "Stuck in leave WFI");
		break;
	case SPM_SUSPEND_ENTER_UART_AWAKE:
		p += scnprintf(p, buff_end-p, "Stuck in uart awake");
		break;
	case SPM_SUSPEND_LEAVE:
		p += scnprintf(p, buff_end-p, "Stuck in suspend exit");
		break;
	default:
		p += scnprintf(p, buff_end-p, "invalid status !!");
		break;
	}
