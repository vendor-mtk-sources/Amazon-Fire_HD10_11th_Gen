...
P_BSS_DESC_T scanAddToBssDesc(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	if (!rlmDomainIsValidRfSetting(prAdapter, prBssDesc->eBand, prBssDesc->ucChannelNum, prBssDesc->eSco,
		...
		DBGLOG(RLM, TRACE, "ScanAddToBssDesc IE Information\n");
		DBGLOG(RLM, TRACE, "IE Length = %d\n", u2IELength);
		DBGLOG_MEM8(RLM, TRACE, pucDumpIE, u2IELength);
...
#if CFG_SUPPORT_ANT_DIVERSITY
	DBGLOG(SCN, TRACE, "eState:%d, [%pM]:%d, scnIndex:%d\n", prAisAntSwInfo->eState,
			prWlanBeacon->aucBSSID,
			HAL_RX_STATUS_GET_RCPI(prSwRfb->prRxStatusGroup3),
			prAisAntSwInfo->ucAntScanIndex);
	if ((prAisAntSwInfo->eState == AIS_ANT_SW_DETECT ||
			prAisAntSwInfo->ucTestCase == ENUM_AS_TEST_SCAN) &&
			EQUAL_MAC_ADDR(prWlanBeacon->aucBSSID, prAisAntSwInfo->aucBSSID)) {
		if (prAisAntSwInfo->ucAntScanIndex < AIS_ANT_SW_SCN_RCPI_NUM) {
			prAisAntSwInfo->aucAntScanRCPI[prAisAntSwInfo->ucAntScanIndex++] =
...
#endif
	DBGLOG(SCN, INFO, "Find ssid XXXXXX in %d BSSes, result %d\n",
		prBSSDescList->u4NumElem, prCurEssLink->u4NumElem);
	DBGLOG(SCN, TRACE, "ssid = %s\n", prConnSettings->aucSSID);
}
...
try_again:
	LINK_FOR_EACH_ENTRY(prBssDesc, prEssLink, rLinkEntryEss, BSS_DESC_T) {
		if (prConnSettings->eConnectionPolicy == CONNECT_BY_BSSID) {
			...
			if (!scanSanityCheckBssDesc(prAdapter, prBssDesc, eBand, ucChannel, fgIsFixedChannel))
				continue;
			DBGLOG(SCN, INFO,
			       "Selected %pM base on bssid, when find ssid XXXXXX, %pM in %d BSSes, fix channel %d.\n",
			       prBssDesc->aucBSSID,
			...
			if (prBssDesc->prBlack) {
				if (prBssDesc->prBlack->blackListSource & AIS_BLACK_LIST_FROM_FWK)
					DBGLOG(SCN, INFO, "ssid XXXXXX(%pM) is in FWK blacklist, skip it\n",
								prBssDesc->aucBSSID);
				continue;
			}
			...
			if (prBssDesc->prBlack->blackListSource	& AIS_BLACK_LIST_FROM_FWK) {
				DBGLOG(SCN, INFO, "Although trying blacklist,ssid XXXXXX(%pM) is in FWK blacklist, skip it\n",
							prBssDesc->aucBSSID);
				continue;
			}
	...
	if (prCandBssDesc) {
		DBGLOG(SCN, INFO,
			"Selected %pM, Score %d when find ssid XXXXXX, %pM in %d BSSes, fix channel %d, blacklist %d\n",
...
#endif
	DBGLOG(SCN, INFO, "Selected None when find ssid XXXXXX, %pM in %d BSSes, fix channel %d, blacklist %d\n",
