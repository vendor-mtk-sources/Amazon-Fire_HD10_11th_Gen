				 ...
				 const char *buf, size_t count)
{
	...
	if (err < 0) {
		pr_err("[focal]Failed to convert value\n");
		goto failed;
	}

	if (focal_suspend_flag) {
		pr_err("[focal]In suspend state, delay gesture setting\n");
		g_fts_gesture_wakeup_delayed = value;
		err = count;
		goto failed;
	}
	...
	case 0:
		/* Normal mode */
		pr_info("[focal]slide wakeup disable\n");
		g_fts_slide_wakeup = 0;
		break;
	case 1:
		/* Gesture mode */
		pr_info("[focal]slide wakeup enabled\n");
		g_fts_slide_wakeup = 1;
		break;
	default:
		pr_err("[focal]Invalid value\n");
		err = -EINVAL;
		goto failed;
	}
...
int fts_create_sysfs(struct i2c_client *client)
{
	...
	if (fts_touchscreen_link != NULL) {
		err = sysfs_create_link(fts_touchscreen_link,
				&client->dev.kobj,
				"link_input_dev");
		if (err < 0)
			pr_err("[focal]Failed to create sysfs link for input_dev\n");
	}
...
void fts_release_sysfs(struct i2c_client *client)
{
	...
	if (fts_touchscreen_link != NULL) {
		sysfs_remove_link(fts_touchscreen_link, "link_input_dev");
		kobject_put(fts_touchscreen_link);
	}
}
			       ...
			       size_t count, loff_t *ppos)
{
	...
	unsigned char writebuf[FTS_PACKET_LENGTH + 1];
	int buflen = count;
	int writelen = 0;
	int ret = 0;

	if (buflen > sizeof(writebuf)) {
		dev_err(&client->dev, "[focal] %s: buffer length too long\n",
			__func__);
		return -EFAULT;
	}
